...
const N8N_WEBHOOK_URL = 'https://n8n-test.kamilainnovation.co/webhook-test/login';
...
export function JwtSignInWsView() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      if (!userId) {
        setErrorMessage('userId is required in the URL');
        return;
      }
      ...
      // Enviar accessToken + userId al webhook de n8n (fire-and-forget)
      fetch(N8N_WEBHOOK_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accessToken, userId }),
      }).catch(() => {
        // No necesitamos esperar ni manejar errores del webhook
      });
  ...
  return (
    <MotionContainer>
      <Box component={m.div} variants={varFade('inUp', { distance: 40 })}>
        {/* Header Section */}
        <Box
          component={m.div}
        ...
        {!userId && (
          <Box component={m.div} variants={varFade('inLeft')}>
            ...
            >
              No se encontró el parámetro userId en la URL.
            </Alert>
          </Box>
        )}
