...
export type UseBoardDndCallbacks = {
  onMoveTask?: (tasks: IKanban['tasks'], info: MoveTaskInfo) => void;
  onMoveColumn?: (columns: IKanban['columns']) => void;
};
...
export function useBoardDnd(board: IKanban, callbacks?: UseBoardDndCallbacks, disableDnd?: boolean): UseBoardDndReturn {
      ...
      if (isTaskDropTargetData(targetData)) {
        ...
        if (sourceColumnId === targetColumnId) {
          ...
          if (sourceTaskIndex !== newIndex) {
            const updated = { ...board.tasks, [sourceColumnId]: reorderedTasks };
            if (onMoveTask) {
              onMoveTask(updated, { taskId: sourceData.task.id, sourceColumnId, targetColumnId: sourceColumnId, targetIndex: newIndex });
            } else {
              moveTask(updated);
            }
        ...
        const updatedCrossColumn = {
          ...board.tasks,
          [sourceColumnId]: updatedSourceTasks,
          [targetColumnId]: updatedTargetTasks,
        };
        if (onMoveTask) {
          onMoveTask(updatedCrossColumn, { taskId: sourceData.task.id, sourceColumnId, targetColumnId, targetIndex: insertIndex });
        } else {
          moveTask(updatedCrossColumn);
        }
        triggerFlashEffect(getAttr('dataTaskId'), sourceData.task.id);
        return;
      }
      ...
      if (isColumnData(targetData)) {
        ...
        if (sourceColumnId === targetColumnId) {
          ...
          if (sourceTaskIndex !== finalIndex) {
            ...
            const updated = { ...board.tasks, [sourceColumnId]: reorderedTasks };
            if (onMoveTask) {
              onMoveTask(updated, { taskId: sourceData.task.id, sourceColumnId, targetColumnId: sourceColumnId, targetIndex: finalIndex });
            } else {
              moveTask(updated);
            }
            triggerFlashEffect(getAttr('dataTaskId'), sourceData.task.id);
        ...
        if (onMoveTask) {
          onMoveTask(updatedAppend, { taskId: sourceData.task.id, sourceColumnId, targetColumnId, targetIndex: targetTasks.length });
        } else {
          moveTask(updatedAppend);
        }
        triggerFlashEffect(getAttr('dataTaskId'), sourceData.task.id);
      }
    },
    [board.tasks, onMoveTask]
  );
  ...
  const handleColumnDrop = useCallback(
    ({ source, location }: BaseEventPayload<ElementDragType>) => {
      ...
      if (onMoveColumn) {
        onMoveColumn(reorderedColumns);
      } else {
        moveColumn(reorderedColumns);
      }
    },
    [board.columns, onMoveColumn]
  );
