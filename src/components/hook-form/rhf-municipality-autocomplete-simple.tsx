...
export type MunicipalityAutocompleteSimpleBaseProps = Omit<
  AutocompleteProps<IMunicipalityOption, Multiple, DisableClearable, FreeSolo>,
  ExcludedProps
>;

export type MunicipalityAutocompleteSimpleProps = MunicipalityAutocompleteSimpleBaseProps & {
  name: string;
  label?: string;
  placeholder?: string;
  ...
  disabled?: boolean;
  slotProps?: MunicipalityAutocompleteSimpleBaseProps['slotProps'] & {
    textField?: Partial<TextFieldProps>;
  };
};

export function MunicipalityAutocompleteSimple({
  name,
  label,
  slotProps,
  helperText,
  ...
  disabled = false,
  ...other
}: MunicipalityAutocompleteSimpleProps) {
  ...
  // Buscar municipios mientras el usuario escribe (solo cuando es input del usuario)
  useEffect(() => {
      ...
      render={({ field, fieldState: { error } }) => (
        <Autocomplete
          ...
          onInputChange={(event, newInputValue, reason) => {
            // Solo actualizar inputValue cuando el usuario está escribiendo
            if (reason === 'input') {
