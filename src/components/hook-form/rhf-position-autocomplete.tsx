      ...
      render={({ field, fieldState: { error } }) => (
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            const positionValue = (newValue && typeof newValue === 'object' && !Array.isArray(newValue))
              ? newValue as IPositionOption
              : null;
            setValue(name, positionValue, { shouldValidate: true, shouldDirty: true, shouldTouch: true });
            if (positionValue) {
              clearErrors(name);
            }
            if (onPositionChange) {
