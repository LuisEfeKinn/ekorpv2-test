...
export function AiRouteChatPanel({ onRouteGenerated, onError, onGenerationStart }: Props) {
    ...
    const loadCatalogs = async () => {
      ...
      try {
        const [positionsRes, competenciesRes, levelsRes] = await Promise.all([
          ...
          GetCompetenciesPaginationService({ page: 1, perPage: 20 }),
        ...
        const compData = competenciesRes?.data;
  ...
  }, [t, inputValue, isStreaming, messages, startStream, selectedProvider, selectedModel, temperature, onError, onGenerationStart, catalogContext]);
              ...
              <Box sx={{ mt: 3 }}>
                ...
                <Stack spacing={3}>
                  ...
                  <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ gap: 1 }}>
                    {jsonData.difficulty && (
                      <Chip
                        label={jsonData.difficulty}
                        size="small"
                        color="primary"
                        variant="soft"
                      />
                    )}
                    {jsonData.estimatedDuration && (
                      <Chip
                        icon={<Iconify icon="solar:clock-circle-bold" width={16} />}
                        label={jsonData.estimatedDuration}
                        size="small"
                        variant="soft"
                      />
                    )}
                  ...
                  {/* Objectives */}
                  {jsonData.objectives?.length > 0 && (
                    <Box>
                      <Typography variant="subtitle2" gutterBottom>
                        {t('ai-route-generation.chat.objectives')}
                      </Typography>
