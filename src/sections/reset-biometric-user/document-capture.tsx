...
import { PublicUploadDocumentService } from 'src/services/auth/biometricRegister.service';
...
export function DocumentCapture({ side, documentId, onCapture, onSkip, allowManualUpload = false }: DocumentCaptureProps) {
  ...
  const processDocument = useCallback(async () => {
    ...
    try {
      ...
      formData.append('documentId', documentId); // documentId viene desde la URL como prop
      formData.append('process', 'accessRecovery');
      formData.append('documentSyde', side);

      // Llamar al servicio de subida de documento
      const uploadResponse = await PublicUploadDocumentService(formData);
  ...
  }, [capturedImage, side, onCapture, documentId, base64ToFile, t, startCamera]);
