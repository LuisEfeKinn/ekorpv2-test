...
export function LearningObjectTableFiltersResult({ filters, onFilters, onReset, totalResults, sx }: Props) {
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={onReset} sx={sx}>
      <FiltersBlock label={`${t('learning-objects.table.filters.status')}:`} isShow={filters.status !== 'all'}>
        <Chip
          {...chipProps}
