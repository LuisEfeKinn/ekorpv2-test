...
export function IntegrationsView() {
  ...
  const handleSyncRow = useCallback(
    async (id: string) => {
      try {
        const response = await GetSyncCoursePlatformByIdService(id);
        toast.success(t(response?.data?.message || 'integrations.messages.success.synced'));
        loadData(); // Recargar datos
      } catch (error: any) {
        console.error('Error syncing integration:', error);
        toast.error(t(error?.message || 'integrations.messages.error.syncing'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        ...
        toast.error(t(error?.message || 'integrations.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
