...
export function AiProgramChatPanel({ onProgramGenerated, onError, onGenerationStart }: Props) {
  ...
  // Load integration instances on mount
  useEffect(() => {
    const loadInstances = async () => {
      ...
      try {
        ...
        if (items.length > 0 && !selectedInstance) {
  ...
  // Search instances when input changes
  useEffect(() => {
    if (!instanceSearchInput.trim()) {
      return () => {};
    }
    ...
    const searchTimeout = setTimeout(async () => {
      try {
        ...
        console.error('Error searching instances:', err);
      }
    }, 300);
  ...
  const catalogContext = useMemo(() => {
    ...
    if (courseCatalog.length > 0) {
      const list = courseCatalog.map((c) =>
        `- lmsCourseId: "${c.lmsCourseId}" | Name: "${c.displayName}" | Description: "${c.shortDescription}"`
      ).join('\n');
          ...
          <Box sx={{ p: 2, bgcolor: 'grey.50' }}>
            <Stack spacing={2}>
              ...
              <Autocomplete
                ...
                onInputChange={(_, newInputValue) => setInstanceSearchInput(newInputValue)}
...
function ProgramMessageBubble({ message, courseCatalog = [] }: MessageBubbleProps) {
  ...
  const extractJsonData = (content: string): any | null => {
    try {
      ...
      const catalogMap = new Map(courseCatalog.map((c) => [c.lmsCourseId, c]));
      return {
        ...parsed,
  ...
  try {
    ...
    if (jsonText.startsWith('{') || jsonText.startsWith('[')) {
      ...
      if (parsed.name && parsed.courses) {
        ...
        const catalogMap = new Map(catalog.map((c) => [c.lmsCourseId, c]));
