...
export function UsersGeneral({ currentUser }: Props) {
  ...
  const defaultAvatarUrl = currentUser?.avatarUrl || currentUser?.avatar || null;
  ...
  useEffect(() => {
    const fetchRoles = async () => {
      const response = await GetRolesPaginationService({ page: 1, perPage: 100 });
        ...
        <Grid size={{ xs: 12, md: 8 }}>
          <Card sx={{ p: 3 }}>
            ...
            >
              ...
              <Field.Phone
                name="tel"
                label={t('users.form.fields.tel.label')}
                defaultCountry="CO"
              />
