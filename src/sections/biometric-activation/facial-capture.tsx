...
interface FacialCaptureProps {
  onSuccess: (identityId: string) => void; // Callback cuando todo sea exitoso
  onError?: (error: Error) => void;
}

export function FacialCapture({ onSuccess, onError }: FacialCaptureProps) {
  ...
  const handleAnalysisComplete = useCallback(async () => {
    ...
    try {
      ...
      // Subir imagen
      const uploadResponse = await UploadLivenessService(formData);
      ...
      // Intentar obtener identityId de múltiples ubicaciones posibles
      const identityId =
        uploadResponse.data?.data?.identityId ||
        uploadResponse.data?.identityId ||
        uploadResponse.data?.data?.biometricValidationId ||
        uploadResponse.data?.biometricValidationId ||
      ...
      const verificationResponse = await BiometricValidationVerifyService(identityId);
      ...
      // Paso 3: Activar usuario con biometría
      toast.info(t('facialCapture.activatingBiometric'));

      const activationResponse = await BiometricActivationService();

      if (activationResponse.data?.statusCode !== 200 && activationResponse.status !== 200) {
        throw new Error(activationResponse.data?.message || t('facialCapture.biometricActivationError'));
      }

      toast.success(t('facialCapture.biometricActivated'));
  ...
  }, [capturedImage, t, onSuccess, onError]);
