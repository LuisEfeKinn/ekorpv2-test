...
export function AiCoursesEditView({ id }: Props) {
    ...
    async (formData: IAiCourseFormData) => {
      ...
      try {
        ...
        if (response.statusCode === 200) {
          toast.success(t('ai-course-generation.messages.success.updated'));
          setCourse(response.data);
        }
      } catch (error) {
        console.error('Error updating course:', error);
        toast.error(t('ai-course-generation.messages.error.updating'));
      } finally {
        setIsSaving(false);
        router.push(paths.dashboard.ai.courseGenerator.root);
      }
  ...
  if (isLoading) {
    return (
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('ai-course-generation.edit.title')}
          links={[
            ...
            { name: t('ai-course-generation.edit.title') },
          ]}
          sx={{ mb: { xs: 3, md: 5 } }}
        />
  ...
  return (
    <DashboardContent maxWidth="xl">
      <CustomBreadcrumbs
        heading={course?.title || t('ai-course-generation.edit.title')}
        links={[
          { name: t('breadcrumbs.dashboard'), href: paths.dashboard.root },
          { name: t('ai-course-generation.title'), href: paths.dashboard.ai.courseGenerator.root },
          { name: course?.title || t('ai-course-generation.edit.title') },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
