...
export function AiCoursesCreateChatView() {
  ...
  const [videoCredits, setVideoCredits] = useState<{ consumed: number; remaining: number } | null>(null);

  // Handle AI course generation
  const handleCourseGenerated = useCallback((data: any, credits?: { consumed: number; remaining: number }) => {
    // Save credits info if provided
    if (credits) {
    ...
    toast.success(t('ai-course-generation.messages.success.generated'));
  }, [t]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (response.statusCode === 200 || response.statusCode === 201) {
        toast.success(t('ai-course-generation.messages.success.saved'));
        router.push(paths.dashboard.ai.courseGenerator.root);
      }
    } catch (error) {
      console.error('Error saving course:', error);
      toast.error(t('ai-course-generation.messages.error.saving'));
    } finally {
      setIsSaving(false);
    }
  }, [courseData, router, t]);
  ...
  return (
    <DashboardContent maxWidth="xl">
      <CustomBreadcrumbs
        heading={t('ai-course-generation.create.title')}
        links={[
          { name: t('ai-course-generation.breadcrumbs.dashboard'), href: paths.dashboard.root },
          { name: t('ai-course-generation.title'), href: paths.dashboard.ai.courseGenerator.root },
          { name: t('ai-course-generation.create.title') },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
        ...
        <Stepper activeStep={activeStep} alternativeLabel>
          {STEPS.map((step, index) => (
            <Step key={step}>
              <StepLabel
                optional={
                  index === 0 ? (
                    ...
                    >
                      {t('ai-course-generation.steps.skipToManual')}
                    </Button>
                  ) : null
                }
                icon={
              ...
              >
                {t(`ai-course-generation.steps.${step}`)}
              </StepLabel>
            </Step>
          ))}
        </Stepper>
      ...
      {/* Credits Info Banner */}
      {videoCredits && activeStep === 0 && (
        <Card sx={{ mt: 2, p: 2, bgcolor: 'success.lighter' }}>
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Typography variant="caption" color="text.secondary">
                {t('ai-course-generation.chat.videoCreditsInfo', {
                  consumed: videoCredits.consumed,
                  remaining: videoCredits.remaining,
                })}
              </Typography>
            </Box>
          </Stack>
      ...
      <Stack direction="row" justifyContent="space-between" sx={{ mt: 3 }}>
        ...
        >
          {activeStep === 0 ? t('ai-course-generation.actions.cancel') : t('ai-course-generation.actions.back')}
        </Button>
        ...
        <Stack direction="row" spacing={2}>
          {activeStep === 0 && courseData && (
            ...
            >
              {t('ai-course-generation.actions.continue')}
            </Button>
          )}
          ...
          {activeStep === 2 && (
            <>
              ...
              >
                {t('ai-course-generation.actions.editMore')}
              </Button>
              <Button
                variant="contained"
                onClick={handleSave}
              ...
              >
                {t('ai-course-generation.actions.saveCourse')}
              </Button>
            </>
          )}
        </Stack>
