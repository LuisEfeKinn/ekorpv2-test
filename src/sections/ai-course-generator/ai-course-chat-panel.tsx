...
// Sanitize JSON string to fix common issues with embedded HTML/code
function sanitizeJsonString(jsonStr: string): string {
  // This function tries to fix common JSON syntax errors
  // caused by unescaped characters in HTML content
  
  let sanitized = jsonStr;
  
  // Fix control characters (newlines, tabs, etc.) inside JSON strings
  // We need to be careful to only replace these inside string values, not in the JSON structure itself
  // This is a simplified approach that works for most cases
  
  // Replace literal newlines with escaped newlines within quoted strings
  // This regex tries to match quoted strings and replace newlines inside them
  sanitized = sanitized.replace(/"html":\s*"((?:[^"\\]|\\.)*)"/g, (match, content) => {
    // For the html field specifically, clean up problematic characters
    const cleaned = content
      .replace(/\r\n/g, ' ') // Replace Windows line endings with space
      .replace(/\n/g, ' ')   // Replace Unix line endings with space
      .replace(/\r/g, ' ')   // Replace Mac line endings with space
