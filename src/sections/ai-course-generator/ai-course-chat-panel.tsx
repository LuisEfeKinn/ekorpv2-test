...
// Helper function to find the provider and model with isDefault=true
  ...
  const quickPrompts: IAiGenerationPrompt[] = [
    {
      topic: t('ai-course-generation.chat.quickPrompts.webDev'),
      description: t('ai-course-generation.chat.quickPrompts.webDevDesc'),
      difficulty: 'beginner',
    },
    {
      topic: t('ai-course-generation.chat.quickPrompts.machineLearning'),
      description: t('ai-course-generation.chat.quickPrompts.machineLearningDesc'),
      difficulty: 'intermediate',
    },
    {
      topic: t('ai-course-generation.chat.quickPrompts.leadership'),
      description: t('ai-course-generation.chat.quickPrompts.leadershipDesc'),
      difficulty: 'advanced',
    },
  ];

  const handleQuickPrompt = (prompt: IAiGenerationPrompt) => {
    const promptText = `${t('ai-course-generation.chat.generateCourseFor')} "${prompt.topic}". ${prompt.description || ''} ${t('ai-course-generation.chat.difficultyLevel')}: ${prompt.difficulty}.`;
      ...
      <Card>
        ...
        >
          <Stack direction="row" alignItems="center" spacing={1}>
            ...
            <Box>
              <Typography variant="subtitle1">{t('ai-course-generation.chat.title')}</Typography>
              <Stack direction="row" spacing={0.5} alignItems="center" flexWrap="wrap">
                <Chip
                  label={selectedProvider?.name || t('ai-course-generation.chat.noProvider')}
                  size="small"
                  variant="soft"
                  color="primary"
                ...
                />
                <Chip
                  label={selectedImageProvider?.name || t('ai-course-generation.chat.noProvider')}
                  size="small"
                  variant="soft"
                  color="success"
                ...
                />
                <Chip
                  label={selectedVideoProvider?.name || t('ai-course-generation.chat.noProvider')}
                  size="small"
                  variant="soft"
                  color="error"
            ...
            <Stack spacing={2}>
              ...
              {isLoadingProviders && (
                <Alert severity="info" icon={<CircularProgress size={16} />}>
                  {t('ai-course-generation.chat.loadingProviders') || 'Cargando proveedores...'}
                </Alert>
              )}
              {providersError && (
                <Alert severity="error">
                  {t('ai-course-generation.chat.providersError') || 'Error al cargar proveedores'}: {providersError}
                </Alert>
              )}

              {/* Language Model Settings */}
              <Typography variant="subtitle2" color="text.secondary">
                {t('ai-course-generation.chat.languageModel') || 'Modelo de Lenguaje'}
              </Typography>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <FormControl size="small" sx={{ minWidth: 150 }}>
                  <InputLabel>{t('ai-course-generation.chat.provider')}</InputLabel>
                  <Select
                    value={selectedProvider?.id || ''}
                    label={t('ai-course-generation.chat.provider')}
                ...
                <FormControl size="small" sx={{ minWidth: 180 }}>
                  <InputLabel>{t('ai-course-generation.chat.model')}</InputLabel>
                  <Select
                    value={selectedModel?.id || ''}
                    label={t('ai-course-generation.chat.model')}
              ...
              <Typography variant="subtitle2" color="text.secondary">
                {t('ai-course-generation.chat.imageModel') || 'Modelo de Imagen'}
              </Typography>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <FormControl size="small" sx={{ minWidth: 150 }}>
                  <InputLabel>{t('ai-course-generation.chat.provider') || 'Proveedor'}</InputLabel>
                  <Select
                    value={selectedImageProviderId}
                    label={t('ai-course-generation.chat.provider') || 'Proveedor'}
                    onChange={(e) => {
                ...
                <FormControl size="small" sx={{ minWidth: 180 }}>
                  <InputLabel>{t('ai-course-generation.chat.model') || 'Modelo'}</InputLabel>
                  <Select
                    value={selectedImageModelId}
                    label={t('ai-course-generation.chat.model') || 'Modelo'}
              ...
              <Typography variant="subtitle2" color="text.secondary">
                {t('ai-course-generation.chat.videoModel') || 'Modelo de Video'}
              </Typography>
              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                <FormControl size="small" sx={{ minWidth: 150 }}>
                  <InputLabel>{t('ai-course-generation.chat.provider') || 'Proveedor'}</InputLabel>
              ...
              {selectedVideoProvider?.name === 'Propietario' && (
                <Box sx={{ mt: 2, p: 2, bgcolor: 'primary.lighter', borderRadius: 1 }}>
                  <Typography variant="caption" color="primary.dark" sx={{ mb: 2, display: 'block', fontWeight: 600 }}>
                    {t('ai-course-generation.chat.proprietaryOptions')}
                  </Typography>
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
                    <FormControl size="small" sx={{ minWidth: 180 }}>
                      <InputLabel>{t('ai-course-generation.chat.sceneDuration')}</InputLabel>
                      <Select
                        value={proprietaryDurationScenes}
                        label={t('ai-course-generation.chat.sceneDuration')}
                        onChange={(e) => setProprietaryDurationScenes(Number(e.target.value))}
                      >
                        <MenuItem value={4}>4 {t('ai-course-generation.chat.seconds')}</MenuItem>
                        <MenuItem value={6}>6 {t('ai-course-generation.chat.seconds')}</MenuItem>
                        <MenuItem value={12}>12 {t('ai-course-generation.chat.seconds')}</MenuItem>
                      </Select>
                    </FormControl>
                    ...
                    <TextField
                      size="small"
                      type="number"
                      label={t('ai-course-generation.chat.scenesNumber')}
                      value={proprietaryScenesNumber}
                      onChange={(e) => setProprietaryScenesNumber(Math.max(1, Math.min(20, Number(e.target.value))))}
                      inputProps={{ min: 1, max: 20 }}
                      sx={{ minWidth: 150 }}
                      helperText={`${t('ai-course-generation.chat.totalDuration')}: ~${proprietaryDurationScenes * proprietaryScenesNumber}s`}
                    />

                    <FormControl size="small" sx={{ minWidth: 180 }}>
                      <InputLabel>{t('ai-course-generation.chat.imageModel') || 'Modelo de Imagen'}</InputLabel>
                      <Select
                        value={proprietaryImageModel}
                        label={t('ai-course-generation.chat.imageModel') || 'Modelo de Imagen'}
              ...
              <Box sx={{ px: 1, pb: 1 }}>
                <Typography variant="caption" gutterBottom>
                  {t('ai-course-generation.chat.temperature')}: {temperature}
                </Typography>
                <Slider
                  size="small"
                  value={temperature}
                  ...
                  step={0.1}
                  marks={[
                    { value: 0, label: t('ai-course-generation.chat.precise') },
                    { value: 1, label: t('ai-course-generation.chat.creative') },
                  ]}
                  sx={{
                    mt: 2,
                    mb: 1.5,
          ...
          <Stack spacing={3} alignItems="center" justifyContent="center" sx={{ py: 6 }}>
            ...
            <Box sx={{ textAlign: 'center' }}>
              <Typography variant="h6" gutterBottom>
                {t('ai-course-generation.chat.welcome')}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 400 }}>
                {t('ai-course-generation.chat.welcomeMessage')}
              </Typography>
            </Box>
            ...
            <Stack spacing={1} sx={{ width: '100%', maxWidth: 500 }}>
              <Typography variant="subtitle2" color="text.secondary">
                {t('ai-course-generation.chat.quickStart')}
              </Typography>
          ...
          <Box sx={{ px: 2, py: 1 }}>
            <Stack direction="row" alignItems="center" spacing={2}>
              ...
              <Typography variant="body2" color="text.secondary">
                {t('ai-course-generation.chat.generatingBanner') || 'Generando banner del curso...'}
              </Typography>
              <Box sx={{ flex: 1 }} />
            </Stack>
          </Box>
        ...
        {isGeneratingImages && (
          <Box sx={{ px: 2, py: 1 }}>
            <Stack direction="row" alignItems="center" spacing={2}>
              <CircularProgress size={20} />
              <Typography variant="body2" color="text.secondary">
                {t('ai-course-generation.chat.generatingImages', {
              ...
              <Box sx={{ flex: 1 }}>
                <Typography variant="body2" color="text.secondary">
                  {selectedVideoProvider?.name === 'Propietario' && proprietaryVideoStatus
                    ...
                    : t('ai-course-generation.chat.generatingVideos', {
                ...
                {selectedVideoProvider?.name === 'Propietario' && (
                  <Typography variant="caption" color="primary.main" sx={{ display: 'block', mt: 0.5 }}>
                    {t('ai-course-generation.chat.proprietaryVideoDisclaimer')}
                  </Typography>
                )}
              </Box>
              <LinearProgress
      ...
      <Card sx={{ position: 'sticky', bottom: 16, zIndex: 10, mt: 2, boxShadow: 3 }}>
        <Stack direction="row" spacing={1} sx={{ p: 2 }}>
          <TextField
            ...
            placeholder={t('ai-course-generation.chat.placeholder')}
              ...
              <Box sx={{ mt: 3 }}>
                ...
                <Stack spacing={3}>
                  ...
                  <Stack direction="row" spacing={1} flexWrap="wrap" sx={{ gap: 1 }}>
                    {jsonData.difficulty && (
                      <Chip
                        label={t(`ai-course-generation.difficulty.${jsonData.difficulty}`)}
                        size="small"
                        color="primary"
                        variant="soft"
                      />
                  ...
                  {jsonData.objectives && jsonData.objectives.length > 0 && (
                    <Box>
                      <Typography variant="subtitle2" gutterBottom>
                        {t('ai-course-generation.preview.objectives')}
                      </Typography>
                  ...
                  {jsonData.sections && jsonData.sections.length > 0 && (
                    <Box>
                      <Typography variant="subtitle2" gutterBottom>
                        {t('ai-course-generation.preview.content')} ({jsonData.sections.length} {t('ai-course-generation.sections')})
                      </Typography>
                      <Stack spacing={1}>
                  ...
                  <Alert severity="success" icon={<Iconify icon="solar:check-circle-bold" />}>
                    <Typography variant="body2">
                      {t('ai-course-generation.chat.courseStructureGenerated')}
                    </Typography>
                  </Alert>
                </Stack>
              </Box>
...
function extractCourseData(response: string): any | null {
  try {
    ...
    // Isolate the JSON object — strip any text the model may have added
    // before the opening brace or after the closing brace inside the block
    ...
    // Sanitize JSON to fix common issues with HTML content
    // This helps prevent JSON parsing errors from malformed strings
...
/**
 * Repairs common AI-generated JSON issues using a character-by-character state machine:
 * 1. Trailing commas before } or ]
 * 2. Literal newlines/tabs inside string values (not allowed in JSON)
 * 3. Unescaped double quotes inside string values (e.g. HTML attributes like class="lang")
 *
 * The heuristic for (3): a `"` inside a string is the *closing* quote if the next
 * non-whitespace character is a JSON structural char (: , } ]) or end-of-input.
 * Otherwise it is an inner unescaped quote and gets escaped.
 */
function sanitizeJsonString(jsonStr: string): string {
  // Pass 1: remove trailing commas — safe to do with regex
  const src = jsonStr.replace(/,(\s*[}\]])/g, '$1');

  // Pass 2: state-machine walk to fix string values
  const out: string[] = [];
  let i = 0;
  ...
  while (i < len) {
    ...
    while (i < len) {
      ...
      if (c === '\n' || c === '\r' || c === '\t') {
        // Literal control character inside a string — replace with space
        out.push(' ');
        i++;
        continue;
      }
      ...
      if (c === '"') {
        // Decide: closing quote or unescaped inner quote?
