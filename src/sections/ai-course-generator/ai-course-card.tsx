  ...
  return (
    ...
    >
      ...
      <Chip
        ...
        label={t(`ai-course-generation.status.${row.status}`)}
      ...
      >
        ...
        <Stack direction="row" spacing={1} sx={{ mb: 2, flexWrap: 'wrap', gap: 0.5 }}>
          {row.difficulty && (
            <Chip
              label={t(`ai-course-generation.difficulty.${row.difficulty}`)}
              size="small"
          ...
          {row.sections?.length > 0 && (
            <Chip
              ...
              label={`${row.sections.length} ${t('ai-course-generation.sections')}`}
              size="small"
              variant="outlined"
            />
          )}
          {row?.sectionsCount > 0 && (
            <Chip
              icon={<Iconify icon="solar:list-bold" width={14} />}
              label={`${row.sectionsCount} ${t('ai-course-generation.sections')}`}
              size="small"
              variant="outlined"
            />
          )}
        ...
        {row.aiProvider && (
          <Stack direction="row" alignItems="center" spacing={0.5} sx={{ mb: 2 }}>
            ...
            <Typography variant="caption" color="text.secondary">
              {t(`ai-course-generation.provider.${row.aiProvider}`)}
            </Typography>
          </Stack>
        )}
        ...
        >
          <Tooltip title={t('ai-course-generation.actions.view')}>
            <IconButton size="small" color="primary" onClick={onView}>
              <Iconify icon="solar:eye-bold" width={20} />
            </IconButton>
          </Tooltip>
          <Tooltip title={t('ai-course-generation.actions.edit')}>
            <IconButton size="small" color="info" onClick={onEdit}>
              <Iconify icon="solar:pen-bold" width={20} />
            </IconButton>
          </Tooltip>
          <Tooltip title={t('ai-course-generation.actions.delete')}>
