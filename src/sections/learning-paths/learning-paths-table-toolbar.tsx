...
export function LearningPathTableToolbar({ filters, onResetPage, onResetFilters }: Props) {
  ...
  const hasActiveFilters =
    currentFilters.order !== 'learningPath.name:asc' || !!currentFilters.positionId || !!currentFilters.isAIGenerated;
  ...
  const AI_GENERATED_OPTIONS = useMemo(
    () => [
      { value: 'true', label: t('learning-paths.table.filters.isAIGenerated.yes') },
      { value: 'false', label: t('learning-paths.table.filters.isAIGenerated.no') },
    ],
    [t]
  );

  const handleFilterIsAIGenerated = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      onResetPage();
      updateFilters({ isAIGenerated: event.target.value });
    },
    [onResetPage, updateFilters]
  );
    ...
    >
      <Box
        ...
        gridTemplateColumns={{
          ...
          md: hasActiveFilters ? '1fr 300px 300px 200px auto' : '1fr 300px 300px 200px',
        }}
        sx={{ width: 1 }}
      >
        <TextField
        ...
        <TextField
          fullWidth
          select
          value={currentFilters.isAIGenerated || ''}
          onChange={handleFilterIsAIGenerated}
          label={t('learning-paths.table.filters.isAIGenerated.label')}
          sx={{
            bgcolor: currentFilters.isAIGenerated ? 'action.selected' : 'transparent',
          }}
        >
          {AI_GENERATED_OPTIONS.map((option) => (
            <MenuItem key={option.value} value={option.value}>
              {option.label}
            </MenuItem>
          ))}
