...
type LearningPathPositionValue =
  | IPosition
  | {
      id: string | number;
      name: string;
...
const mapLearningPathPosition = (position?: LearningPathPositionValue | null): IPosition | null => {
  if (position === null || position === undefined) {
    return null;
  }

  if (typeof position === 'string' || typeof position === 'number') {
    const stringId = String(position).trim();
...
const getLearningPathPreloadedPositions = (learningPath?: ILearningPath | null): IPosition[] => {
  if (!learningPath) {
    return [];
  }

  const collectedPositions = [
    ...(learningPath.positions ?? []),
    ...(learningPath.jobPositions ?? []),
    ...(learningPath.position ? [learningPath.position] : []),
    ...(learningPath.job ? [{ id: learningPath.job.id, name: learningPath.job.name }] : []),
  ];

  const mappedPositions = collectedPositions
    .map((position) => mapLearningPathPosition(position))
    .filter((position): position is IPosition => Boolean(position));

  if (mappedPositions.length > 0) {
    return Array.from(new Map(mappedPositions.map((position) => [String(position.id), position])).values());
  }

  const fallbackIds = [
    ...(learningPath.positionIds ?? []),
    ...(learningPath.jobPositionIds ?? []),
    ...(learningPath.jobPositionId ? [learningPath.jobPositionId] : []),
  ];

  return Array.from(
    new Map(
      fallbackIds
        .map((positionId) => mapLearningPathPosition(positionId))
        .filter((position): position is IPosition => Boolean(position))
...
const getLearningPathPositionIds = (learningPath?: ILearningPath | null): string[] => {
  if (!learningPath) {
    return [];
  }
  ...
  const derivedIds = getLearningPathPreloadedPositions(learningPath)
    .map((position) => String(position.id).trim())
    .filter(Boolean);

  if (derivedIds.length > 0) {
    return Array.from(new Set(derivedIds));
  }

  const legacyId = String(learningPath.jobPositionId ?? '').trim();

  return legacyId ? [legacyId] : [];
};

const requiredAutocompleteValue = (message: string) =>
  z.preprocess(
...
export function LearningPathsCreateEditForm({ currentLearningPath }: Props) {
  ...
  const preloadedPositions = useMemo(
    () => getLearningPathPreloadedPositions(currentLearningPath),
    [currentLearningPath]
  );
  const availablePositions = useMemo(
    () =>
      Array.from(
        new Map(
          [...preloadedPositions, ...positions].map((position) => [String(position.id), position])
        ).values()
      ),
  ...
  const LearningPathsFormSchema = z.object({
    ...
    positionIds: z.array(z.union([z.string(), z.number()])).min(1, {
      message: t('learning-paths.form.fields.position.required'),
    }),
    modules: z.array(
      z.object({
        competencyId: requiredAutocompleteValue(t('learning-paths.form.fields.skill.required')),
        skillLevelId: requiredAutocompleteValue(t('learning-paths.form.fields.skillLevel.required')),
        order: z.number().min(1, { message: t('learning-paths.form.fields.moduleOrder.min') }),
        learningObjects: z.array(
          z.object({
            learningObjectId: requiredAutocompleteValue(
              t('learning-paths.form.fields.learningObject.required')
            ),
  ...
  const defaultValues: LearningPathsFormSchemaType = useMemo(() => {
    if (currentLearningPath?.modules && currentLearningPath.modules.length > 0) {
      return {
        ...
        positionIds: getLearningPathPositionIds(currentLearningPath),
  ...
  const loadSkills = useCallback(async (search: string = '') => {
    ...
    try {
      ...
      const competencieData = response?.data?.data?.data || [];
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const dataToSend = {
        ...
        isAIGenerated: currentLearningPath?.isAIGenerated ?? false,
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      <Grid container spacing={4} sx={{ mb: 6 }}>
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Stack spacing={3}>
            {/* Banner del Learning Path */}
            <Box>
              <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
                <Iconify icon="solar:gallery-add-bold" width={20} sx={{ color: 'secondary.main' }} />
              ...
              <ImageUploader
                ...
                placeholderText={t('learning-paths.form.fields.bannerUrl.placeholder', 'Sube el banner de la ruta de aprendizaje')}
                height={240}
                onUploadSuccess={(url) => {
            ...
            <Box>
              <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
                <Iconify icon="solar:pen-bold" width={20} sx={{ color: 'primary.main' }} />
                <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
                  {t('learning-paths.form.fields.name.label')}
                </Typography>
              </Stack>
              <Field.Text
                name="name"
            ...
            <Box>
              <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
                <Iconify icon="solar:file-text-bold" width={20} sx={{ color: 'text.secondary' }} />
                <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
                  {t('learning-paths.form.fields.description.label')}
                </Typography>
              </Stack>
              <Field.Text
                name="description"
              ...
              <Stack direction="row" alignItems="flex-start" spacing={1.5}>
                ...
                <Box sx={{ flexGrow: 1 }}>
                  ...
                  <Field.Autocomplete
                    ...
                    options={availablePositions.map((position) => String(position.id))}
                    ...
                    getOptionLabel={(option) => {
                      if (typeof option === 'string' || typeof option === 'number') {
                        const found = availablePositions.find((p) => String(p.id) === String(option));
          ...
          <Stack spacing={3}>
            ...
            >
              ...
              >
                ...
                {/* Info overlay - Mostrar info cuando hay contenido Y no está reproduciendo */}
                  ...
                  >
                    <Stack spacing={1}>
                      <Stack direction="row" alignItems="center" spacing={1}>
                        <Iconify icon="solar:play-circle-bold" width={20} sx={{ color: 'common.white' }} />
                        <Typography variant="caption" sx={{ color: 'common.white', fontWeight: 600 }}>
                          {totalModules} {t('learning-paths.details.modulesLabel')} • {totalCourses} {t('learning-paths.details.elementsTotal')}
                        </Typography>
                      </Stack>
                    </Stack>
                  </Box>
            ...
            {/* URL del Video debajo del preview */}
            <Box>
              <Stack direction="row" alignItems="center" spacing={1} sx={{ mb: 1.5 }}>
              ...
              <Field.Text
                name="videoUrl"
                placeholder="https://youtube.com/watch?v=..."
                helperText={t('learning-paths.form.fields.videoUrl.helperText', 'Soporta YouTube y Vimeo (se convierte automáticamente)')}
              />
            </Box>
          </Stack>
        </Grid>
    ...
    <Stack spacing={3}>
      ...
      >
        <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 700 }}>
          {t('learning-paths.form.fields.module.label')} {moduleIndex + 1} - {t('learning-paths.form.sections.configuration')}
        </Typography>
            ...
            >
              ...
              <Stack spacing={2}>
                <Stack direction="row" alignItems="center" spacing={1.5}>
                  ...
                  <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
                    {t('learning-paths.form.fields.learningObject.label')} {objectIndex + 1}
                  </Typography>
                </Stack>
