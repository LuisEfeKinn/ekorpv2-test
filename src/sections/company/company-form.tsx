...
export function CompanyForm({ currentCompany, onSuccess }: Props) {
  ...
  const CompanyFormSchema = z.object({
    ...
    webSite: z.string().url({ message: t('company.form.fields.webSite.required') }).min(1, { message: t('company.form.fields.webSite.required') }),
  ...
  const loadCoinsOptions = useCallback(async () => {
    try {
      ...
      toast.error(t(error?.message || 'company.messages.loadCoinsError'));
    } finally {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      toast.error(t(error?.message || 'company.messages.saveError'));
    }
  });
