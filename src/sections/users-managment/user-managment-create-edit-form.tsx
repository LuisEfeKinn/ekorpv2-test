...
export function UserManagementCreateEditForm({ currentUser }: Props) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const { countrySelect, regionSelect, skillId, positionId, organizationalUnitId, employmentTypeId, municipalityId, userId, ...formDataWithoutAutocompletes } = data;
      ...
      const formDataForService: IUserManagementFormData = {
        ...
        skillIds: skillId ? skillId.map(skill => parseInt(skill.id, 10)) : [],
        organizationalUnitId: organizationalUnitId ? parseInt(organizationalUnitId.id, 10) : 0,
        userId: userId ? parseInt(userId.id, 10) : 0,
      };
