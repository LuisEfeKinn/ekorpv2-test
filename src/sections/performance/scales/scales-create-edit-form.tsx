...
const createEmptyLevelFormData = (): LevelFormData => ({
  value: '',
  labels: [],
  description: '',
});

const normalizeLevelLabels = (labels: string[]): string[] =>
  Array.from(
    new Set(
      labels
...
const parseLevelLabels = (label: string): string[] =>
  normalizeLevelLabels(label.split(','));
...
export function ScalesCreateEditForm({ currentScale }: Props) {
  ...
  const [levelFormData, setLevelFormData] = useState<LevelFormData>(createEmptyLevelFormData);
  const [levelLabelInputValue, setLevelLabelInputValue] = useState('');
  ...
  const handleAddLevel = () => {
    const normalizedLabels = normalizeLevelLabels([
      ...levelFormData.labels,
      levelLabelInputValue,
    ]);

    if (!levelFormData.value || normalizedLabels.length === 0 || !levelFormData.description) {
    ...
    setLevelFormData(createEmptyLevelFormData());
    setLevelLabelInputValue('');
  ...
  const handleOpenEditDialog = (index: number) => {
    ...
    setLevelFormData({
      ...
      labels: parseLevelLabels(level.label),
      description: level.description,
    });
    setLevelLabelInputValue('');
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      <Stack direction="row" flexWrap="wrap" sx={{ gap: 0.75 }}>
                        {parseLevelLabels(level.label).map((label) => (
                          <Chip key={`${level.value}-${label}`} label={label} size="small" variant="soft" color="info" />
                        ))}
                      </Stack>
                    </TableCell>
      ...
      <DialogContent>
        <Stack spacing={3} sx={{ pt: 2 }}>
          ...
          <Autocomplete<string, true, false, true>
            ...
            options={[]}
            value={levelFormData.labels}
            inputValue={levelLabelInputValue}
            onInputChange={(_, newInputValue, reason) => {
              setLevelLabelInputValue(reason === 'reset' ? '' : newInputValue);
            }}
            onChange={(_, newValue) => {
              setLevelFormData((prev) => ({
                ...prev,
                labels: normalizeLevelLabels(newValue.map((label) => String(label))),
              }));
            }}
            renderInput={(params) => (
              <TextField
                ...
                placeholder={t('scales.form.fields.levels.label.placeholder')}
              />
            )}
            slotProps={{
              chip: {
