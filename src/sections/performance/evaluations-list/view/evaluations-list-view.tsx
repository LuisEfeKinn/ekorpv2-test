...
export function EvaluationsListView() {
  ...
  const canReset =
    ...
    (filters.state.organizationalUnitIds?.length ?? 0) > 0 ||
  ...
  const TABLE_HEAD: TableHeadCellProps[] = [
    { id: '', label: '' },
    { id: 'employeeName', label: t('evaluations-list.table.columns.employeeName') },
    { id: 'campaignName', label: t('evaluations-list.table.columns.campaignName') },
    { id: 'jobPosition', label: t('evaluations-list.table.columns.jobPosition') },
    { id: 'vigencyName', label: t('evaluations-list.table.columns.vigency') },
    { id: 'status', label: t('evaluations-list.table.columns.status') },
    {
      id: 'dueDate',
      label: t('evaluations-list.table.columns.dueDate'),
      align: 'center',
    },
  ];
            ...
            <TableBody>
              {dataInPage?.map((row) => (
                <EvaluationsListTableRow
                  key={row.id}
                  row={row}
                  onViewParticipants={() => handleViewParticipants(row.participantId.toString())}
                />
              ))}

              <TableEmptyRows
                height={table.dense ? 56 : 76}
                emptyRows={emptyRows(table.page, table.rowsPerPage, dataFiltered?.length ?? 0)}
              />
...
function applyFilter({ inputData, comparator, filters }: ApplyFilterProps) {
  ...
  // Validar que inputData sea un array válido
  ...
  if (name) {
    inputData = inputData.filter(
      (evaluation) =>
        evaluation.employeeName.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        evaluation.campaignName.toLowerCase().indexOf(name.toLowerCase()) !== -1 ||
        evaluation.jobPosition.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
