...
export function ConfigureEvaluationsTableToolbar({ filters, onResetPage }: Props) {
  ...
  const getSelectedOptions = useCallback(
    <T extends { id: string; name: string }>(idsCsv: string, namesCsv: string | undefined, options: T[]): T[] => {
      const ids = idsCsv.split(',').filter(Boolean);
      const names = (namesCsv || '').split(',').map((name) => name.trim());

      return ids.map((id, index) => {
        const existing = options.find((option) => String(option.id) === String(id));
  ...
  const loadPositions = useCallback(async (searchTerm: string) => {
    ...
    try {
      ...
      setPositionOptions(
        response.data?.data?.map((pos: any) => ({ id: String(pos.id), name: pos.name })) || []
      );
    } catch (error) {
    ...
    try {
      ...
      setEmployeeOptions(
        response.data?.data?.map((emp: any) => ({
          id: String(emp.id),
          name: `${emp.firstName || ''} ${emp.firstLastName || ''}`.trim(),
        })) || []
      );
    } catch (error) {
  ...
  const renderCountTag = (tagValue: { id: string; name: string }[]) => (
    <Box
      component="span"
      sx={{
        display: 'inline-flex',
      ...
      >
        ...
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            ...
            filters.setState({
              departmentIds: newValue.map((v) => v.id).join(','),
              departmentNames: newValue.map((v) => v.name).join(', '),
            });
            onResetPage();
          }}
          onOpen={() => {
            if (departmentOptions.length === 0) {
              loadDepartments(departmentInputValue);
            }
          }}
        ...
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            ...
            filters.setState({
              positionIds: newValue.map((v) => v.id).join(','),
              positionNames: newValue.map((v) => v.name).join(', '),
            });
            onResetPage();
          }}
          onOpen={() => {
            if (positionOptions.length === 0) {
              loadPositions(positionInputValue);
            }
          }}
          onInputChange={(_, newInputValue, reason) => {
            if (reason === 'input') {
              setPositionInputValue(newInputValue);
        ...
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            ...
            filters.setState({
              employeeIds: newValue.map((v) => v.id).join(','),
              employeeNames: newValue.map((v) => v.name).join(', '),
            });
            onResetPage();
          }}
          onOpen={() => {
            if (employeeOptions.length === 0) {
              loadEmployees(employeeInputValue);
            }
          }}
          onInputChange={(_, newInputValue, reason) => {
            if (reason === 'input') {
