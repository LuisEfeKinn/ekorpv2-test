import type { IObjective } from 'src/types/performance';
...
export function AddObjectiveModal({ open, onClose, onAdd }: Props) {
  ...
  const handleAdd = () => {
    if (selectedObjective && weight > 0) {
      onAdd(selectedObjective.id, selectedObjective.name, weight, targetValue, customKpi);
      handleClose();
    }
  };
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>{t('configure-evaluations.dialogs.addObjective.title')}</DialogTitle>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, pt: 1 }}>
          <Autocomplete
            ...
            renderInput={(params) => (
              <TextField
                {...params}
                label={t('configure-evaluations.form.fields.objectives.select')}
                placeholder={t('configure-evaluations.form.fields.objectives.label')}
              />
            )}
          />
          ...
          <TextField
            fullWidth
            type="number"
            label={t('configure-evaluations.form.fields.objectives.weight')}
            value={weight}
          ...
          <TextField
            fullWidth
            type="number"
            label={t('configure-evaluations.form.fields.objectives.targetValue')}
            value={targetValue}
          ...
          <TextField
            fullWidth
            label={t('configure-evaluations.form.fields.objectives.customKpi')}
            value={customKpi}
      ...
      <DialogActions>
        <Button onClick={handleClose} color="inherit">
          {t('configure-evaluations.dialogs.addObjective.cancel')}
        </Button>
        <Button onClick={handleAdd} variant="contained" disabled={!isValid}>
          {t('configure-evaluations.dialogs.addObjective.add')}
        </Button>
      </DialogActions>
    </Dialog>
  );
