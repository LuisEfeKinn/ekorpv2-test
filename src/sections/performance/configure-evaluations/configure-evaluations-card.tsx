...
export function ConfigureEvaluationsCard({ row, onEditRow, onLaunchRow, onDeleteRow, onCancelRow, onViewParticipants, onViewAnalysis }: Props) {
    ...
    <>
      ...
      >
        <MenuList>
          ...
          {/* Ver análisis — solo ACTIVE y COMPLETED */}
          {(row.status === 'ACTIVE' || row.status === 'COMPLETED') && (
            ...
            >
              ...
              {t('configure-evaluations.actions.viewAnalysis')}
            </MenuItem>
          )}

          {/* Ver participantes — solo cuando la campaña está activa, completada o en borrador */}
          {(row.status === 'ACTIVE' || row.status === 'COMPLETED' || row.status === 'DRAFT') && (
            <MenuItem
              onClick={() => {
                onViewParticipants();
                popover.onClose();
