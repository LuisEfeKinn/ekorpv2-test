...
function ParticipationBarChart({ data }: { data: IRelationshipItem[] }) {
  ...
  const labels = data.map((d) => RELATIONSHIP_LABELS[d.relationship] ?? d.relationship);
  ...
  return (
    <Chart
      type="bar"
      series={[
        { name: 'Asignados', data: data.map((d) => d.totalAssigned) },
        { name: 'Respondidos', data: data.map((d) => d.totalResponded) },
      ]}
...
function RadarChart({ data }: { data: IRadarCompetency[] }) {
  ...
  return (
    <Box sx={{ px: 1, pb: 1 }}>
      <Chart
        type="radar"
        series={[{ name: 'Cumplimiento', data: scores }]}
        options={chartOptions}
        sx={{ height: 420 }}
      />
    </Box>
        ...
        <Table size={dense ? 'small' : 'medium'}>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ minWidth: 200 }}>Empleado</TableCell>
              <TableCell sx={{ minWidth: 160 }}>Cargo</TableCell>
              <TableCell sx={{ minWidth: 180 }}>Campaña</TableCell>
              <TableCell sx={{ minWidth: 120 }}>Fecha Límite</TableCell>
              <TableCell align="center" sx={{ minWidth: 110 }}>Estado</TableCell>
...
export function CampaignAnalysisView({ campaignId }: Props) {
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading="Análisis de Campaña"
        links={[
          { name: 'Dashboard', href: paths.dashboard.root },
          { name: 'Campañas', href: paths.dashboard.performance.configureEvaluations },
          { name: 'Análisis' },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid size={{ xs: 12, md: 7 }}>
          <Card sx={{ height: '100%' }}>
            <CardHeader
              ...
              subheader="Nivel esperado vs. puntaje obtenido por competencia"
            />
        ...
        <Grid size={{ xs: 12, md: 5 }}>
          <Card sx={{ height: '100%' }}>
            <CardHeader title="Competencias" subheader="Porcentaje de cumplimiento" />
            {loadingCharts ? (
              <LoadingBox />
            ) : (
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid size={{ xs: 12, md: 5 }}>
          <Card sx={{ height: '100%' }}>
            <CardHeader
              title="Distribución por Tipo de Evaluador"
              subheader="Respuestas por relación evaluador en esta campaña"
            />
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Card sx={{ height: '100%' }}>
            <CardHeader
              title="Participación por Tipo de Evaluador"
              subheader="Asignados vs respondidos por relación"
            />
      ...
      <Card>
        <CardHeader
          title="Participantes"
          subheader="Lista de empleados con sus resultados"
          sx={{ pb: 1 }}
        />
        <Divider />
