...
import { ParticipantsDrawer } from '../drawer/participants-drawer';
...
export function ConfigureEvaluationsView() {
  ...
  const participantsDrawerOpen = useBoolean();
  const [editingEvaluation, setEditingEvaluation] = useState<IConfigureEvaluation | undefined>(
    undefined
  );
  const [viewingParticipantsEvaluation, setViewingParticipantsEvaluation] = useState<
    IConfigureEvaluation | undefined
  >(undefined);
  ...
  const handleViewParticipants = useCallback(
    (id: string) => {
      const evaluation = tableData.find((e) => e.id === id);
      setViewingParticipantsEvaluation(evaluation);
      participantsDrawerOpen.onTrue();
    },
    [tableData, participantsDrawerOpen]
  );

  const handleViewAnalysis = useCallback(
    (id: string) => {
      router.push(paths.dashboard.performance.campaignAnalysis(id));
    },
    [router]
  );
  ...
  return (
          ...
          {tableData.map((row) => (
            <Grid key={row.id} size={{ xs: 12, sm: 6 }}>
              <ConfigureEvaluationsCard
                ...
                onViewAnalysis={() => handleViewAnalysis(row.id)}
              />
            </Grid>
          ))}
        </Grid>
    ...
    <ParticipantsDrawer
      open={participantsDrawerOpen.value}
      onClose={participantsDrawerOpen.onFalse}
      campaignId={viewingParticipantsEvaluation?.id ?? ''}
      campaignName={viewingParticipantsEvaluation?.name ?? ''}
    />
    </>
  );
}
