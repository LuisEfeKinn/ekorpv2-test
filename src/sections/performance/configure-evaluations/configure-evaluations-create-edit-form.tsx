...
export function ConfigureEvaluationsCreateEditForm({ currentEvaluation }: Props) {
  ...
  const campaignCompetences = (currentEvaluation as any)?.campaignCompetences || [];
  ...
  const defaultValues = {
    ...
    isActive: (currentEvaluation as any)?.isActive ?? true,
  ...
  const evaluationType = watch('type');
  ...
  useEffect(() => {
    if (currentEvaluation) {
      ...
      // Precargar vigencia si existe
      if (evaluation.vigency) {
        setSelectedVigency({
          id: evaluation.vigency.id,
          name: evaluation.vigency.name,
        });
      } else if (evaluation.vigencyId) {
        // Si solo tenemos el ID, lo cargamos
        loadVigencies().then(() => {
          const vigency = vigencies.find((v) => v.id === evaluation.vigencyId);
          if (vigency) {
            setSelectedVigency(vigency);
          }
        });
      }
  ...
  }, [currentEvaluation, departments, positions, employees, templates, vigencies, loadTemplates, loadVigencies]);
  ...
  const handleConfigureQuestions = (campaignId: string | number, competenceId: string | number) => {
    router.push(paths.dashboard.performance.configureQuestions(String(campaignId), String(competenceId)));
  };
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3 }}>
        <Box
          sx={{
            ...
            justifyContent: 'center',
            borderRadius: 1.5,
            background: `linear-gradient(135deg, ${alpha(theme.palette.warning.main, 0.16)} 0%, ${alpha(theme.palette.warning.dark, 0.24)} 100%)`,
          }}
        >
          <Iconify icon="solar:file-text-bold" width={28} sx={{ color: 'warning.main' }} />
        </Box>
        <Box>
          <Typography variant="h6">
            {t('configure-evaluations.form.sections.competences')}
          </Typography>
          <Typography variant="body2" sx={{ color: 'text.secondary' }}>
            {t('configure-evaluations.form.sections.competencesSubtitle')}
          </Typography>
        </Box>
      </Box>
      ...
      {campaignCompetences.length === 0 ? (
        ...
        >
          <Typography variant="body1" color="text.secondary">
            {t('configure-evaluations.form.fields.competences.emptyMessage')}
          </Typography>
        </Box>
      ) : (
        <TableContainer sx={{ borderRadius: 2, border: `1px solid ${alpha(theme.palette.grey[500], 0.16)}` }}>
          <Table>
            <TableBody>
              {campaignCompetences.map((item: any) => (
                <TableRow
                  key={item.id}
                  sx={{
                ...
                >
                  <TableCell>
                    <Stack spacing={0.5}>
                      <Typography variant="subtitle2">{item?.competence?.name || '-'}</Typography>
                      <Typography variant="caption" color="text.secondary">
                  ...
                  <TableCell align="right" width={180}>
                    <Chip
                      label={`${Number(item?.weight || 0).toFixed(1)}%`}
                      size="small"
                      color="warning"
                      variant="soft"
                    />
                  ...
                  <TableCell align="right" width={220}>
                    <Button
                      size="small"
                      variant="contained"
                      onClick={() => handleConfigureQuestions(item?.campaignId, item?.competenceId)}
                    >
                      {t('configure-evaluations.form.fields.competences.configureQuestionsButton')}
                    </Button>
                  </TableCell>
                </TableRow>
              ))}
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, md: 6 }}>
          ...
          >
            <Field.Switch
              name="isActive"
              label={`${t('configure-evaluations.form.fields.isActive.label')} (${isActive ? t('configure-evaluations.form.fields.isActive.active') : t('configure-evaluations.form.fields.isActive.inactive')})`}
              helperText={t('configure-evaluations.form.fields.isActive.helperText')}
            />
          </Box>
        </Grid>
    ...
    <>
      <Form methods={methods} onSubmit={onSubmit}>
        <Stack spacing={3}>
          ...
          {renderDates()}
          {renderScope()}
          {currentEvaluation && renderCompetences()}
          {evaluationType !== 'CLIMA_LABORAL' && renderEvaluatorConfigs()}
          {renderConfig()}
          {renderActions()}
        </Stack>
      </Form>
