...
// Competency option with color from skills data
    ...
    >
      ...
      <Tooltip title="Arrastrar para ordenar">
        <IconButton
          ...
          size="small"
          disableRipple
          sx={{ cursor: 'grab', flexShrink: 0, color: 'text.disabled', '&:hover': { color: 'text.primary' } }}
        >
      ...
      {/* Delete — en xs sube a fila del handle (order:3, ml:auto); en sm+ va al final (order:5) */}
      <Tooltip title="Eliminar pregunta">
        <IconButton
          size="small"
          color="error"
          onClick={() => onRemove(question._id)}
          sx={{ flexShrink: 0, order: { xs: 3, sm: 5 }, ml: { xs: 'auto', sm: 0 } }}
        >
          <Iconify icon="solar:trash-bin-trash-bold" width={14} />
        </IconButton>
      </Tooltip>

      {/* Campos — flexBasis:100% en xs fuerza nueva fila; flexBasis:0 + flexGrow:1 en sm+ */}
      <Box
        sx={{
          order: 4,
          flexGrow: 1,
      ...
      >
        <TextField
          ...
          sx={{ '& .MuiInputBase-inputMultiline': { minHeight: { xs: '80px', sm: 'auto' } } }}
        />
...
const CompetenceCard = memo(function CompetenceCard({
  ...
  // Use String() to avoid type mismatch between number and string IDs
  const skillOptions = skills.some((s) => String(s.id) === String(skillValue.id))
    ? skills
    : [skillValue, ...skills];
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Inline skill selector + weight — full width on xs */}
        <Box sx={{ display: 'flex', flex: 1, alignItems: 'center', gap: 1, order: { xs: 3, sm: 2 }, minWidth: { xs: '100%', sm: 0 } }}>
          <Autocomplete
        ...
        {/* Expand / collapse + Delete — stay in row 1 on xs */}
        <Box sx={{ display: 'flex', alignItems: 'center', order: { xs: 2, sm: 3 }, ml: { xs: 'auto', sm: 0 } }}>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Fields group: objective + weight + target + kpi — full width on xs */}
        <Box sx={{ display: 'flex', flex: 1, flexWrap: 'wrap', alignItems: 'center', gap: 1, order: { xs: 3, sm: 2 }, minWidth: { xs: '100%', sm: 0 } }}>
        ...
        {/* Delete — stays in row 1 on xs */}
        <Tooltip title="Eliminar objetivo">
          <IconButton size="small" color="error" onClick={() => onRemove(obj._id)} sx={{ order: { xs: 2, sm: 3 }, ml: { xs: 'auto', sm: 0 } }}>
            <Iconify icon="solar:trash-bin-trash-bold" width={16} />
          </IconButton>
        </Tooltip>
      </Box>
...
export function ConfigureTestsCreateEditForm({ currentTest }: Props) {
  ...
  const loadSkills = useCallback(async (search: string = '') => {
    ...
    try {
      const response = await GetSkillsPaginationService({ perPage: 100, search: search || undefined });
      if (response?.data?.data?.data) {
        setSkills(
          response.data.data.data.map((s) => ({
            id: Number(s.id),
            name: s.name,
            color: s.color || '',
          }))
