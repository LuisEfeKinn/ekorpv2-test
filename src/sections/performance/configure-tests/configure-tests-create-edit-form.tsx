...
import type { ITestQuestion, ITestObjective, IConfigureTest, ITestCompetence } from 'src/types/performance';
...
export function ConfigureTestsCreateEditForm({ currentTest }: Props) {
  ...
  const handleOpenQuestionsModal = (index: number) => {
    setSelectedCompetenceIndex(index);
    setOpenQuestionsModal(true);
  };

  const handleSaveCompetenceQuestions = (questions: ITestQuestion[]) => {
    if (selectedCompetenceIndex === null) return;

    setCompetences((prev) =>
      prev.map((competence, index) =>
        index === selectedCompetenceIndex
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const normalizeQuestionWeight = (weightValue: unknown) => {
        const parsedWeight = Number(weightValue ?? 0);
        if (!Number.isFinite(parsedWeight)) return 0;

        return parsedWeight > 1 ? parsedWeight / 100 : parsedWeight;
      };
      ...
      const payload = {
        ...
        competences: competences.map((comp) => ({
          ...
          questions: (comp.questions || []).map((question: any, index: number) => ({
            description: question?.description || '',
            scaleId: Number(question?.scaleId),
            weight: normalizeQuestionWeight(question?.weight),
            isOptional: Boolean(question?.isOptional),
            order: Number.isInteger(Number(question?.order)) ? Number(question.order) : index + 1,
      ...
      toast.error(
        testId
          ? t(error?.message || 'configure-tests.messages.error.update')
          : t(error?.message || 'configure-tests.messages.error.create')
      );
    }
  });
  ...
  return (
    <>
              ...
              >
                ...
                <Box>
                  <Typography variant="subtitle2" sx={{ mb: 1.5 }}>
                    {t('configure-tests.form.fields.coverImage')}
                  </Typography>
                  <ImageUploader
                    imageUrl={watch('coverImage')}
                    placeholderText={t(
                      'configure-tests.form.placeholders.coverImage',
                        ...
                        >
                          <Stack spacing={0.5}>
                            <Typography variant="subtitle2">
                              {comp?.competency?.name || comp?.competenceName || `${t('configure-tests.form.fields.competence')} #${comp.competencyId}`}
                            </Typography>
                          ...
                          <Stack direction="row" spacing={1} alignItems="center">
                            <Button
                              ...
                              size="small"
                              variant="outlined"
                              onClick={() => handleOpenQuestionsModal(index)}
                              aria-label={`${t('configure-tests.actions.questions')} (${comp.questions?.length || 0})`}
                            >
                              {t('configure-tests.actions.questions')} ({comp.questions?.length || 0})
                            </Button>
                            <IconButton
                              color="error"
      ...
      <AddQuestionsModal
        ...
        onSaved={handleSaveCompetenceQuestions}
        initialQuestions={
          selectedCompetenceIndex !== null
            ? competences[selectedCompetenceIndex]?.questions || []
            : []
        }
      />
    </>
