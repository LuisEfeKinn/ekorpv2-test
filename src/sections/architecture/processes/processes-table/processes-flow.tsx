...
type ProcessesFlowProps = {
  ...
  onEditProcess?: (id: number) => void;
  onNodeDoubleClick?: (node: ProcessFlowNode) => void;
  onOpenNodeMap?: (id: number, label: string) => void;
  reloadKey?: number;
  onBack?: () => void;
  parentLabel?: string;
};
      ...
      <Stack spacing={1.5}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
          ...
          <Stack direction="row" spacing={0.75} alignItems="center">
            <Tooltip title={t('process.table.actions.viewNodeMap', { defaultValue: 'Ver mapa de nodos' })}>
              <span>
                <IconButton
                  size="small"
                  onClick={(event) => {
                ...
                >
                  <Iconify icon="solar:point-on-map-perspective-bold" width={18} />
                </IconButton>
              </span>
            </Tooltip>
            ...
            <Tooltip title={isExpanded ? t('data.diagram.tooltips.collapse') : t('data.diagram.tooltips.expand')}>
              <span>
                ...
                >
                  <Iconify icon={(isExpanded ? 'eva:chevron-up-fill' : 'eva:chevron-down-fill') as any} width={20} />
                </IconButton>
              </span>
            </Tooltip>
          </Stack>
...
export function ProcessesFlow({
  ...
  const generateNodesAndEdges = useCallback(() => {
    ...
    const processNode = (
      ...
      newNodes.push({
        ...
        data: {
          ...
          onOpenMap: onOpenNodeMap
            ? () => onOpenNodeMap(node.id, String(node.data.name || node.label || node.id))
            : undefined,
          onDoubleClick: () => {
    ...
    setEdges(newEdges);
  }, [rootProcesses, expandedNodes, colors, handleExpandNode, handleEditNode, applyDagreLayout, setNodes, setEdges, onNodeDoubleClick, onOpenNodeMap]);
  ...
  const handleResetView = useCallback(() => {
    // Si hay callback externo (onBack), lo usamos para volver al nivel anterior
    if (onBack) {
      onBack();
      return;
    }
    
    // Si no, reseteamos la vista normal
  ...
  return (
    ...
    >
      <Box sx={{ height: '100%', width: '100%', bgcolor: 'background.neutral' }}>
        {loading && (
          <Box
            sx={{
              position: 'absolute',
          ...
          >
            <CircularProgress color="primary" thickness={3} size={48} />
          </Box>
        )}
