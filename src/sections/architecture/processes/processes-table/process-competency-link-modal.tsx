...
import { SaveProcessCompetencyService, UpdateProcessCompetencyService, DeleteProcessCompetencyService } from 'src/services/architecture/process/processRelations.service';
...
export function ProcessCompetencyLinkModal({ open, onClose, onSuccess, processId, existingItemIds, relationId, initialData, allowDelete }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const currentSelectedId = initialData?.competencyId ?? initialData?.competency?.id ?? null;
        ...
        const response = await GetCompetenciesPaginationService({ perPage: 1000 });
    ...
    if (open) {
      setSubmitted(false);
      setForm({
        itemId: initialData?.competencyId ?? initialData?.competency?.id ?? null,
      });
      load();
    }
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      competencyId: Number(form.itemId)
    };
    try {
      setLoading(true);
      if (relationId) {
        await UpdateProcessCompetencyService(relationId, payload);
      } else {
        await SaveProcessCompetencyService(payload);
      }
    ...
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 420 } }}>
      <Box sx={{ px: 2, py: 2 }}>
        <Typography variant="h6" sx={{ mb: 1 }}>
          {t('process.map.modals.competency.title')}
        </Typography>
        <Stack spacing={2} sx={{ pt: 1 }}>
          <Autocomplete
            ...
            renderInput={(params: any) => (
              <TextField
                {...params}
                label={t('process.map.modals.competency.fieldLabel')}
