...
import { GetIndicatorsListService, SaveProcessIndicatorService, UpdateProcessIndicatorService, DeleteProcessIndicatorService } from 'src/services/architecture/process/processRelations.service';
...
export function ProcessIndicatorLinkModal({ open, onClose, onSuccess, processId, relationId, initialData, allowDelete }: Props) {
  ...
  useEffect(() => {
    ...
    if (open) {
      setForm({
        itemId: initialData?.indicatorId ?? initialData?.indicator?.id ?? null,
      ...
      <Box sx={{ px: 2, py: 2 }}>
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
          <Autocomplete
            ...
            renderInput={(params: any) => <TextField {...params} label={t('process.map.modals.indicator.fieldLabel')} />}
          />
          <TextField 
