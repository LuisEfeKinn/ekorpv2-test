...
import { GetProcessMapByIdService } from 'src/services/architecture/process/processTable.service';

import { toast } from 'src/components/snackbar';
import { Iconify } from 'src/components/iconify';

import { ProcessTableExpandedDiagram } from './processes-table-expanded-diagram';
  ...
  return (
    ...
    >
      ...
      <Stack spacing={1.5} alignItems="center">
        <Chip label={String(data.label || '')} size="small" sx={{ bgcolor: alpha(theme.palette.common.white, 0.25), color: 'common.white', fontWeight: 700, fontSize: '0.75rem', height: 24, backdropFilter: 'blur(10px)' }} />
        ...
        <Typography variant="caption" sx={{ color: alpha(theme.palette.common.white, 0.85), fontWeight: 600, fontSize: '0.7rem' }}>
          {t('process.map.diagram.systemTitle')}
        </Typography>
      </Stack>
    </Paper>
  );
  ...
  return (
    <Paper
      ...
      sx={{
        ...
        '&:hover': { transform: 'translateY(-4px) scale(1.03)', boxShadow: `0 8px 32px ${alpha(color, 0.35)}`, borderColor: color },
...
export function ProcessTableDiagram({ processId, sx }: ProcessTableDiagramProps) {
  const { t } = useTranslate('architecture');
  const theme = useTheme();
  ...
  const generateNodesAndEdges = useCallback((data: MapData) => {
    ...
    const childNodes: Node[] = (data.children || []).map((child: ChildNode, index: number) => {
      ...
      return {
        ...
        type: 'child',
        position: { x, y },
        data: { label: child.label, id: String(child.id), color, onClick: () => setExpandedNode({ id: String(child.id), label: child.label }) },
        draggable: true,
      };
    });
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        setLoading(true);
        const response = await GetProcessMapByIdService(processId);
        const data = response.data;
        setMapData(data);
        generateNodesAndEdges(data);
      } catch (error) {
        console.error('Error al cargar el mapa de procesos:', error);
        toast.error(t('process.map.diagram.errorLoading'));
      } finally {
        setLoading(false);
      }
    };
    if (processId) fetchMapData();
  }, [processId, generateNodesAndEdges, t]);

  if (expandedNode) {
    return (
      <ProcessTableExpandedDiagram
        processId={processId}
        nodeId={expandedNode.id}
        nodeLabel={expandedNode.label}
        parentLabel={mapData?.label || ''}
        onBack={() => setExpandedNode(null)}
        onNavigateToChild={(child: ChildNode) => setExpandedNode({ id: String(child.id), label: child.label })}
        sx={sx}
      />
    );
  }
    ...
    return (
      <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 600 }}>
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">{t('process.map.diagram.loading')}</Typography>
        </Stack>
      </Box>
    );
  }
  ...
  if (!mapData) {
    return (
      <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 600 }}>
        <Typography variant="body1" color="text.secondary">{t('process.map.diagram.noData')}</Typography>
      </Box>
    );
  }
        ...
        <Box sx={{ width: '100%', height: 600, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Stack alignItems="center" spacing={3}>
            ...
            <Stack spacing={1} alignItems="center">
              ...
              <Typography variant="body2" color="text.secondary">{t('process.map.diagram.noRelations')}</Typography>
            </Stack>
          </Stack>
        </Box>
      </Card>
          ...
          <Paper elevation={2} sx={{ px: { xs: 1.5, sm: 2 }, py: { xs: 1, sm: 1.25 }, borderRadius: 1.5, bgcolor: alpha(theme.palette.background.paper, 0.9), backdropFilter: 'blur(8px)', border: `1px solid ${alpha(theme.palette.divider, 0.12)}` }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>
                {(mapData.children || []).length} {t('process.map.diagram.connected')}
              </Typography>
            </Stack>
          </Paper>
        </Box>
          ...
          <Paper elevation={2} sx={{ px: { xs: 1.5, sm: 2 }, py: { xs: 1, sm: 1.25 }, borderRadius: 1.5, bgcolor: alpha(theme.palette.background.paper, 0.9), backdropFilter: 'blur(8px)', border: `1px solid ${alpha(theme.palette.divider, 0.12)}` }}>
            <Stack spacing={0.75}>
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>{t('process.map.diagram.zoom')}</Typography>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
                <Box sx={{ width: 16, height: 16, borderRadius: '50%', bgcolor: alpha(theme.palette.text.secondary, 0.08), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Iconify icon="eva:move-fill" width={10} sx={{ color: 'text.secondary' }} />
                </Box>
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>{t('process.map.diagram.move')}</Typography>
              </Stack>
            </Stack>
          </Paper>
        </Box>
