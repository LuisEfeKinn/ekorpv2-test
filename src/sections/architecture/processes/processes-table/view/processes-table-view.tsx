...
export function ProcessTableView() {
  ...
  const handleDownloadExcel = useCallback(async () => {
    try {
      setDownloading(true);
      const response = await DownloadProcessExcelService();
    ...
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          action={
            <Stack direction="row" spacing={1}>
              ...
              <Button
                onClick={openCreateDrawer.onTrue}
                variant="contained"
        ...
        <Drawer
          ...
          onClose={openCreateDrawer.onFalse}
          anchor="right"
          slotProps={{ backdrop: { invisible: true } }}
          PaperProps={{ sx: { width: { xs: 1, md: 480 } } }}
        >
          <ProcessCreateEditForm onClose={openCreateDrawer.onFalse} />
        </Drawer>
  ...
  if (name) {
    inputData = inputData.filter(
      (item) => 
        ...
        item?.nomenclature?.toLowerCase().indexOf(name.toLowerCase()) !== -1
    );
  }
