...
export function ProcessesFlowView({ sx }: ProcessesFlowViewProps) {
  ...
  const [mapProcessId, setMapProcessId] = useState<string | null>(null);
  const [mapProcessLabel, setMapProcessLabel] = useState<string>('');
  ...
  return (
    <>
      <Container maxWidth="xl">
        ...
        {visibleData.length > 0 ? (
            <ProcessesFlow
                ...
                onOpenNodeMap={(id, label) => {
                  setMapProcessId(String(id));
                  setMapProcessLabel(label);
                  mapDrawer.onTrue();
                }}
      ...
      <Drawer
        open={mapDrawer.value}
        onClose={() => {
          mapDrawer.onFalse();
          setMapProcessId(null);
          setMapProcessLabel('');
        }}
        anchor="right"
        PaperProps={{
          sx: {
            width: { xs: '100%', sm: '92vw', md: '84vw', lg: '76vw' },
            maxWidth: 1200,
            display: 'flex',
      ...
      >
        ...
        >
          <Typography variant="h6">
            {mapProcessLabel ? `${t('process.map.title')} - ${mapProcessLabel}` : t('process.map.title')}
          </Typography>
          <IconButton
            aria-label={t('process.table.actions.cancel', { defaultValue: 'Cerrar' })}
            onClick={() => {
              mapDrawer.onFalse();
              setMapProcessId(null);
              setMapProcessLabel('');
        ...
        <Box sx={{ p: 2, flex: '1 1 auto', overflow: 'hidden' }}>
          {mapProcessId ? (
            <Box sx={{ height: 1, minHeight: 520 }}>
              <ProcessTableDiagram processId={mapProcessId} />
            </Box>
          ) : (
            <EmptyContent title={t('process.table.table.emptyState.noData')} />
          )}
