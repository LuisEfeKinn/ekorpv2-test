...
import { SaveProcessToolService, UpdateProcessToolService, DeleteProcessToolService } from 'src/services/architecture/process/processRelations.service';
...
export function ProcessToolLinkModal({ open, onClose, onSuccess, processId, existingItemIds, relationId, initialData, allowDelete }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const currentSelectedId = initialData?.toolId ?? initialData?.tool?.id ?? null;
        ...
        const response = await GetToolsTablePaginationService({ perPage: 1000 });
        // Process Tools
    ...
    if (open) {
      setSubmitted(false);
      setForm({
        itemId: initialData?.toolId ?? initialData?.tool?.id ?? null,
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      toolId: Number(form.itemId)
    };
    try {
      setLoading(true);
      if (relationId) {
        await UpdateProcessToolService(relationId, payload);
      } else {
        await SaveProcessToolService(payload);
      }
    ...
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 420 } }}>
      <Box sx={{ px: 2, py: 2 }}>
        <Typography variant="h6" sx={{ mb: 1 }}>
          {t('process.map.modals.tool.title')}
        </Typography>
        <Stack spacing={2} sx={{ pt: 1 }}>
          <Autocomplete
            ...
            renderInput={(params: any) => (
              <TextField
                {...params}
                label={t('process.map.modals.tool.fieldLabel')}
