...
export function ProcessJobLinkModal({ open, onClose, onSuccess, processId, relationId, initialData, allowDelete }: Props) {
    ...
    if (open) {
      setForm({
        jobId:
          ...
          initialData?.job_id ??
          null,
        isMain: Boolean(initialData?.isMain ?? initialData?.main ?? false),
        description: initialData?.description ?? initialData?.observations ?? '',
        actionTypeId:
          initialData?.actionType?.id ??
          initialData?.actionTypeId ??
          null,
      });
      load();
    }
