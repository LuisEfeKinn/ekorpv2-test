...
import { Box, Stack, Drawer, Button, MenuItem, TextField, Typography, Autocomplete, CircularProgress } from '@mui/material';

import { useTranslate } from 'src/locales';
import { GetObjectivesPaginationService } from 'src/services/architecture/business/objectives.service';
import { SaveObjectiveProcessService, UpdateObjectiveProcessService, DeleteObjectiveProcessService } from 'src/services/architecture/process/processRelations.service';
...
export function ProcessObjectiveLinkModal({ open, onClose, onSuccess, processId, existingItemIds, relationId, initialData, allowDelete }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const currentSelectedId = initialData?.objectiveId ?? initialData?.objective?.id ?? null;
        ...
        const response = await GetObjectivesPaginationService({ perPage: 1000 });
        // Process Objectives
    ...
    if (open) {
      setSubmitted(false);
      setForm({
        itemId: initialData?.objectiveId ?? initialData?.objective?.id ?? null,
  ...
  const handleSubmit = async () => {
    setSubmitted(true);
    if (!processId || !form.itemId || !form.impact) {
      toast.error(t('process.map.modals.common.missingData'));
      return;
    }
    const payload = {
      processId: Number(processId),
      objectiveId: Number(form.itemId),
      impact: form.impact
    };
    try {
      setLoading(true);
      if (relationId) {
        await UpdateObjectiveProcessService(relationId, payload);
      } else {
        await SaveObjectiveProcessService(payload);
      }
    ...
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 420 } }}>
      <Box sx={{ px: 2, py: 2 }}>
        <Typography variant="h6" sx={{ mb: 1 }}>
          {t('process.map.modals.objective.title')}
        </Typography>
        <Stack spacing={2} sx={{ pt: 1 }}>
          <Autocomplete
            ...
            renderInput={(params: any) => (
              <TextField
                {...params}
                label={t('process.map.modals.objective.fieldLabel')}
          ...
          <TextField
            ...
            error={submitted && !form.impact}
            helperText={submitted && !form.impact ? t('process.map.modals.common.missingData') : ''}
          >
