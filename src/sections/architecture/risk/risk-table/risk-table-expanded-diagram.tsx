...
import { GetToolRisksService, DeleteToolRiskRelationService } from 'src/services/architecture/risk/riskTools.service';
import { GetRiskActionMeasuresService, DeleteRiskActionMeasureRelationService } from 'src/services/architecture/risk/riskActionMeasures.service';
import {
    ...
    <Paper elevation={6} sx={{ px: { xs: 2, sm: 2.5 }, py: { xs: 2, sm: 2.5 }, borderRadius: 2.5, background: theme.palette.background.paper, border: `2px solid ${alpha(color, 0.3)}`, boxShadow: `0 4px 20px ${alpha(color, 0.2)}`, cursor: 'pointer', minWidth: 180, position: 'relative', overflow: 'hidden', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', '&:hover': { transform: 'translateY(-4px) scale(1.03)', boxShadow: `0 8px 32px ${alpha(color, 0.35)}`, borderColor: color }, '&:active': { transform: 'scale(0.98)' }, '&::before': { content: '""', position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: `linear-gradient(90deg, ${color}, ${alpha(color, 0.6)})`, boxShadow: `0 2px 8px ${alpha(color, 0.3)}` } }}>
      ...
      {onDeleteRelation && (
        <Tooltip title="Eliminar Relación">
          <IconButton size="small" onClick={(e) => { e.stopPropagation(); onDeleteRelation(); }} sx={{ position: 'absolute', top: 6, right: onLinkJob ? 36 : 6 }}>
    ...
    const childNodes: Node[] = (data.children || []).map((child: ChildNode, index: number) => {
      ...
      return {
        ...
        data: {
          ...
          onEditRelation: (nodeId === 'process' || nodeId === 'tool' || nodeId === 'measure') ? () => {
            const rid = Number.isFinite(relationId as any) ? (relationId as number) : null;
            setEditingRelation({ id: rid, data: relationData });
            if (nodeId === 'process') setOpenEditModal(true);
            if (nodeId === 'tool') setOpenToolCreateModal(true);
            if (nodeId === 'measure') setOpenMeasureCreateModal(true);
          } : undefined,
          onDeleteRelation: (nodeId === 'process' || nodeId === 'tool' || nodeId === 'measure') ? () => {
            const rid = Number.isFinite(relationId as any) ? (relationId as number) : null;
  ...
  return (
    <Card sx={{ width: '100%', position: 'relative', overflow: 'hidden', bgcolor: 'background.neutral', ...sx }}>
      ...
      {(nodeId === 'process' || nodeId === 'tool' || nodeId === 'measure') && (
        <Box sx={{ position: 'absolute', top: { xs: 12, sm: 16 }, right: { xs: 12, sm: 16 }, zIndex: 100 }}>
          <Stack direction="row" spacing={1}>
            {nodeId === 'process' && (
              <Button variant="contained" color="primary" startIcon={<Iconify icon="solar:add-circle-bold" />} onClick={() => { setEditingRelation(null); setOpenCreateModal(true); }} sx={{ borderRadius: 2, boxShadow: 4 }}>
                {t('process.table.actions.add')}
              </Button>
            )}
            {nodeId === 'tool' && (
              <Button variant="contained" color="primary" startIcon={<Iconify icon="solar:add-circle-bold" />} onClick={() => { setEditingRelation(null); setOpenToolCreateModal(true); }} sx={{ borderRadius: 2, boxShadow: 4 }}>
                {t('tools.table.actions.add')}
              </Button>
            )}
            {nodeId === 'measure' && (
              <Button variant="contained" color="primary" startIcon={<Iconify icon="solar:add-circle-bold" />} onClick={() => { setEditingRelation(null); setOpenMeasureCreateModal(true); }} sx={{ borderRadius: 2, boxShadow: 4 }}>
      ...
      <Dialog open={openDeleteConfirm} onClose={() => setOpenDeleteConfirm(false)}>
        ...
        <DialogContent>¿Deseas eliminar la relación?</DialogContent>
        <DialogActions>
                  ...
                  if (!rid) {
                     ...
                     const list: any[] = Array.isArray(raw)
                        ...
                        : Array.isArray((raw as any)?.data)
                          ? (raw as any).data
                          : [];
                     const found = list.find((it: any) => String(it?.risk?.id ?? '') === String(riskId) && String(it?.tool?.id ?? '') === String(deletingProcessId ?? '')); // Using deletingProcessId as generic related ID
                     rid = Number(found?.id);
                  }
                  if (!rid || !Number.isFinite(rid)) throw new Error('No se pudo identificar la relación');
                  await DeleteToolRiskRelationService(rid as number);
                } else if (nodeId === 'measure') {
                  if (!rid) {
                     const res = await GetRiskActionMeasuresService();
                     const raw = res?.data;
                     const list: any[] = Array.isArray(raw)
                        ...
                        : Array.isArray((raw as any)?.data)
                          ? (raw as any).data
                          : [];
                     const found = list.find((it: any) => String(it?.risk?.id ?? '') === String(riskId) && String(it?.actionMeasure?.id ?? '') === String(deletingProcessId ?? '')); // Using deletingProcessId as generic related ID
                     rid = Number(found?.id);
                  }
                  if (!rid || !Number.isFinite(rid)) throw new Error('No se pudo identificar la relación');
                  await DeleteRiskActionMeasureRelationService(rid as number);
                }
      ...
      {nodeId === 'tool' && (
        <RiskTableToolCreateModal
          ...
          onClose={() => { setOpenToolCreateModal(false); setEditingRelation(null); }}
          onSuccess={fetchExpandedData}
          riskId={riskId}
          parentNodeId={nodeId}
          relationId={editingRelation?.id ?? undefined}
      ...
      {nodeId === 'measure' && (
        <RiskTableMeasureCreateModal
          ...
          onClose={() => { setOpenMeasureCreateModal(false); setEditingRelation(null); }}
          onSuccess={fetchExpandedData}
          riskId={riskId}
          parentNodeId={nodeId}
          relationId={editingRelation?.id ?? undefined}
