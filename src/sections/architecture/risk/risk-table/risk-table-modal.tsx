...
export function RiskTableModal({ open, onClose, riskId, onSave }: Props) {
  ...
  const isFormValid = () =>
    ...
    formData.description.trim().length <= RISK_DESCRIPTION_MAX_LENGTH &&
    formData.code.trim() !== '' &&
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await GetRiskTablePaginationService({ page: 1, perPage: 50, search: code });
      const raw = res?.data;
      const list: any[] = Array.isArray(raw)
        ...
        : Array.isArray((raw as any)?.data?.data)
          ...
          : Array.isArray((raw as any)?.data)
            ? (raw as any).data
            : [];
      const duplicate = list.some((it: any) => {
        const itemCode = String(it?.code ?? '').trim().toLowerCase();
        const itemId = String(it?.id ?? '');
        const currentId = riskId ? String(riskId) : '';
      ...
      if (duplicate) {
        const msg = tf(
          'risk.table.messages.error.duplicateCode',
      ...
      const isDuplicateCode =
        ...
        (lower.includes('code') && (lower.includes('exist') || lower.includes('duplicate') || lower.includes('already')));

      console.error('Error saving risk:', { status, message: msg, data: error?.response?.data });

      if (isDuplicateCode) {
        const shown = typeof msg === 'string' && msg.trim()
          ? msg
          : tf('risk.table.messages.error.duplicateCode', 'The code already exists.', 'El código ya existe.');
        setCodeError(shown);
        toast.error(shown);
        return;
      }
        ...
        {loading ? (
          ...
          <Stack spacing={3} sx={{ pt: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => handleChange('description', String(e.target.value).slice(0, RISK_DESCRIPTION_MAX_LENGTH))}
              multiline
              rows={3}
              required
              disabled={saving}
              error={formData.description.length > RISK_DESCRIPTION_MAX_LENGTH}
              helperText={`${formData.description.length}/${RISK_DESCRIPTION_MAX_LENGTH}`}
              inputProps={{ maxLength: RISK_DESCRIPTION_MAX_LENGTH }}
            />
