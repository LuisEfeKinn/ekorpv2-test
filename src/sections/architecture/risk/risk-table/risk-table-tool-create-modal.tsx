...
import {
  ...
  UpdateToolRiskRelationService,
  GetToolRiskRelationByIdService
...
export function RiskTableToolCreateModal({ open, onClose, onSuccess, riskId, relationId, initialData }: Props) {
  ...
  useEffect(() => {
    const loadInitial = async () => {
      if (relationId && open) {
        try {
          const res = await GetToolRiskRelationByIdService(relationId);
          const raw = res.data;
          const data = raw?.data ?? raw;
          setForm({
            toolId: data.tool?.id ?? data.toolId ?? null,
            severityLevel: '1',
            frequency: '1',
            probability: '1',
            impact: '1',
            riskImpactLevelId: (typeof data.riskImpactLevel === 'object' ? data.riskImpactLevel?.id : data.riskImpactLevel) ?? data.riskImpactLevelId ?? null,
            riskProbabilityLevelId: (typeof data.riskProbabilityLevel === 'object' ? data.riskProbabilityLevel?.id : data.riskProbabilityLevel) ?? data.riskProbabilityLevelId ?? null,
            riskDeficiencyLevelId: (typeof data.riskDeficiencyLevel === 'object' ? data.riskDeficiencyLevel?.id : data.riskDeficiencyLevel) ?? data.riskDeficiencyLevelId ?? null,
            riskCondition: data.description ?? data.riskCondition ?? '',
          });
        } catch (e) {
          console.error(e);
          ...
          if (initialData) {
             setForm({
              toolId: initialData.tool?.id ?? initialData.toolId ?? null,
              severityLevel: '1',
              frequency: '1',
              probability: '1',
              impact: '1',
              riskImpactLevelId: (typeof initialData.riskImpactLevel === 'object' ? initialData.riskImpactLevel?.id : initialData.riskImpactLevel) ?? initialData.riskImpactLevelId ?? null,
              riskProbabilityLevelId: (typeof initialData.riskProbabilityLevel === 'object' ? initialData.riskProbabilityLevel?.id : initialData.riskProbabilityLevel) ?? initialData.riskProbabilityLevelId ?? null,
              riskDeficiencyLevelId: (typeof initialData.riskDeficiencyLevel === 'object' ? initialData.riskDeficiencyLevel?.id : initialData.riskDeficiencyLevel) ?? initialData.riskDeficiencyLevelId ?? null,
              riskCondition: initialData.description ?? initialData.riskCondition ?? '',
            });
          }
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      if (relationId) {
        await UpdateToolRiskRelationService(relationId, payload);
        toast.success(t('tools.table.messages.success.updated'));
      } else {
  ...
  return (
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 420 } }}>
      <Box sx={{ px: 2, py: 2 }}>
        <Typography variant="h6" sx={{ mb: 1 }}>{relationId ? t('tools.table.actions.edit') : t('riskMap.form.actions.addTools')}</Typography>
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          <Stack direction="row" spacing={1} sx={{ pt: 1 }}>
            ...
            <Button variant="contained" onClick={handleSubmit} disabled={loading} startIcon={loading ? <CircularProgress size={18} color="inherit" /> : undefined}>
              {relationId ? t('tools.table.actions.edit') : t('tools.table.actions.add')}
            </Button>
          </Stack>
        </Stack>
      </Box>
