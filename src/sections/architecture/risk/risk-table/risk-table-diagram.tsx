...
import { GetRiskTableMapByIdService } from 'src/services/architecture/risk/riskMap.service';

import { toast } from 'src/components/snackbar';
import { Iconify } from 'src/components/iconify';

import { RiskTableExpandedDiagram } from 'src/sections/architecture/risk/risk-table/risk-table-expanded-diagram';
...
export function RiskTableDiagram({ riskId, sx }: RiskTableDiagramProps) {
  const { t } = useTranslate('architecture');
  const theme = useTheme();
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        setLoading(true);
        const response = await GetRiskTableMapByIdService(riskId);
        const data = response.data;
        setMapData(data);
        generateNodesAndEdges(data);
      } catch (error) {
        console.error('Error al cargar el mapa de riesgos:', error);
        toast.error(t('data.map.diagram.messages.error.loadMapError'));
      } finally {
        setLoading(false);
      }
    };
    if (riskId) fetchMapData();
  }, [riskId, generateNodesAndEdges, t]);

  if (expandedNode) {
    return (
      <RiskTableExpandedDiagram
        riskId={riskId}
          ...
          <Paper elevation={2} sx={{ px: { xs: 1.5, sm: 2 }, py: { xs: 1, sm: 1.25 }, borderRadius: 1.5, bgcolor: alpha(theme.palette.background.paper, 0.9), backdropFilter: 'blur(8px)', border: `1px solid ${alpha(theme.palette.divider, 0.12)}` }}>
            <Stack spacing={0.75}>
              <Stack direction="row" spacing={1} alignItems="center">
                ...
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>{t('data.map.diagram.controls.zoom')}</Typography>
              </Stack>
              <Stack direction="row" spacing={1} alignItems="center">
                <Box sx={{ width: 16, height: 16, borderRadius: '50%', bgcolor: alpha(theme.palette.text.secondary, 0.08), display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <Iconify icon="eva:move-fill" width={10} sx={{ color: 'text.secondary' }} />
                </Box>
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>{t('data.map.diagram.controls.move')}</Typography>
              </Stack>
            </Stack>
          </Paper>
        </Box>
