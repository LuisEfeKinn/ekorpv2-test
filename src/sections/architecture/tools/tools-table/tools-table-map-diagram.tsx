...
export function ToolsTableMapDiagram({ toolId, sx }: Props) {
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        ...
        console.error('Error al cargar el mapa de herramienta:', error);
    ...
    if (toolId) fetchMapData();
  }, [generateNodesAndEdges, t, toolId]);
      ...
      <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 600 }}>
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('tools.map.diagram.loadingMap')}
          </Typography>
        </Stack>
      </Box>
    );
