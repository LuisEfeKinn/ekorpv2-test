...
export function ToolsFiltersResult({ filters, onFilters, typeLabel, onClearType, onReset, totalResults, sx }: Props) {
  ...
  return (
    <FiltersResult totalResults={totalResults} onReset={onReset} sx={sx}>
      <FiltersBlock label={t('tools.table.table.filters.status')} isShow={filters.status !== 'all'}>
        <Chip
          {...chipProps}
      ...
      <FiltersBlock label={`${t('tools.table.table.filters.keyword')}:`} isShow={!!filters.name}>
