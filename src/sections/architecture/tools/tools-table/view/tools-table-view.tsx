...
export function ToolsTableView() {
  ...
  type ToolFlowNode = {
    ...
    children?: ToolFlowNode[];
  };

  type ToolFlatRow = Record<string, unknown> & {
    id: number;
    label: string;
    level: number;
    parentId?: number;
  ...
  const [originalData, setOriginalData] = useState<ToolFlowNode[]>([]);
  ...
  const flattenDataWithHierarchy = useCallback((data: ToolFlowNode[], level = 0, parentId?: number): ToolFlatRow[] => {
    const flattened: ToolFlatRow[] = [];
  ...
  const flattenAllNodes = useCallback((data: ToolFlowNode[], level = 0, parentId?: number): ToolFlatRow[] => {
    const flattened: ToolFlatRow[] = [];
  ...
  const countFlowNodes = useCallback(function count(nodes: ToolFlowNode[]): number {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      setOriginalData(Array.isArray(data) ? (data as ToolFlowNode[]) : []);
  ...
  const flowTotalCount = useMemo(() => countFlowNodes(originalData), [countFlowNodes, originalData]);

  const baseRows = useMemo(
    () => (hasKeyword ? flattenAllNodes(originalData) : flattenDataWithHierarchy(originalData)),
    [flattenAllNodes, flattenDataWithHierarchy, hasKeyword, originalData]
  );

  const dataFiltered = applyFilter({ inputData: baseRows, filters: currentFilters });
  const tabCount = hasKeyword ? dataFiltered.length : flowTotalCount;

  const selectedTypeLabel = useMemo(() => {
    if (typeFromQuery === null) return null;
    return toolTypeOptions.find((opt) => opt.value === typeFromQuery)?.label ?? String(typeFromQuery);
  }, [toolTypeOptions, typeFromQuery]);
