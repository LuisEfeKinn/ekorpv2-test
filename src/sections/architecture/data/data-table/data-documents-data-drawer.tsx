...
import {
  SaveDataDocumentRelationService,
  UpdateDataDocumentRelationService,
  GetDataDocumentRelationByIdService,
...
export function DataDocumentsDataDrawer({ open, onClose, dataId, dataLabel, relationId, onSuccess, excludeIds, sx }: Props) {
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      setOptions([]);
      toast.error(t('data.map.documentsData.messages.listLoadError'));
    } finally {
  ...
  const loadRelation = useCallback(async () => {
    ...
    try {
      ...
      const res = await GetDataDocumentRelationByIdService(relationId);
      ...
      toast.error(t('data.map.documentsData.messages.loadError'));
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    if (selectedId == null) {
      toast.error(t('data.map.documentsData.form.validation.required'));
      return;
    }
    if (!observations.trim()) {
      toast.error(t('data.map.documentsData.form.validation.observationsRequired'));
      return;
    }
    try {
      setSaving(true);
      ...
      if (relationId != null) {
        await UpdateDataDocumentRelationService(relationId, payload);
        toast.success(t('data.map.documentsData.messages.updated'));
      } else {
        await SaveDataDocumentRelationService(payload);
        toast.success(t('data.map.documentsData.messages.created'));
      }
      onSuccess?.();
      onClose();
    } catch {
      toast.error(t('data.map.documentsData.messages.saveError'));
    } finally {
      setSaving(false);
    }
  ...
  const title = dataLabel
    ? t('data.map.documentsData.titleWithData', { data: dataLabel })
    : t('data.map.documentsData.title');
  ...
  return (
    <Drawer open={open} onClose={onClose} anchor="right" PaperProps={{ sx: { width: { xs: 1, sm: 520, md: 680 }, ...sx } }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 3, py: 2 }}>
        <Stack spacing={0.5}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('data.map.documentsData.subtitle')}
          </Typography>
        </Stack>
      ...
      <Stack spacing={2.5} sx={{ px: 3, py: 2.5 }}>
        <Typography variant="subtitle2">{t('data.map.documentsData.form.title')}</Typography>
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              label={t('data.map.documentsData.form.fields.document')}
              InputProps={{
                ...params.InputProps,
                endAdornment: (
                  <>
        ...
        <TextField
          label={t('data.map.documentsData.form.fields.observations')}
          value={observations}
        ...
        <Stack direction="row" spacing={1.5} justifyContent="flex-end">
          ...
          >
            {isEditing ? t('data.map.documentsData.actions.update') : t('data.map.documentsData.actions.create')}
          </Button>
        </Stack>
      </Stack>
    </Drawer>
