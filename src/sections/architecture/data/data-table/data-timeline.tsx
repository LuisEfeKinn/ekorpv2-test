...
export function DataTimeline({ data, field, loading }: Props) {
    ...
    return (
      <Card sx={{ p: 3 }}>
        <Typography variant="body2" color="text.secondary" textAlign="center">
          {t('data.timeline.messages.loading')}
        </Typography>
      </Card>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <Card sx={{ p: 3 }}>
        <EmptyContent
          filled
          title={t('data.timeline.emptyState.title')}
          description={t('data.timeline.emptyState.description')}
          sx={{ py: 10 }}
        />
      </Card>
    );
  ...
  const getDateLabel = (): string => {
    switch (field) {
      case 'renewal':
        return t('data.timeline.labels.renewalDate');
      case 'expired':
        return t('data.timeline.labels.expirationDate');
      case 'obsolescence':
        return t('data.timeline.labels.obsolescenceDate');
      default:
        return t('data.timeline.labels.adoptionDate');
    }
  };
          ...
          <Box sx={{ p: { xs: 2, sm: 3 }, position: 'relative' }}>
            ...
            <Stack spacing={{ xs: 1.5, sm: 2 }}>
              ...
              >
                ...
                >
                  {t('data.timeline.labels.adoptionDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
              ...
              >
                ...
                >
                  {t('data.timeline.labels.expirationDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
              ...
              >
                ...
                >
                  {t('data.timeline.labels.renewalDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
              ...
              >
                ...
                >
                  {t('data.timeline.labels.obsolescenceDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
        ...
        {selectedItem && (
          <Box sx={{ p: { xs: 2, sm: 3 }, position: 'relative' }}>
            ...
            >
              ...
              >
                {t('data.timeline.labels.dependencies')} {selectedItem.name}
              </Typography>
              <Chip
                label={`${selectedItem.dependencies?.length || 0} ${(selectedItem.dependencies?.length || 0) === 1 ? t('data.timeline.labels.dependenciesSingular') : t('data.timeline.labels.dependenciesPlural')}`}
                size="small"
                sx={{
                  bgcolor: `${selectedItem.color}15`,
                  color: selectedItem.color,
            ...
            <Stack spacing={{ xs: 1.5, sm: 2 }}>
              {(!selectedItem.dependencies || selectedItem.dependencies.length === 0) ? (
                ...
                >
                  ...
                  >
                    {t('data.timeline.labels.noDependencies')}
                  </Typography>
                </Box>
              ) : (
                selectedItem.dependencies.map((dependency, idx) => (
                  <Box key={idx}>
                    ...
                    >
                      {t('data.timeline.labels.type')}: {dependency.type}
                    </Typography>
