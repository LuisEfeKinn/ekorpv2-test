...
import {
  DeleteDataTableService
} from 'src/services/architecture/data/dataTable.service';
import {
  GetDataFlowService,
...
import { DataTableFiltersResult } from '../data-table-filters-result';
...
export function DataTableView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('data.table.table.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 50 }, // Columna de expansión
    { id: '', width: 88 }, // Columna de acciones
    { id: 'name', label: t('data.table.table.columns.name') },
    { id: 'description', label: t('data.table.table.columns.description') },
    { id: 'type', label: t('data.table.table.columns.type'), align: 'center' },
    { id: 'localExternal', label: t('data.table.table.columns.localExternal'), align: 'center' },
    { id: 'nomenclature', label: t('data.table.table.columns.nomenclature'), align: 'center' },
    { id: 'code', label: t('data.table.table.columns.code'), align: 'center' },
    { id: 'expirationDate', label: t('data.table.table.columns.expirationDate'), align: 'center' },
    { id: 'renewalDate', label: t('data.table.table.columns.renewalDate'), align: 'center' },
    { id: 'requiresSla', label: t('data.table.table.columns.requiresSla'), align: 'center' },
    { id: 'hasSla', label: t('data.table.table.columns.hasSla'), align: 'center' },
  ], [t]);
  ...
  // Función para cargar datos usando GetDataFlowService
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteDataTableService(id));
      await Promise.all(deletePromises);

      toast.success(t('data.table.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data tables:', error);
      toast.error(t('data.table.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const handleToggleExpand = useCallback(async (rowId: number, hasLoadedChildren: boolean) => {
    ...
    if (isCurrentlyExpanded) {
      ...
      if (!hasLoadedChildren) {
        ...
        try {
          const response = await GetDataFlowByIdService(rowId);
          if (response?.data) {
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      content={
        <>
          {t('data.table.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('data.table.title')}
          links={[
            { name: t('data.table.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('data.diagram.title'), href: paths.dashboard.architecture.dataDiagram },
            { name: t('data.table.title') },
          ]}
          action={
            <Button
...
function applyFilter({ inputData, comparator, filters }: ApplyFilterProps) {
  ...
  // Mantener el orden natural del árbol ya aplanado para que los hijos
  // respeten su posición inmediatamente debajo del padre.
