...
export function DataTimelineView() {
  ...
  const fetchTimelineData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('data.timeline.messages.error.fetchError'));
      setTimelineData([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading={t('data.timeline.title')}
        links={[
          { name: t('data.timeline.breadcrumbs.dashboard'), href: paths.dashboard.root },
          { name: t('data.diagram.title'), href: paths.dashboard.architecture.dataDiagram },
          { name: t('data.timeline.title') },
        ]}
        sx={{ mb: { xs: 3, md: 5 } }}
      />
      {/* Filters Section */}
      <Card sx={{ p: 2.5, mb: 3 }}>
        <Stack spacing={2.5}>
          <Typography variant="h6">{t('data.timeline.filters.status')}</Typography>
          ...
          >
            ...
            <Stack direction="row" alignItems="center" spacing={1}>
              <Typography variant="body2" color="text.secondary">
                {t('data.timeline.filters.inactive')}
              </Typography>
              <Switch
              ...
              />
              <Typography variant="body2" color="text.secondary">
                {t('data.timeline.filters.active')}
              </Typography>
            </Stack>

            {/* Field Selector */}
            <FormControl sx={{ minWidth: 200 }}>
              <InputLabel id="field-select-label">{t('data.timeline.filters.field')}</InputLabel>
              <Select
                labelId="field-select-label"
                id="field-select"
                value={filters.state.field}
                label={t('data.timeline.filters.field')}
                onChange={handleFilterField}
              >
                <MenuItem value="renewal">{t('data.timeline.filters.renewal')}</MenuItem>
                <MenuItem value="expired">{t('data.timeline.filters.expired')}</MenuItem>
                <MenuItem value="obsolescence">
                  {t('data.timeline.filters.obsolescence')}
                </MenuItem>
              </Select>
            </FormControl>
          </Stack>
      ...
      {canReset && (
        <FiltersResult totalResults={timelineData.length} onReset={handleResetFilters} sx={{ mb: 3 }}>
          <FiltersBlock
            label={`${t('data.timeline.filters.status')}:`}
          ...
          >
            <Chip
              {...chipProps}
              label={t(`data.timeline.filters.${filters.state.field}`)}
      ...
      {/* Timeline Component */}
      <DataTimeline data={timelineData} field={filters.state.field} loading={loading} />
    </DashboardContent>
  );
}
