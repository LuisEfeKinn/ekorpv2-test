...
export function DataJobDataDrawer({ open, onClose, dataId, dataLabel, relationId, onSuccess, excludeIds, sx }: Props) {
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      setOptions([]);
      toast.error(t('data.map.jobData.messages.listLoadError'));
    } finally {
  ...
  const loadRelation = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('data.map.jobData.messages.loadError'));
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    if (selectedId == null) {
      toast.error(t('data.map.jobData.form.validation.required'));
      return;
    }
    try {
      setSaving(true);
      ...
      if (relationId != null) {
        ...
        toast.success(t('data.map.jobData.messages.updated'));
      } else {
        await SaveJobDataRelationService(payload);
        toast.success(t('data.map.jobData.messages.created'));
      }
      onSuccess?.();
      onClose();
    } catch {
      toast.error(t('data.map.jobData.messages.saveError'));
    } finally {
      setSaving(false);
    }
  ...
  const title = dataLabel
    ? t('data.map.jobData.titleWithData', { data: dataLabel })
    : t('data.map.jobData.title');
  ...
  return (
    <Drawer open={open} onClose={onClose} anchor="right" PaperProps={{ sx: { width: { xs: 1, sm: 520, md: 680 }, ...sx } }}>
      ...
      <Stack spacing={2.5} sx={{ px: 3, py: 2.5 }}>
        <Typography variant="subtitle2">{t('data.map.jobData.form.title')}</Typography>
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              label={t('data.map.jobData.form.fields.job')}
              InputProps={{
                ...params.InputProps,
                endAdornment: (
                  <>
        ...
        <TextField
          label={t('data.map.jobData.form.fields.observations')}
          value={observations}
        ...
        <Stack direction="row" spacing={1.5} justifyContent="flex-end">
          ...
          >
            {isEditing ? t('data.map.jobData.actions.update') : t('data.map.jobData.actions.create')}
          </Button>
        </Stack>
      </Stack>
    </Drawer>
