...
export function DataSystemDataDrawer({ open, onClose, dataId, dataLabel, relationId, onSuccess, excludeIds, sx }: Props) {
  ...
  const loadList = useCallback(async () => {
    try {
      ...
      setOptions([]);
      toast.error(t('data.map.systemData.messages.listLoadError'));
    } finally {
  ...
  const loadRelation = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('data.map.systemData.messages.loadError'));
    } finally {
  ...
  const handleSubmit = useCallback(async () => {
    if (selectedId == null) {
      toast.error(t('data.map.systemData.form.validation.required'));
      return;
    }
    try {
      setSaving(true);
      ...
      if (relationId != null) {
        ...
        toast.success(t('data.map.systemData.messages.updated'));
      } else {
        await SaveSystemDataRelationService(payload);
        toast.success(t('data.map.systemData.messages.created'));
      }
      onSuccess?.();
      onClose();
    } catch {
      toast.error(t('data.map.systemData.messages.saveError'));
    } finally {
      setSaving(false);
    }
  ...
  const title = dataLabel
    ? t('data.map.systemData.titleWithData', { data: dataLabel })
    : t('data.map.systemData.title');
  ...
  return (
    <Drawer open={open} onClose={onClose} anchor="right" PaperProps={{ sx: { width: { xs: 1, sm: 520, md: 680 }, ...sx } }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 3, py: 2 }}>
        <Stack spacing={0.5}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('data.map.systemData.subtitle')}
          </Typography>
        </Stack>
      ...
      <Stack spacing={2.5} sx={{ px: 3, py: 2.5 }}>
        <Typography variant="subtitle2">{t('data.map.systemData.form.title')}</Typography>
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField
              {...params}
              label={t('data.map.systemData.form.fields.system')}
              InputProps={{
                ...params.InputProps,
                endAdornment: (
                  <>
        ...
        <TextField
          label={t('data.map.systemData.form.fields.observations')}
          value={observations}
        ...
        <Stack direction="row" spacing={1.5} justifyContent="flex-end">
          ...
          >
            {isEditing ? t('data.map.systemData.actions.update') : t('data.map.systemData.actions.create')}
          </Button>
        </Stack>
      </Stack>
    </Drawer>
