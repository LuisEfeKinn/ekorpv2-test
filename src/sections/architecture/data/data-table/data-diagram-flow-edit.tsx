...
export function DataDiagramFlowEditModal({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    if (!isFormValid()) {
      toast.error(t('data.table.messages.error.validation'));
      return;
    }
    ...
    try {
      ...
      toast.success(
        dataId
          ? t('data.table.messages.success.updated')
          : t('data.table.messages.success.created')
      );
      ...
      toast.error(
        dataId
          ? t('data.table.messages.error.updating')
          : t('data.table.messages.error.creating')
      );
    } finally {
      setSaving(false);
    }
        ...
        <Stack direction="row" spacing={2} alignItems="center">
          <Box sx={{ flex: 1 }}>
            <Typography variant="h5" sx={{ color: 'common.white', fontWeight: 700, mb: 0.5 }}>
              {dataId
                ? t('data.table.dialogs.edit.title')
                : t('data.table.dialogs.create.title')}
            </Typography>
          </Box>
        </Stack>
      </Box>
          ...
          <Stack spacing={0}>
            ...
            >
              ...
              <Stack spacing={2.5}>
                ...
                <TextField
                  fullWidth
                  label={t('data.table.table.columns.description')}
            ...
            <Paper elevation={0} sx={{ p: 3, borderRadius: 0 }}>
              ...
              <Stack spacing={2.5}>
                ...
                <TextField
                  fullWidth
                  label={t('data.table.table.columns.rules')}
              ...
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr 1fr' }, gap: 2 }}>
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t('data.table.table.columns.localExternal')}
                      required
                      placeholder="U/E"
                      InputProps={{
                        ...params.InputProps,
                ...
                <TextField
                  fullWidth
                  label={t('data.table.table.columns.nomenclature')}
                ...
                <TextField
                  fullWidth
                  label={t('data.table.table.columns.code')}
                  value={formData.code}
