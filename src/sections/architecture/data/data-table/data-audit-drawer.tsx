...
export function DataAuditDrawer({ open, onClose, dataId, dataLabel, auditId, onSuccess }: Props) {
  ...
  const relatedDataOptions = useMemo<AuditRelationOption[]>(
    () => [{ id: String(dataId), label: dataLabel ?? `#${dataId}` }],
    [dataId, dataLabel]
  );
  ...
  const loadAudit = useCallback(async () => {
    ...
    try {
      ...
      if (!record) {
        toast.error(t('data.map.audits.messages.loadError'));
  ...
  const formDefaults = useMemo<Partial<AuditFrotmValues>>(() => {
    const base: Partial<AuditFrotmValues> = {
      relatedSystems: relatedDataOptions,
  ...
  }, [currentAudit, relatedDataOptions, resolvedUserName]);
    ...
    async (values: AuditFrotmValues) => {
      ...
      try {
        if (isEdit && auditId != null) {
          ...
          toast.success(t('data.map.audits.messages.updated'));
        } else {
          await axios.post('/api/audits', payload);
          toast.success(t('data.map.audits.messages.created'));
        }
        onSuccess?.();
        onClose();
      } catch {
        toast.error(isEdit ? t('data.map.audits.messages.updateError') : t('data.map.audits.messages.saveError'));
      }
    },
    [auditId, dataId, isEdit, onClose, onSuccess, t]
  );

  const title = dataLabel
    ? `${t('audit.form.fields.relatedSystems.label')}: ${dataLabel}`
    ...
    <Drawer open={open} onClose={onClose} anchor="right" PaperProps={{ sx: { width: { xs: 1, md: 720 } } }}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ px: 3, py: 2 }}>
        <Stack spacing={0.5}>
          ...
          <Typography variant="body2" color="text.secondary">
            {loading ? t('data.map.diagram.loadingMap') : ' '}
          </Typography>
        </Stack>
      ...
      <Scrollbar sx={{ height: 1 }}>
        <AuditFrotm
          ...
          relatedSystemOptions={relatedDataOptions}
