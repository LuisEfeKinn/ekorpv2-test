...
import { DeleteApplicationTableService } from 'src/services/architecture/applications/applicationTable.service';
import {
...
// import { ApplicationFiltersPopover } from './components/application-filters-popover';
...
type ApplicationDiagramFlowProps = {
    ...
    >
      ...
      <Stack spacing={1.5}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Stack direction="row" spacing={0.5}>
              {hasSla && (
                <Tooltip title={t('application.diagram.tooltips.hasSla')}>
                  <Chip
                    size="small"
              ...
              {localExternal === 'E' && (
                <Tooltip title={t('application.diagram.tooltips.external')}>
                  <Chip
                    size="small"
          ...
          <Tooltip title={isExpanded ? t('application.diagram.tooltips.collapse') : t('application.diagram.tooltips.expand')}>
            <span>
              <IconButton
                size="small"
                onClick={onExpand}
              ...
              {expirationDate && (
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {t('application.diagram.labels.expiration')}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{
              ...
              {renewalDate && (
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {t('application.diagram.labels.renew')}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{
        ...
        >
          ...
          >
            {t('application.diagram.actions.add')}
          </Button>
          <Tooltip title={t('application.diagram.actions.edit')} placement="top">
            <Button
              size="medium"
              variant="outlined"
              onClick={onEdit}
          ...
          <Tooltip title={t('application.diagram.actions.viewMap') || 'Ver Mapa'} placement="top">
            <Button
              size="medium"
              variant="outlined"
              onClick={onViewMap}
        ...
        {nodeType === 'domain' && (
          <Stack spacing={1}>
            {corporateScope && (
              <Box>
                <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 600 }}>
                  {t('application.diagram.labels.corporateScope')}:
                </Typography>
            ...
            {owner && (
              <Box>
                <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 600 }}>
                  {t('application.diagram.labels.owner')}:
                </Typography>
                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.813rem' }}>
                  {owner}
                </Typography>
...
function CustomControls({
  ...
  return (
    ...
    >
      <Tooltip title={t('application.diagram.tooltips.expandAll')} placement="right">
      ...
      <Tooltip title={t('application.diagram.tooltips.collapseAll')} placement="right">
      ...
      <Tooltip title={t('application.diagram.tooltips.initialView')} placement="right">
      ...
      <Tooltip title={t('application.diagram.tooltips.restartDiagram')} placement="right">
      ...
      {/* <Tooltip title={t('application.diagram.filters.title')} placement="right">
...
export function ApplicationDiagramFlow({
  ...
  const loadRootNodes = useCallback(async () => {
    try {
      ...
      toast.error(t('application.diagram.messages.error.loading'));
    } finally {
      setLoading(false);
    }
  }, [t, filterTypeProp]);
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      await DeleteApplicationTableService(childId);
      toast.success(t('application.diagram.messages.success.deleted'));
      await loadRootNodes();
    } catch (error) {
      console.error('Error deleting application flow node:', error);
      toast.error(t('application.diagram.messages.error.deleting'));
    } finally {
      setDeletingId(null);
    }
    ...
    if (isExpanded) {
      ...
      if (!expandedNodesData.has(rootNodeId)) {
        try {
          ...
          const response = await GetApplicationFlowService(params);
  ...
  const handleExpandApplicationNode = useCallback(async (nodeId: number, rootNodeId: number) => {
    ...
    if (isExpanded || hasChildrenInData) {
      ...
      if (!hasLoadedFromService) {
        try {
          setLoadingExpand(true);
          const response = await GetApplicationFlowByIdService(nodeId);
      ...
      if ((isExpanded || hasChildren) && !isCollapsed) {
        if (hasChildren) {
          ...
          newNodes.push({
            ...
            data: {
              label: t('application.diagram.messages.noChildren'),
            },
            style: {
      ...
      >
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('application.diagram.messages.loading')}
          </Typography>
        </Stack>
      </Card>
    );
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
                {rootNodes.length} {rootNodes.length === 1 ? t('application.diagram.node.singular') : t('application.diagram.node.plural')}
              </Typography>
            </Stack>
          </Paper>
          ...
          >
            {t('application.diagram.addDiagram')}
          </Button>
        </Box>
        ...
        >
            ...
            >
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 600 }}>
                  {t('application.diagram.messages.loadingExpand')}
                </Typography>
              </Paper>
            </Box>
          )}
          ...
          >
            ...
            >
              <Tooltip title={t('application.diagram.tooltips.zoomIn')} placement="right">
                <IconButton
              ...
              <Tooltip title={t('application.diagram.tooltips.zoomOut')} placement="right">
                <IconButton
              ...
              <Tooltip title={t('application.diagram.tooltips.fitView')} placement="right">
                <IconButton
              ...
              <Tooltip title={t('application.diagram.tooltips.lock')} placement="right">
                <IconButton
                  onClick={() => {
