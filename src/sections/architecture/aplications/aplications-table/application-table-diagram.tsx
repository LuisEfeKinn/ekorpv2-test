...
import { GetApplicationTableMapByIdService } from 'src/services/architecture/applications/applicationMap.service';
...
export function ApplicationTableDiagram({ applicationId, sx }: DataTableDiagramProps) {
  const { t } = useTranslate('architecture');
  const theme = useTheme();
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        setLoading(true);
        const response = await GetApplicationTableMapByIdService(applicationId);
        const data = response.data;
        setMapData(data);
        generateNodesAndEdges(data);
      } catch (error) {
        console.error('Error al cargar el mapa:', error);
        toast.error(t('application.map.diagram.messages.error.loadMapError'));
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [applicationId, generateNodesAndEdges, t]);
