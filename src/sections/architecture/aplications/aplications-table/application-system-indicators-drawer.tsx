...
export function ApplicationSystemIndicatorsDrawer({
  ...
  const loadIndicators = useCallback(async () => {
    try {
      ...
      setIndicatorOptions(finalOptions);
    } catch {
      setIndicatorOptions([]);
  ...
  }, [creationDate, observations, onClose, onSuccess, relationId, selectedIndicatorId, systemId, t]);
  ...
  return (
    ...
    >
      ...
      <Stack spacing={2.5} sx={{ px: 3, py: 2.5 }}>
        ...
        <TextField
          label={t('application.map.systemIndicators.form.fields.creationDate')}
          type="date"
          value={creationDate}
