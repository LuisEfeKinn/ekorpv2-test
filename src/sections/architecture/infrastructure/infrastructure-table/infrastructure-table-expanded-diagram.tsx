...
import {
  DeleteInfraestructureTableMapNodeService,
  GetInfraestructureTableMapByIdExpandService,
...
import {
  type JobTechnologyRelation,
  GetJobTechnologyRelationsService,
  DeleteJobTechnologyRelationService,
} from 'src/services/architecture/infrastructure/jobTechnologies.service';
import {
...
import {
  type TechnologyProcessRelation,
  GetTechnologyProcessRelationsService,
  DeleteTechnologyProcessRelationService,
} from 'src/services/architecture/infrastructure/technologyProcesses.service';
import {
  type TechnologyDocumentRelation,
  GetTechnologyDocumentRelationsService,
  DeleteTechnologyDocumentRelationService,
} from 'src/services/architecture/infrastructure/technologyDocuments.service';
import {
  type TechnologyIndicatorRelation,
  GetTechnologyIndicatorRelationsService,
  DeleteTechnologyIndicatorRelationService,
} from 'src/services/architecture/infrastructure/technologyIndicators.service';

import { toast } from 'src/components/snackbar';
import { Iconify } from 'src/components/iconify';

import { InfrastructureTechnologyDataDrawer } from './infrastructure-technology-data-drawer';
import { InfrastructureTableNodeCreateModal } from './infrastructure-table-node-create-modal';
import { InfrastructureJobTechnologiesDrawer } from './infrastructure-job-technologies-drawer';
import { InfrastructureTechnologyAuditDrawer } from './infrastructure-technology-audit-drawer';
import { InfrastructureSystemTechnologiesDrawer } from './infrastructure-system-technologies-drawer';
import { InfrastructureTechnologyProcessesDrawer } from './infrastructure-technology-processes-drawer';
import { InfrastructureTechnologyDocumentsDrawer } from './infrastructure-technology-documents-drawer';
import { InfrastructureTechnologyIndicatorsDrawer } from './infrastructure-technology-indicators-drawer';
...
type InfrastructureTableExpandedDiagramProps = {
    ...
    >
      ...
      <Stack spacing={1.5} alignItems="center">
        ...
        <Typography variant="caption" sx={{ color: alpha(theme.palette.common.white, 0.85), fontWeight: 600, fontSize: '0.7rem' }}>
          {t('infrastructure.map.diagram.systemTitle')}
        </Typography>
      </Stack>
    </Paper>
  );
...
export function InfrastructureTableExpandedDiagram({
  infraestructureId,
  nodeId,
  nodeLabel,
  technologyLabel,
  ...
  sx,
}: InfrastructureTableExpandedDiagramProps) {
  const { t } = useTranslate('architecture');
  const theme = useTheme();
  ...
  // Drawer states
  const [jobTechOpen, setJobTechOpen] = useState(false);
  const [jobTechRelationId, setJobTechRelationId] = useState<number | null>(null);
  const [techProcessOpen, setTechProcessOpen] = useState(false);
  const [techProcessRelationId, setTechProcessRelationId] = useState<number | null>(null);
  const [techDataOpen, setTechDataOpen] = useState(false);
  const [techDataRelationId, setTechDataRelationId] = useState<number | null>(null);
  const [systemTechOpen, setSystemTechOpen] = useState(false);
  const [systemTechRelationId, setSystemTechRelationId] = useState<number | null>(null);
  const [techIndicatorOpen, setTechIndicatorOpen] = useState(false);
  const [techIndicatorRelationId, setTechIndicatorRelationId] = useState<number | null>(null);
  const [techDocOpen, setTechDocOpen] = useState(false);
  const [techDocRelationId, setTechDocRelationId] = useState<number | null>(null);
  ...
  const isDataModule = useMemo(() => {
    const id = String(nodeId ?? '').toLowerCase();
    return id === 'data' || id === 'datos' || id === 'dato' || (id.includes('data') && !id.includes('process'));
  }, [nodeId]);
  ...
  const isRelationModule = useMemo(() =>
    isJobsModule || isProcessModule || isDataModule || isSystemsModule || isIndicatorsModule || isDocumentsModule || isAuditsModule,
    [isJobsModule, isProcessModule, isDataModule, isSystemsModule, isIndicatorsModule, isDocumentsModule, isAuditsModule]
  );
  ...
  const fetchExpandedData = useCallback(async () => {
    try {
      setLoading(true);
      const entityId = Number(infraestructureId);

      if (isJobsModule) {
        const res = await GetJobTechnologyRelationsService();
        const raw = (res as { data?: unknown })?.data;
        const list: JobTechnologyRelation[] = Array.isArray(raw) ? raw : Array.isArray((raw as any)?.[0]) ? (raw as any)[0] : [];
        const filtered = list.filter((r) => Number(r?.technology?.id) === entityId);
        setMapData({ id: nodeId, label: nodeLabel, children: filtered.map((r) => ({ id: String(r.id), label: r.job?.name ?? `#${r.job?.id ?? r.id}` })) });
        return;
      }

      if (isProcessModule) {
        const res = await GetTechnologyProcessRelationsService();
        const raw = (res as { data?: unknown })?.data;
        const list: TechnologyProcessRelation[] = Array.isArray(raw) ? raw : Array.isArray((raw as any)?.[0]) ? (raw as any)[0] : [];
        const filtered = list.filter((r) => Number(r?.technology?.id) === entityId);
      ...
      if (isDataModule) {
        ...
        const filtered = list.filter((r) => Number(r?.technology?.id ?? (r as any)?.technologyId) === entityId);
        setMapData({ id: nodeId, label: nodeLabel, children: filtered.map((r) => ({ id: String(r.id), label: r.data?.name ?? `#${r.data?.id ?? r.id}` })) });
        return;
      }
      ...
      if (isSystemsModule) {
        ...
        const raw = (res as { data?: unknown })?.data;
        const list: SystemTechnologyRelation[] = Array.isArray(raw) ? raw : Array.isArray((raw as any)?.[0]) ? (raw as any)[0] : [];
        const filtered = list.filter((r) => Number(r?.technology?.id ?? (r as any)?.technologyId ?? (r as any)?.technology_id) === entityId);
      ...
      if (isIndicatorsModule) {
        const res = await GetTechnologyIndicatorRelationsService();
        const raw = (res as { data?: unknown })?.data;
        const list: TechnologyIndicatorRelation[] = Array.isArray(raw) ? raw : Array.isArray((raw as any)?.[0]) ? (raw as any)[0] : [];
        const filtered = list.filter((r) => Number(r?.technology?.id) === entityId);
      ...
      if (isDocumentsModule) {
        const res = await GetTechnologyDocumentRelationsService();
        const raw = (res as { data?: unknown })?.data;
        const list: TechnologyDocumentRelation[] = Array.isArray(raw) ? raw : Array.isArray((raw as any)?.[0]) ? (raw as any)[0] : [];
        const filtered = list.filter((r) => Number(r?.technology?.id) === entityId);
      ...
      if (isAuditsModule) {
        const res = await axios.get(`/api/audits?technologyId=${encodeURIComponent(String(entityId))}`);
        const raw = (res as { data?: unknown })?.data;
      ...
      const response = await GetInfraestructureTableMapByIdExpandService(infraestructureId, nodeId);
      setMapData(response.data);
    } catch (error) {
      console.error('Error loading expanded data:', error);
      toast.error(t('infrastructure.map.messages.error.loadMapError'));
    } finally {
      setLoading(false);
    }
  }, [infraestructureId, isJobsModule, isProcessModule, isDataModule, isSystemsModule, isIndicatorsModule, isDocumentsModule, isAuditsModule, nodeId, nodeLabel, t]);
  ...
  const generateNodesAndEdges = useCallback((data: MapData) => {
    ...
    const childNodes: Node[] = data.children.map((child: ChildNode, index: number) => {
      ...
      const handleEditRelation = () => {
        ...
        if (isJobsModule) { setJobTechRelationId(relationId); setJobTechOpen(true); return; }
        if (isProcessModule) { setTechProcessRelationId(relationId); setTechProcessOpen(true); return; }
        if (isDataModule) { setTechDataRelationId(relationId); setTechDataOpen(true); return; }
        if (isSystemsModule) { setSystemTechRelationId(relationId); setSystemTechOpen(true); return; }
        if (isIndicatorsModule) { setTechIndicatorRelationId(relationId); setTechIndicatorOpen(true); return; }
        if (isDocumentsModule) { setTechDocRelationId(relationId); setTechDocOpen(true); return; }
    ...
    setEdges(newEdges);
  }, [colors, isJobsModule, isProcessModule, isDataModule, isSystemsModule, isIndicatorsModule, isDocumentsModule, isAuditsModule, isRelationModule, onNavigateToChild, setEdges, setNodes]);

  const handleDeleteNode = async () => {
    if (!deleteDialog.nodeId) return;
    try {
      setDeleting(true);
      if (isJobsModule) { await DeleteJobTechnologyRelationService(deleteDialog.nodeId); toast.success(t('infrastructure.map.jobTechnologies.messages.deleted')); }
      else if (isProcessModule) { await DeleteTechnologyProcessRelationService(deleteDialog.nodeId); toast.success(t('infrastructure.map.technologyProcesses.messages.deleted')); }
      else if (isDataModule) { await DeleteTechnologyDataRelationService(deleteDialog.nodeId); toast.success(t('infrastructure.map.technologyData.messages.deleted')); }
      else if (isSystemsModule) { await DeleteSystemTechnologyRelationService(deleteDialog.nodeId); toast.success(t('infrastructure.map.systemTechnologies.messages.deleted')); }
      else if (isIndicatorsModule) { await DeleteTechnologyIndicatorRelationService(deleteDialog.nodeId); toast.success(t('infrastructure.map.technologyIndicators.messages.deleted')); }
      else if (isDocumentsModule) { await DeleteTechnologyDocumentRelationService(deleteDialog.nodeId); toast.success(t('infrastructure.map.technologyDocuments.messages.deleted')); }
      else if (isAuditsModule) { await axios.delete(`/api/audits/${encodeURIComponent(String(deleteDialog.nodeId))}`); toast.success(t('infrastructure.map.audits.messages.deleted')); }
      else { await DeleteInfraestructureTableMapNodeService(infraestructureId, deleteDialog.nodeId); toast.success(t('infrastructure.table.messages.success.deleted')); }
      setDeleteDialog({ open: false, nodeId: null });
      await fetchExpandedData();
    } catch (error) {
      console.error('Error deleting node:', error);
      toast.error(t('infrastructure.table.messages.error.deleting'));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleAddClick = () => {
    if (isJobsModule) { setJobTechRelationId(null); setJobTechOpen(true); return; }
    if (isProcessModule) { setTechProcessRelationId(null); setTechProcessOpen(true); return; }
    if (isDataModule) { setTechDataRelationId(null); setTechDataOpen(true); return; }
    if (isSystemsModule) { setSystemTechRelationId(null); setSystemTechOpen(true); return; }
    if (isIndicatorsModule) { setTechIndicatorRelationId(null); setTechIndicatorOpen(true); return; }
    if (isDocumentsModule) { setTechDocRelationId(null); setTechDocOpen(true); return; }
  ...
  const breadcrumbNav = (
    <Paper elevation={4} sx={{ px: 2, py: 1, borderRadius: 2, bgcolor: alpha(theme.palette.background.paper, 0.95), backdropFilter: 'blur(10px)', border: `1px solid ${alpha(theme.palette.divider, 0.1)}` }}>
      <Stack direction="row" spacing={1} alignItems="center">
        <Tooltip title={t('infrastructure.map.diagram.subDiagram.buttonBack')}>
        ...
        {path && path.length > 0 ? (
          <Stack direction="row" spacing={0.5} alignItems="center">
            <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary' }}>{t('infrastructure.map.diagram.subDiagram.home')}</Typography>
  ...
  const addButton = (
    <Button variant="contained" color="primary" startIcon={<Iconify icon="solar:add-circle-bold" />} onClick={handleAddClick} sx={{ borderRadius: 2, boxShadow: 4 }}>
      {t('infrastructure.table.actions.add')}
    </Button>
  );
  ...
  const drawers = (
    <>
      {!isRelationModule && (
        <InfrastructureTableNodeCreateModal open={openCreateModal} onClose={() => setOpenCreateModal(false)} onSuccess={fetchExpandedData} infraestructureId={infraestructureId} parentNodeId={nodeId} />
      )}
      <InfrastructureJobTechnologiesDrawer open={jobTechOpen} onClose={() => { setJobTechOpen(false); setJobTechRelationId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} relationId={jobTechRelationId} />
      <InfrastructureTechnologyProcessesDrawer open={techProcessOpen} onClose={() => { setTechProcessOpen(false); setTechProcessRelationId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} relationId={techProcessRelationId} />
      <InfrastructureTechnologyDataDrawer open={techDataOpen} onClose={() => { setTechDataOpen(false); setTechDataRelationId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} relationId={techDataRelationId} />
      <InfrastructureSystemTechnologiesDrawer open={systemTechOpen} onClose={() => { setSystemTechOpen(false); setSystemTechRelationId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} relationId={systemTechRelationId} />
      <InfrastructureTechnologyIndicatorsDrawer open={techIndicatorOpen} onClose={() => { setTechIndicatorOpen(false); setTechIndicatorRelationId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} relationId={techIndicatorRelationId} />
      <InfrastructureTechnologyDocumentsDrawer open={techDocOpen} onClose={() => { setTechDocOpen(false); setTechDocRelationId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} relationId={techDocRelationId} />
      <InfrastructureTechnologyAuditDrawer open={auditOpen} onClose={() => { setAuditOpen(false); setAuditId(null); }} onSuccess={fetchExpandedData} technologyId={Number(infraestructureId)} technologyLabel={technologyLabel} auditId={auditId} />
      <Dialog open={deleteDialog.open} onClose={() => !deleting && setDeleteDialog({ open: false, nodeId: null })} maxWidth="xs" fullWidth>
        <DialogTitle>{t('infrastructure.table.dialogs.delete.title')}</DialogTitle>
        <DialogContent><DialogContentText>{t('infrastructure.table.dialogs.delete.content')}</DialogContentText></DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button variant="outlined" color="inherit" onClick={() => setDeleteDialog({ open: false, nodeId: null })} disabled={deleting}>{t('infrastructure.table.actions.cancel')}</Button>
          <Button variant="contained" color="error" onClick={handleDeleteNode} disabled={deleting} startIcon={deleting ? <CircularProgress size={20} color="inherit" /> : <Iconify icon="solar:trash-bin-trash-bold" />}>{t('infrastructure.table.dialogs.delete.confirm')}</Button>
        </DialogActions>
      </Dialog>
    </>
  );
    ...
    return (
      <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 600 }}>
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">{t('infrastructure.map.diagram.loadingMap')}</Typography>
        </Stack>
      </Box>
    );
  }
  ...
  if (!mapData) {
    return (
      <Box sx={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 600 }}>
        <Typography variant="body1" color="text.secondary">{t('infrastructure.map.diagram.noData')}</Typography>
      </Box>
    );
  }
        ...
        <Box sx={{ width: '100%', height: 600, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Stack alignItems="center" spacing={3}>
            ...
            <Stack spacing={1} alignItems="center">
              ...
              <Typography variant="body2" color="text.secondary">{t('infrastructure.map.diagram.subDiagram.noRelations')}</Typography>
            </Stack>
          </Stack>
        </Box>
        {drawers}
        ...
        <Box sx={{ position: 'absolute', top: { xs: 60, sm: 68 }, left: { xs: 12, sm: 16 }, zIndex: 100, pointerEvents: 'none' }}>
          <Paper elevation={2} sx={{ px: { xs: 1.5, sm: 2 }, py: { xs: 1, sm: 1.25 }, borderRadius: 1.5, bgcolor: alpha(theme.palette.background.paper, 0.9), backdropFilter: 'blur(8px)', border: `1px solid ${alpha(theme.palette.divider, 0.12)}` }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>{mapData.children.length} {t('infrastructure.map.diagram.conectedModules')}</Typography>
            </Stack>
          </Paper>
        </Box>
