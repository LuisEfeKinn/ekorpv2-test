...
import { GetInfraestructureTableMapByIdService } from 'src/services/architecture/infrastructure/infrastructureMap.service';

import { toast } from 'src/components/snackbar';
import { Iconify } from 'src/components/iconify';

import { InfrastructureTableExpandedDiagram } from './infrastructure-table-expanded-diagram';
...
type InfraestructureTableDiagramProps = {
...
export function InfrastructureTableDiagram({ infraestructureId, sx }: InfraestructureTableDiagramProps) {
  const { t } = useTranslate('architecture');
  const theme = useTheme();
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        setLoading(true);
        const response = await GetInfraestructureTableMapByIdService(infraestructureId);
        const data = response.data;
        setMapData(data);
        generateNodesAndEdges(data);
      } catch (error) {
        console.error('Error al cargar el mapa:', error);
        toast.error(t('infrastructure.map.diagram.messages.error.loadMapError'));
      } finally {
        setLoading(false);
      }
    };

    if (infraestructureId) {
      fetchMapData();
    }
  }, [infraestructureId, generateNodesAndEdges, t]);

  // Si hay un nodo expandido, mostrar el diagrama expandido
  if (expandedNode) {
    return (
      <InfrastructureTableExpandedDiagram
        infraestructureId={infraestructureId}
        nodeId={expandedNode.id}
        nodeLabel={expandedNode.label}
        technologyLabel={mapData?.label}
