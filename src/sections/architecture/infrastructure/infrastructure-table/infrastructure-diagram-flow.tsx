...
import { DeleteInfraestructureTableService } from 'src/services/architecture/infrastructure/infrastructureTable.service';
import {
  GetInfraestructureFlowService,
  GetInfraestructureFlowByIdService
} from 'src/services/architecture/infrastructure/infrastructureMap.service';

import { toast } from 'src/components/snackbar';
import { Iconify } from 'src/components/iconify';

import { InfrastructureTableDiagram } from './infrastructure-table-diagram';
import { InfrastructureDiagramFlowEditModal } from './infrastructure-diagram-flow-edit';
import { InfrastructureDiagramFlowCreateModal } from './infrastructure-diagram-flow-create';
...
type InfrastructureDiagramFlowProps = {
    ...
    >
      ...
      <Stack spacing={1.5}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
          <Stack direction="row" spacing={1} alignItems="center">
            ...
            <Stack direction="row" spacing={0.5}>
              {hasSla && (
                <Tooltip title={t('infrastructure.diagram.tooltips.hasSla')}>
                  <Chip
                    size="small"
              ...
              {localExternal === 'E' && (
                <Tooltip title={t('infrastructure.diagram.tooltips.external')}>
                  <Chip
                    size="small"
          ...
          <Tooltip title={isExpanded ? t('infrastructure.diagram.tooltips.collapse') : t('infrastructure.diagram.tooltips.expand')}>
            <span>
              <IconButton
                size="small"
                onClick={onExpand}
              ...
              {expirationDate && (
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {t('infrastructure.diagram.labels.expiration')}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{
              ...
              {renewalDate && (
                <Stack direction="row" spacing={1} alignItems="center">
                  ...
                  <Box sx={{ flex: 1, minWidth: 0 }}>
                    ...
                    >
                      {t('infrastructure.diagram.labels.renew')}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{
        ...
        >
          ...
          >
            {t('infrastructure.diagram.actions.add')}
          </Button>
          <Tooltip title={t('infrastructure.diagram.actions.edit')} placement="top">
            <Button
              size="medium"
              variant="outlined"
              onClick={onEdit}
          ...
          <Tooltip title={t('infrastructure.diagram.actions.viewMap') || 'Ver Mapa'} placement="top">
            <Button
              size="medium"
              variant="outlined"
              onClick={onViewMap}
        ...
        {nodeType === 'domain' && (
          <Stack spacing={1}>
            {corporateScope && (
              <Box>
                <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 600 }}>
                  {t('infrastructure.diagram.labels.corporateScope')}:
                </Typography>
            ...
            {owner && (
              <Box>
                <Typography variant="caption" sx={{ color: 'text.disabled', fontWeight: 600 }}>
                  {t('infrastructure.diagram.labels.owner')}:
                </Typography>
                <Typography variant="body2" sx={{ color: 'text.secondary', fontSize: '0.813rem' }}>
                  {owner}
                </Typography>
...
function CustomControls({
  ...
  return (
    ...
    >
      <Tooltip title={t('infrastructure.diagram.tooltips.expandAll')} placement="right">
      ...
      <Tooltip title={t('infrastructure.diagram.tooltips.collapseAll')} placement="right">
      ...
      <Tooltip title={t('infrastructure.diagram.tooltips.initialView')} placement="right">
      ...
      <Tooltip title={t('infrastructure.diagram.tooltips.restartDiagram')} placement="right">
...
export function InfrastructureDiagramFlow({
  filterType: filterTypeProp = 'domains',
  sx
}: InfrastructureDiagramFlowProps) {
  const { t } = useTranslate('architecture');
  const theme = useTheme();
  ...
  const loadRootNodes = useCallback(async () => {
    try {
      ...
      if (filterTypeProp === 'domains') {
        ...
        const response = await GetTechnologyTypesPaginationService({});
        if (response?.data && Array.isArray(response.data[0])) {
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      await DeleteInfraestructureTableService(childId);
      toast.success(t('infrastructure.diagram.messages.success.deleted'));
      await loadRootNodes();
    } catch (error) {
      console.error('Error deleting data flow node:', error);
      toast.error(t('infrastructure.diagram.messages.error.deleting'));
    } finally {
      setDeletingId(null);
    }
    ...
    if (isExpanded) {
      ...
      if (!expandedNodesData.has(rootNodeId)) {
        try {
          ...
          const response = await GetInfraestructureFlowService(params);
  ...
  const handleExpandApplicationNode = useCallback(async (nodeId: number, rootNodeId: number) => {
    ...
    if (isExpanded || hasChildrenInData) {
      ...
      if (!hasLoadedFromService) {
        try {
          setLoadingExpand(true);
          const response = await GetInfraestructureFlowByIdService(nodeId);
  ...
  // Función para aplicar layout usando dagre
  const applyDagreLayout = useCallback((nodesToLayout: Node[], edgesToLayout: Edge[]): Node[] => {
    ...
    nodesToLayout.forEach((node) => {
      const width = node.type === 'rootNode' ? 360 : node.type === 'default' ? 200 : 320;
      const height = node.type === 'rootNode' ? 200 : node.type === 'default' ? 80 : 280;
  ...
  const generateNodesAndEdges = useCallback(() => {
      ...
      if ((isExpanded || hasChildren) && !isCollapsed) {
        if (hasChildren) {
          ...
          newNodes.push({
            ...
            data: {
              label: t('infrastructure.diagram.messages.noChildren'),
            },
            style: {
    ...
    rootNodes.forEach((rootNode) => {
      ...
      // Crear nodo raíz (dominio o tipo)
      newNodes.push({
        id: `root-${rootNode.id}`,
        type: 'rootNode',
        position: { x: 0, y: 0 }, // Posición temporal
        data: {
          ...
          hasChildren: true, // Siempre puede tener hijos
      ...
      >
        <Stack alignItems="center" spacing={2}>
          ...
          <Typography variant="body2" color="text.secondary">
            {t('infrastructure.diagram.messages.loading')}
          </Typography>
        </Stack>
      </Card>
    );
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography variant="subtitle2" sx={{ fontWeight: 600 }}>
                {rootNodes.length} {rootNodes.length === 1 ? t('infrastructure.diagram.node.singular') : t('infrastructure.diagram.node.plural')}
              </Typography>
            </Stack>
          </Paper>
          ...
          >
            {t('infrastructure.diagram.addDiagram')}
          </Button>
        </Box>
            ...
            >
              ...
              >
                ...
                <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 600 }}>
                  {t('infrastructure.diagram.messages.loadingExpand')}
                </Typography>
              </Paper>
            </Box>
          )}
      ...
      <InfrastructureDiagramFlowCreateModal
        open={openModal}
        onClose={() => {
          setOpenModal(false);
          setSelectedParentId(null);
      ...
      <InfrastructureDiagramFlowEditModal
        open={openEditModal}
        ...
        >
          {diagramModal.dataId && (
            <Box sx={{ flex: 1, position: 'relative' }}>
              <InfrastructureTableDiagram
                infraestructureId={diagramModal.dataId}
