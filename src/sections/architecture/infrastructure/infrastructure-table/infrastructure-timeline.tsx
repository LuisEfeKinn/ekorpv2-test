...
export function InfrastructureTimeline({ data, field, loading }: Props) {
    ...
    return (
      <Card sx={{ p: 3 }}>
        <Typography variant="body2" color="text.secondary" textAlign="center">
          {t('infrastructure.timeline.messages.loading')}
        </Typography>
      </Card>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <Card sx={{ p: 3 }}>
        <EmptyContent
          filled
          title={t('infrastructure.timeline.emptyState.title')}
          description={t('infrastructure.timeline.emptyState.description')}
          sx={{ py: 10 }}
        />
      </Card>
    );
  ...
  const getDateLabel = (): string => {
    switch (field) {
      case 'renewal':
        return t('infrastructure.timeline.labels.renewalDate');
      case 'expired':
        return t('infrastructure.timeline.labels.expirationDate');
      case 'obsolescence':
        return t('infrastructure.timeline.labels.obsolescenceDate');
      default:
        return t('infrastructure.timeline.labels.adoptionDate');
    }
  };
          ...
          <Box sx={{ p: { xs: 2, sm: 3 }, position: 'relative' }}>
            ...
            <Stack spacing={{ xs: 1.5, sm: 2 }}>
              ...
              >
                ...
                >
                  {t('infrastructure.timeline.labels.adoptionDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
              ...
              >
                ...
                >
                  {t('infrastructure.timeline.labels.expirationDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
              ...
              >
                ...
                >
                  {t('infrastructure.timeline.labels.renewalDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
              ...
              >
                ...
                >
                  {t('infrastructure.timeline.labels.obsolescenceDate')}
                </Typography>
                <Typography
                  variant="body2"
                  sx={{
        ...
        {selectedItem && (
          <Box sx={{ p: { xs: 2, sm: 3 }, position: 'relative' }}>
            ...
            >
              ...
              >
                {t('infrastructure.timeline.labels.dependencies')} {selectedItem.name}
              </Typography>
              <Chip
                label={`${selectedItem.dependencies?.length || 0} ${(selectedItem.dependencies?.length || 0) === 1 ? t('infrastructure.timeline.labels.dependenciesSingular') : t('infrastructure.timeline.labels.dependenciesPlural')}`}
                size="small"
                sx={{
                  bgcolor: `${selectedItem.color}15`,
                  color: selectedItem.color,
            ...
            <Stack spacing={{ xs: 1.5, sm: 2 }}>
              {(!selectedItem.dependencies || selectedItem.dependencies.length === 0) ? (
                ...
                >
                  ...
                  >
                    {t('infrastructure.timeline.labels.noDependencies')}
                  </Typography>
                </Box>
              ) : (
                selectedItem.dependencies.map((dependency, idx) => (
                  <Box key={idx}>
                    ...
                    >
                      {t('infrastructure.timeline.labels.type')}: {dependency.type}
                    </Typography>
