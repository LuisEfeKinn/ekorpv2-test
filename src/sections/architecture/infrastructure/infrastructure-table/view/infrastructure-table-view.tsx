...
import {
  DeleteInfraestructureTableService
...
import { InfrastructureTableRow } from '../infrastructure-table-row';
import { InfrastructureTableToolbar } from '../infrastructure-table-toolbar';
import { InfrastructureDiagramFlowEditModal } from '../infrastructure-diagram-flow-edit';
import { InfrastructureTableFiltersResult } from '../infrastructure-table-filters-result';
...
export function InfrastructureTableView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('infrastructure.table.table.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 50 }, // Columna de expansión
    { id: '', width: 88 }, // Columna de acciones
    { id: 'name', label: t('infrastructure.table.table.columns.name') },
    { id: 'description', label: t('infrastructure.table.table.columns.description') },
    { id: 'type', label: t('infrastructure.table.table.columns.type'), align: 'center' },
    { id: 'localExternal', label: t('infrastructure.table.table.columns.localExternal'), align: 'center' },
    { id: 'nomenclature', label: t('infrastructure.table.table.columns.nomenclature'), align: 'center' },
    { id: 'code', label: t('infrastructure.table.table.columns.code'), align: 'center' },
    { id: 'expirationDate', label: t('infrastructure.table.table.columns.expirationDate'), align: 'center' },
    { id: 'renewalDate', label: t('infrastructure.table.table.columns.renewalDate'), align: 'center' },
    { id: 'requiresSla', label: t('infrastructure.table.table.columns.requiresSla'), align: 'center' },
    { id: 'hasSla', label: t('infrastructure.table.table.columns.hasSla'), align: 'center' },
  ], [t]);
  ...
  // Función para cargar datos usando GetInfraestructureFlowService
  const loadData = useCallback(async () => {
    try {
      const response = await GetInfraestructureFlowService();
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteInfraestructureTableService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('infrastructure.table.messages.success.deleted'));
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteInfraestructureTableService(id));
      await Promise.all(deletePromises);

      toast.success(t('infrastructure.table.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data tables:', error);
      toast.error(t('infrastructure.table.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const handleToggleExpand = useCallback(async (rowId: number, hasLoadedChildren: boolean) => {
    ...
    if (isCurrentlyExpanded) {
      ...
      if (!hasLoadedChildren) {
        ...
        try {
          const response = await GetInfraestructureFlowByIdService(rowId);
          if (response?.data) {
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      content={
        <>
          {t('infrastructure.table.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('infrastructure.table.title')}
          links={[
            { name: t('infrastructure.table.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('infrastructure.diagram.title'), href: paths.dashboard.architecture.infrastructureDiagram },
            { name: t('infrastructure.table.title') },
          ]}
          action={
            <Button
        ...
        <Card>
          ...
          {canReset && (
            <InfrastructureTableFiltersResult
              filters={currentFilters}
                  ...
                  {dataFiltered
                    ...
                    .map((row) => (
                      <InfrastructureTableRow
                        key={row.id}
                        row={row}
                        mapHref={paths.dashboard.architecture.infrastructureTableMap(row.id)}
