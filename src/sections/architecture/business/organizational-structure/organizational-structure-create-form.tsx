...
export function OrganizationalStructureCreateEditDrawer({ open, onClose, onSuccess, currentOrganizationalUnit }: Props) {
          ...
          .then((response) => {
            ...
            if (data) {
              ...
              const mergedValues = {
                ...
                expectedResults: data.expectedResults ?? data.expected_results ?? defaultValues.expectedResults,
                parentId: data.parent?.id ? String(data.parent.id) : (data.parentId ? String(data.parentId) : defaultValues.parentId),
                orgUnitTypeId: data.orgUnitTypeId ? String(data.orgUnitTypeId) : (data.orgUnitType?.id ? String(data.orgUnitType.id) : defaultValues.orgUnitTypeId),
              };

              // Ensure the selected parent is in the options list
              if (data.parent?.id) {
                const parentId = String(data.parent.id);
                const parentLabel = data.parent.name || data.parent.code || `#${parentId}`;
              ...
              if (data.orgUnitTypeId) {
                const typeId = String(data.orgUnitTypeId);
                const typeLabel = data.orgUnitType?.name || `#${typeId}`;
  ...
  }, [open, defaultValues, reset, isEdit, currentOrganizationalUnit?.id, t]);
