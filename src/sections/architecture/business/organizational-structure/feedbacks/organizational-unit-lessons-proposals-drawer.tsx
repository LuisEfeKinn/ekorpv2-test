...
const getLessonProposalSchema = (t: (key: string) => string) => zod.object({
  ...
  problem: zod.string().min(1, { message: 'El problema es obligatorio' }),
  rootCause: zod.string().min(1, { message: 'La causa raíz es obligatoria' }),
  proposedStrategy: zod.string().min(1, { message: 'La estrategia propuesta es obligatoria' }),
  estimatedResourceCosts: zod.string().min(1, { message: 'Los recursos y costos son obligatorios' }),
  expectedResults: zod.string().min(1, { message: 'Los resultados esperados son obligatorios' }),
  effectivenessIndicators: zod.string().min(1, { message: 'Los indicadores de efectividad son obligatorios' }),
  proposedWorkTeam: zod.string().min(1, { message: 'El equipo de trabajo es obligatorio' }),
  link: zod.string().min(1, { message: 'El enlace es obligatorio' }).url({ message: 'URL inválida' }),
});
