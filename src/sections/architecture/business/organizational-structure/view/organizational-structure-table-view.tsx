...
export function OrganizationalStructureTableView() {
  ...
  const canReset = !!currentFilters.name.trim() || currentFilters.status !== 'all' || flowFilters.type !== null;
  const notFound = !dataFiltered.length && canReset;

  const selectedTypeLabel = useMemo(() => {
    if (!flowFilters.type) return null;
    return orgUnitTypeOptions.find((opt) => opt.value === flowFilters.type)?.label ?? String(flowFilters.type);
  }, [flowFilters.type, orgUnitTypeOptions]);
        ...
        <Card>
          ...
          {canReset && (
            <FiltersResult totalResults={totalItems} onReset={handleResetFilters} sx={{ p: 2.5, pt: 0 }}>
              <FiltersBlock label={`${t('organization.table.filters.keyword')}:`} isShow={currentFilters.name.trim().length > 0}>
                <Chip {...chipProps} label={currentFilters.name} onDelete={() => updateFilters({ name: '' })} />
              </FiltersBlock>

              <FiltersBlock label={`${tCommon('filters.type')}:`} isShow={Boolean(selectedTypeLabel)}>
                <Chip {...chipProps} label={selectedTypeLabel ?? ''} onDelete={() => handleFlowFilters({ type: null })} />
              </FiltersBlock>
            </FiltersResult>
          )}
