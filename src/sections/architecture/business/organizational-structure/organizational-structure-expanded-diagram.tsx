...
export function OrganizationalStructureExpandedDiagram({ organizationalUnitId, nodeId, sx }: Props) {
  ...
  const buildGraph = useCallback(
    (data: OrganizationalUnitMapNode) => {
      ...
      const childNodes: Node[] = (data.children || []).map((child, index) => {
        ...
        return {
          ...
          data: {
            id: String(child.id),
            label: String(child.label || t('organization.map.unnamed')),
            color,
            onClick: () => {
              if (isRelationsChild) {
    ...
    [colors, handleOpenDocumentDrawer, handleOpenRelationDrawer, organizationalUnitId, router, setEdges, setNodes, t]
  );
...
function OrganizationalUnitRelationsDrawer({
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      toast.error(t('organization.relations.error'));
    }
  });
...
function OrganizationalUnitDocumentsDrawer({
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      toast.error(t('organization.documents.error'));
    }
  });
