...
export function OrganizationalStructureExpandedDiagram({ organizationalUnitId, nodeId, sx }: Props) {
  ...
  const isLessonsNodeKey = nodeId === 'lec';
  const isProposalsNodeKey = nodeId === 'prp';
  const isFeedbackLeafNode = isLessonsNodeKey || isProposalsNodeKey;
  ...
  const buildGraph = useCallback(
    (data: OrganizationalUnitMapNode) => {
      ...
      const childNodes: Node[] = (data.children || []).map((child, index) => {
        ...
        const feedbackType: 'lesson' | 'proposal' = isLessonsNodeKey ? 'lesson' : 'proposal';
