...
export function StrategicObjectivesTableRow({
  ...
  const { id, level, hasChildren } = row;
  ...
  const scrollTextColumns = new Set<string>(['description', 'measurementForm', 'consequencesOfNotAchieving']);
  ...
  const renderCellContent = (columnId: string) => {
    switch (columnId) {
      case 'typeName':
        return row.objectiveType?.typeName || '-';
      case 'superiorObjectiveName':
        return row.superiorObjective?.name || '-';
      case 'createdDate':
      case 'lastModifiedDate':
      case 'startDate':
      case 'endDate':
        return row[columnId] ? new Date(row[columnId]).toLocaleDateString() : '-';
      default:
        if (scrollTextColumns.has(columnId)) {
          return renderScrollableText(row[columnId]);
        }
        return row[columnId];
    }
  };
