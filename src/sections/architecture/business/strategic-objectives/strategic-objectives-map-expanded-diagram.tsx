    ...
    children.forEach((child) => {
      ...
      const pickId = (): number | null => {
        if (relationKind === 'process') {
          const value = (data.processId ?? (data.process as { id?: unknown } | undefined)?.id) as unknown;
          const id = Number(value);
          return Number.isFinite(id) ? id : null;
        }
        if (relationKind === 'job') {
          const value = (data.jobId ?? (data.job as { id?: unknown } | undefined)?.id) as unknown;
          const id = Number(value);
          return Number.isFinite(id) ? id : null;
        }
        if (relationKind === 'document') {
          const value = (data.documentId ?? (data.document as { id?: unknown } | undefined)?.id) as unknown;
          const id = Number(value);
          return Number.isFinite(id) ? id : null;
        }
        const value = (data.indicatorId ?? (data.indicator as { id?: unknown } | undefined)?.id) as unknown;
        const id = Number(value);
        return Number.isFinite(id) ? id : null;
      };
