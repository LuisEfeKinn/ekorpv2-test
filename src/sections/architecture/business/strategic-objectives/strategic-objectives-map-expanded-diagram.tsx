...
import { DeleteStrategicObjectiveFeedbackService } from 'src/services/architecture/business/strategic-objectives-feedbacks.service';
...
export function StrategicObjectivesMapExpandedDiagram({ objectiveId, nodeId, sx }: Props) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<{ child: any; type: string } | null>(null);
  ...
  const handleEdit = useCallback((child: any, type: string) => {
    setEditMode(true);
    setEditData(child.data); // Usar child.data en lugar de child directamente
    setEditId(child.id);
    ...
    if (type === 'lesson' || type === 'proposal') {
      ...
      setRelationKind(type as any);
      setRelationOpen(true);
    }
  }, []);

  const confirmDelete = useCallback((child: any, type: string) => {
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      if (type === 'process') {
        ...
        await DeleteStrategicObjectiveFeedbackService(String(child.id));
      }
  ...
  const generateNodesAndEdges = useCallback(
      ...
      const childNodes: Node[] = (data.children || []).map((child: ChildNode, index: number) => {
        ...
        return {
          ...
          data: {
            ...
            onClick: () => {
              // Only navigate if it's not a leaf node (lessons/proposals might be leaf nodes?)
              // The user said: "la ruta para cada uno de los nodos es la misma que la del post añadiendo el id al final, a excepcion de lecciones aprendidas y propuestas de mejora"
              // The navigation logic was: router.push(paths.dashboard.architecture.strategicObjectivesTableMapExpand(objectiveId, childId));
    ...
    [colors, nodeId, objectiveId, router, setEdges, setNodes, handleEdit, confirmDelete]
  );
