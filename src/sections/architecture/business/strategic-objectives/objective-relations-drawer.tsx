...
export function ObjectiveRelationsDrawer({ open, onClose, onSuccess, objectiveId, kind, editMode, initialData, relationId, existingItemIds }: Props) {
  ...
  const loadOptions = useCallback(async () => {
    try {
      ...
      const filtered = !editMode && excludedIds.size > 0 ? mapped.filter((it) => !excludedIds.has(it.id)) : mapped;
      setOptions(filtered);
    } catch {
      setOptions([]);
  ...
  useEffect(() => {
    ...
    if (editMode && initialData) {
      // Intentar encontrar el ID del item relacionado en la data
      const data = initialData.data || initialData;
      
      // La propiedad puede variar dependiendo de cómo viene del backend
      // Para procesos: processId, o process.id
      // Para cargos: jobId, o job.id
      // Para documentos: documentId, o document.id
      // Para indicadores: indicatorId, o indicator.id
      
      let targetId = data.itemId;
      
      if (!targetId) {
        if (kind === 'process') targetId = data.processId || data.process?.id;
        else if (kind === 'job') targetId = data.jobId || data.job?.id;
        else if (kind === 'document') targetId = data.documentId || data.document?.id;
        else if (kind === 'indicator') targetId = data.indicatorId || data.indicator?.id;
      }
      
      // Si aún no se encuentra, intentar buscar por el ID directo del nodo si coincide con la lista de opciones
      if (!targetId && data.id) {
         // Esto es un fallback, asumiendo que el ID del nodo hijo podría ser el ID del elemento relacionado
         targetId = data.id;
      }

      setSelectedId(targetId ? Number(targetId) : null);
    ...
    loadOptions();
  }, [open, loadOptions, editMode, initialData, kind]);
