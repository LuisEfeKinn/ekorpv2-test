    ...
    <Drawer anchor="right" open={open} onClose={onClose} PaperProps={{ sx: { width: 440 } }}>
      <Box sx={{ px: 2, py: 2 }}>
        ...
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          {kind === 'process' && (
            <TextField select label="Impacto" required value={impact} onChange={(e) => setImpact(e.target.value)}>
          ...
          {kind === 'job' && (
            <TextField select label="Rol" required value={role} onChange={(e) => setRole(e.target.value)}>
              <MenuItem value="" disabled>
                Seleccione
              </MenuItem>
          ...
          <Stack direction="row" spacing={1} sx={{ pt: 1 }}>
            <Button variant="outlined" onClick={onClose} disabled={loading}>
              {t('strategicObjectives.relations.actions.cancel', { defaultValue: 'Cancelar' })}
            </Button>
            <Button
              variant="contained"
            ...
            >
              {editMode
                ? t('strategicObjectives.relations.actions.saveChanges', { defaultValue: 'Guardar cambios' })
