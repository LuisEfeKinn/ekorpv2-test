...
const formatUploadErrorDetails = (details: unknown): string[] => {
    ...
    return details
      .map((item) => {
        ...
        if (isRecord(item)) {
          ...
          const field = getRecordString(item, 'field');
          ...
          if (row && field && message) return `Fila ${row} (${field}): ${message}`;
          if (row && message) return `Fila ${row}: ${message}`;
          if (row && field && errorsList.length) return `Fila ${row} (${field}): ${errorsList.join(', ')}`;
  ...
  if (isRecord(details)) {
    ...
    const nestedErrors = getRecordUnknown(details, 'errors') ?? getRecordUnknown(details, 'details');
    if (nestedErrors) return formatUploadErrorDetails(nestedErrors);
  }
...
export function StrategicObjectivesTableView() {
  ...
  const countFlowNodes = useCallback(function count(nodes: IStrategicObjectiveFlowNode[]): number {
  ...
  const tabCount = countFlowNodes(dataFiltered);

  const canReset =
    currentFilters.name.trim().length > 0 ||
    currentFilters.status !== 'all' ||
    flowFilters.type !== null ||
    flowFilters.objectiveLevel !== null ||
    flowFilters.startDate !== null ||
    flowFilters.endDate !== null;

  const selectedTypeLabel = useMemo(() => {
    if (!flowFilters.type) return null;
    return objectiveTypeOptions.find((opt) => opt.value === flowFilters.type)?.label ?? String(flowFilters.type);
  }, [flowFilters.type, objectiveTypeOptions]);
  ...
  return (
    <DashboardContent>
      ...
      <Card>
        ...
        {canReset && (
          <FiltersResult totalResults={tabCount} onReset={handleResetAllFilters} sx={{ p: 2.5, pt: 0 }}>
            <FiltersBlock label={`${t('strategicObjectives.table.filters.keyword', { defaultValue: 'Palabra clave' })}:`} isShow={currentFilters.name.trim().length > 0}>
              <Chip
                {...chipProps}
                label={currentFilters.name}
                onDelete={() => updateFilters({ name: '' })}
              />
            </FiltersBlock>

            <FiltersBlock label={`${t('strategicObjectives.table.filters.type', { defaultValue: 'Tipo' })}:`} isShow={Boolean(selectedTypeLabel)}>
              <Chip
                {...chipProps}
                label={selectedTypeLabel ?? ''}
                onDelete={() => handleFlowFilters({ type: null })}
              />
            </FiltersBlock>

            <FiltersBlock label={`${t('strategicObjectives.table.filters.level', { defaultValue: 'Nivel' })}:`} isShow={flowFilters.objectiveLevel !== null}>
              <Chip
                {...chipProps}
                label={flowFilters.objectiveLevel !== null ? String(flowFilters.objectiveLevel) : ''}
                onDelete={() => handleFlowFilters({ objectiveLevel: null })}
              />
            </FiltersBlock>

            <FiltersBlock label={`${t('strategicObjectives.table.filters.startDate', { defaultValue: 'Fecha de inicio' })}:`} isShow={flowFilters.startDate !== null}>
              <Chip
                {...chipProps}
                label={flowFilters.startDate ? dayjs(flowFilters.startDate).format('YYYY-MM-DD') : ''}
                onDelete={() => handleFlowFilters({ startDate: null })}
              />
            </FiltersBlock>

            <FiltersBlock label={`${t('strategicObjectives.table.filters.endDate', { defaultValue: 'Fecha de fin' })}:`} isShow={flowFilters.endDate !== null}>
              <Chip
                {...chipProps}
                label={flowFilters.endDate ? dayjs(flowFilters.endDate).format('YYYY-MM-DD') : ''}
                onDelete={() => handleFlowFilters({ endDate: null })}
              />
            </FiltersBlock>
          </FiltersResult>
        )}
