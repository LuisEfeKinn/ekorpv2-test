...
import { DeleteJobsService, UploadJobsService, DownloadJobsTemplateService } from 'src/services/architecture/business/jobs.service';
...
export function JobsTableView() {
  ...
  const handleDownloadTemplate = useCallback(
    async (lang: 'es' | 'en') => {
      try {
        const response = await DownloadJobsTemplateService(lang);
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', `jobs_template_${lang}.xlsx`);
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        ...
        action={
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            >
              {t('positions.table.actions.downloadTemplate') || 'Download Template'}
            </Button>
            ...
            >
              {t('positions.table.actions.upload') || 'Upload'}
            </Button>
      ...
      >
        <MenuList>
          <MenuItem onClick={() => handleDownloadTemplate('es')}>
            Español
          </MenuItem>

          <MenuItem onClick={() => handleDownloadTemplate('en')}>
            English
          </MenuItem>
        </MenuList>
      </CustomPopover>
