...
export function JobsTableView() {
  ...
  const selectedJobTypeLabel = useMemo(() => {
    if (!flowFilters.jobTypeId) return null;
    return jobTypeOptions.find((opt) => opt.value === flowFilters.jobTypeId)?.label ?? String(flowFilters.jobTypeId);
  }, [flowFilters.jobTypeId, jobTypeOptions]);

  const selectedActorStatusLabel = useMemo(() => {
    if (flowFilters.actorStatus === 1) return t('positions.table.filters.actorStatusActive');
    if (flowFilters.actorStatus === 2) return t('positions.table.filters.actorStatusInactive');
    return null;
  }, [flowFilters.actorStatus, t]);
  ...
  return (
    <>
      ...
      <Card>
        ...
        {hasAnyFilter && (
          <FiltersResult totalResults={totalItems} onReset={handleResetAllFilters} sx={{ p: 2.5, pt: 0 }}>
            <FiltersBlock
              label={`${t('positions.table.filters.keyword', { defaultValue: 'Palabra clave' })}:`}
              isShow={currentFilters.name.trim().length > 0}
            >
              <Chip {...chipProps} label={currentFilters.name} onDelete={() => updateFilters({ name: '' })} />
            </FiltersBlock>

            <FiltersBlock label={`${t('positions.table.filters.headquarters')}:`} isShow={flowFilters.headquarters.trim().length > 0}>
              <Chip {...chipProps} label={flowFilters.headquarters} onDelete={() => setFlowFilters((prev) => ({ ...prev, headquarters: '' }))} />
            </FiltersBlock>

            <FiltersBlock label={`${t('positions.table.filters.supervises')}:`} isShow={flowFilters.supervises.trim().length > 0}>
              <Chip {...chipProps} label={flowFilters.supervises} onDelete={() => setFlowFilters((prev) => ({ ...prev, supervises: '' }))} />
            </FiltersBlock>

            <FiltersBlock label={`${t('positions.table.filters.jobType')}:`} isShow={Boolean(selectedJobTypeLabel)}>
              <Chip {...chipProps} label={selectedJobTypeLabel ?? ''} onDelete={() => setFlowFilters((prev) => ({ ...prev, jobTypeId: null }))} />
            </FiltersBlock>

            <FiltersBlock label={`${t('positions.table.filters.actorStatus')}:`} isShow={Boolean(selectedActorStatusLabel)}>
              <Chip {...chipProps} label={selectedActorStatusLabel ?? ''} onDelete={() => setFlowFilters((prev) => ({ ...prev, actorStatus: null }))} />
            </FiltersBlock>
          </FiltersResult>
        )}
