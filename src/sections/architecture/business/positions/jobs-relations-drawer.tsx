...
export function JobsRelationsDrawer({ open, onClose, onSuccess, jobId, kind, initialData }: Props) {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      if (kind === 'relation') {
        setSelectedId(initialData.job2Id || null);
        setRelationName(initialData.name || '');
        setReverseName(initialData.reverseName || '');
      } else if (kind === 'nextJob') {
        setSelectedId(initialData.nextJobId || null);
      } else {
        // ... handle other kinds if needed, but current scope is only relations/nextJob for edit
        if (initialData.id) setSelectedId(initialData.id); // Default assumption
      }
    }

    loadOptions();
    loadActionTypes();
  }, [open, loadOptions, loadActionTypes, initialData, kind]);
