  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        ...
        console.log('JobsRelationsDrawer loaded full data:', data);

        if (kind === 'relation') {
          setSelectedId(data.job2Id || data.job2?.id || null);
          setRelationName(data.name || '');
          setReverseName(data.reverseName || '');
        } else if (kind === 'nextJob') {
          setSelectedId(data.nextJobId || data.nextJob?.id || null);
    ...
    loadOptions();
    loadActionTypes();
  }, [open, loadOptions, loadActionTypes, initialData, kind, t]);
