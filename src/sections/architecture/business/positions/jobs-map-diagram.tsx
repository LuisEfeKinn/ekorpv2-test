...
import { GetJobsMapByIdService } from 'src/services/architecture/business/jobs.service';

import { toast } from 'src/components/snackbar';
import { Iconify } from 'src/components/iconify';

import { JobsMapExpandedDiagram } from './jobs-map-expanded-diagram';
...
export function JobsMapDiagram({ jobId, sx }: JobsMapDiagramProps) {
  const { t } = useTranslate('business');
  const theme = useTheme();
              ...
              setPath((prev) => {
                if (!prev.length) {
                  return [
                    ...
                    { id: String(child.id), label: child.label },
                  ];
                }
                return [...prev, { id: String(child.id), label: child.label }];
              });
            },
          },
          draggable: true,
  ...
  useEffect(() => {
    const fetchMapData = async () => {
      try {
        setLoading(true);
        const response = await GetJobsMapByIdService(jobId);
        const data = response.data;
        setMapData(data);
        setPath([{ id: String(data.id), label: data.label }]);
        generateNodesAndEdges(data);
      } catch {
        toast.error(t('positions.errors.loadMap'));
      } finally {
        setLoading(false);
      }
    };

    if (jobId) fetchMapData();
  }, [t, jobId, generateNodesAndEdges]);

  const expandedNode: ExpandedNode = path.length > 1
    ? { id: path[path.length - 1].id, label: path[path.length - 1].label }
    : null;

  if (expandedNode) {
    return (
      <JobsMapExpandedDiagram
        jobId={jobId}
        nodeId={expandedNode.id}
        nodeLabel={expandedNode.label}
        parentLabel={path.length > 1 ? path[path.length - 2].label : ''}
        onBack={() => {
          setPath((prev) => {
        ...
        onNavigateToChild={(child: ChildNode) => {
          setPath((prev) => [...prev, { id: String(child.id), label: child.label }]);
        }}
        sx={sx}
      />
    );
          ...
          <Paper elevation={2} sx={{ px: { xs: 1.5, sm: 2 }, py: { xs: 1, sm: 1.25 }, borderRadius: 1.5, bgcolor: alpha(theme.palette.background.paper, 0.9), backdropFilter: 'blur(8px)', border: `1px solid ${alpha(theme.palette.divider, 0.12)}` }}>
            <Stack direction="row" spacing={1} alignItems="center">
              ...
              <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', fontSize: { xs: '0.7rem', sm: '0.75rem' } }}>
                {(mapData?.children || []).length} {t('positions.map.modulesConnected')}
              </Typography>
            </Stack>
          </Paper>
        </Box>
