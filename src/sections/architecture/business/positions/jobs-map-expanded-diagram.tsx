...
function ChildNode({ data }: any) {
  const theme = useTheme();
  const { color, label, id, onEdit, onDelete, actionTypeNomenclature, isProcessNode } = data;
    ...
    >
      ...
      <Box onClick={onEdit} sx={{ pt: 3 }}>
        <Stack spacing={1.5} alignItems="center">
          ...
          <Chip
            label={isProcessNode ? (actionTypeNomenclature || '') : id}
            size="small"
            sx={{
          ...
          {isProcessNode && (
            <>
              ...
              <Button
                variant="soft"
                color="primary"
                startIcon={<Iconify icon={'solar:upload-minimalistic-bold' as any} />}
