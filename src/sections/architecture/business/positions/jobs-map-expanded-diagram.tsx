  ...
  const existingItemIds = useMemo(() => {
    ...
    children.forEach((child) => {
      ...
      if (relationKind === 'process') {
        const id = pickNumber(rec.processId ?? (rec.process as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'document') {
        const id = pickNumber(rec.documentId ?? (rec.document as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'system') {
        const id = pickNumber(rec.systemId ?? (rec.system as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'indicator') {
        const id = pickNumber(rec.indicatorId ?? (rec.indicator as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'data') {
        const id = pickNumber(rec.dataId ?? (rec.data as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'technology') {
        const id = pickNumber(rec.technologyId ?? (rec.technology as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'relation') {
        const id = pickNumber(rec.job2Id ?? (rec.job2 as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
        return;
      }
      if (relationKind === 'nextJob') {
        const id = pickNumber(rec.nextJobId ?? (rec.nextJob as { id?: unknown } | undefined)?.id);
        if (id != null) ids.add(id);
      }
    });
