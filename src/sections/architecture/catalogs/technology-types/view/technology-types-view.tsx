...
import {
  DeleteTechnologyTypeService,
  GetTechnologyTypesPaginationService
} from 'src/services/architecture/catalogs/technologyTypes.service';
...
export function TechnologyTypesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('technology-types.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('technology-types.columns.name') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetTechnologyTypesPaginationService(params);
      ...
      toast.error(t('technology-types.messages.error.loading'));
      setTableData([]);
      setTotalItems(0);
    }
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteTechnologyTypeService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('technology-types.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('technology-types.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteTechnologyTypeService(id));
      await Promise.all(deletePromises);

      toast.success(t('technology-types.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('technology-types.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('technology-types.dialogs.delete.title')}
      content={
        <>
          {t('technology-types.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('technology-types.title')}
          links={[
            { name: t('technology-types.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('technology-types.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('technology-types.title') },
          ]}
          action={
            <Button
            ...
            >
              {t('technology-types.actions.add')}
            </Button>
          }
          sx={{ mb: { xs: 3, md: 5 } }}
        />
        ...
        <Card>
          ...
          {canReset && (
            <TechnologyTypesTableFiltersResult
              filters={currentFilters}
