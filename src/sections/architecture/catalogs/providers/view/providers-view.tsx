...
export function ProvidersView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('providers.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('providers.columns.name') },
    { id: 'contact', label: t('providers.columns.contact') },
    { id: 'supportName', label: t('providers.columns.supportName') },
    { id: 'supportEmail', label: t('providers.columns.supportEmail') },
    { id: 'supportPhone', label: t('providers.columns.supportPhone') },
    { id: 'contractualName', label: t('providers.columns.contractualName') },
    { id: 'contractualEmail', label: t('providers.columns.contractualEmail') },
    { id: 'contractualPhone', label: t('providers.columns.contractualPhone') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetProvidersPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteProviderService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('providers.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('providers.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteProviderService(id));
      await Promise.all(deletePromises);

      toast.success(t('providers.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('providers.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('providers.dialogs.delete.title')}
      content={
        <>
          {t('providers.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
    ...
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          ...
          links={[
            { name: t('providers.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('providers.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('providers.title') },
          ]}
          action={
            <Button
