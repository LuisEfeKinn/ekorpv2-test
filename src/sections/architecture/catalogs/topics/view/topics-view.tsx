...
export function TopicsView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('topics.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('topics.columns.name') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetTopicsPaginationService(params);
    ...
    async (id: string) => {
      try {
        ...
        if (response.data.statusCode === 200) {
          toast.success(t('topics.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('topics.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteTopicService(id));
      await Promise.all(deletePromises);

      toast.success(t('topics.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('topics.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('topics.dialogs.delete.title')}
      content={
        <>
          {t('topics.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('topics.title')}
          links={[
            { name: t('topics.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('topics.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('topics.title') },
          ]}
          action={
            <Button
