...
export function TopicsTableModal({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    if (!isFormValid()) {
      toast.error(t('topics.messages.error.validation'));
      return;
    }
    ...
    try {
      ...
      toast.success(
        dataId
          ? t('topics.messages.success.updated')
          : t('topics.messages.success.created')
      );
      ...
      toast.error(
        dataId
          ? (error?.message[0] || t('topics.messages.error.updating'))
          : (error?.message[0] || t('topics.messages.error.creating'))
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    >
      <DialogTitle>
        {dataId
          ? t('topics.dialogs.edit.title')
          : t('topics.dialogs.create.title')}
      </DialogTitle>
      ...
      <DialogActions>
        ...
        >
          {dataId
            ? t('topics.actions.update')
            : t('topics.actions.save')}
        </LoadingButton>
      </DialogActions>
    </Dialog>
  );
