...
import {
  DeleteDateControlService,
  GetDateControlsPaginationService
...
export function DateControlsView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('date-controls.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('date-controls.columns.name') },
    { id: 'alertDays', label: t('date-controls.columns.alertDays') },
    { id: 'color', label: t('date-controls.columns.color') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetDateControlsPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteDateControlService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('date-controls.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('date-controls.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteDateControlService(id));
      await Promise.all(deletePromises);

      toast.success(t('date-controls.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('date-controls.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('date-controls.dialogs.delete.title')}
      content={
        <>
          {t('date-controls.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('date-controls.title')}
          links={[
            { name: t('date-controls.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('date-controls.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('date-controls.title') },
          ]}
          action={
            <Button
        ...
        <Card>
          ...
          {canReset && (
            <DateControlsTableFiltersResult
              filters={currentFilters}
