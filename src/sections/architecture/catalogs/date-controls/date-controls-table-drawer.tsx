...
type FormData = { name: string; alertDays: string; color: string };

// ----------------------------------------------------------------------

export function DateControlsTableDrawer({ open, onClose, dataId, onSave, ...other }: Props) {
  const { t } = useTranslate('catalogs');

  const colorInputRef = useRef<HTMLInputElement>(null);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState<FormData>({ name: '', alertDays: '', color: '' });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (response?.data?.data) {
        ...
        setFormData({ name: data.name || '', alertDays: data.alertDays || '', color: data.color || '' });
      }
    } catch {
  ...
  useEffect(() => {
    ...
    setFormData({ name: '', alertDays: '', color: '' });
  }, [open, dataId, loadData]);

  const isFormValid = formData.name.trim() !== '' && formData.color.trim() !== '';

  const handleSave = async () => {
    if (!isFormValid) { toast.error(t('date-controls.messages.error.validation')); return; }
    setSaving(true);
    try {
      await SaveOrUpdateDateControlService(formData, dataId);
      toast.success(dataId ? t('date-controls.messages.success.updated') : t('date-controls.messages.success.created'));
      onSave();
      onClose();
    } catch (error: any) {
      toast.error(dataId ? (error?.message?.[0] || t('date-controls.messages.error.updating')) : (error?.message?.[0] || t('date-controls.messages.error.creating')));
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
          <Typography variant="h6">
            {dataId ? t('date-controls.dialogs.edit.title') : t('date-controls.dialogs.create.title')}
          </Typography>
        </Box>
        ...
        <Box sx={{ pt: 3, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
          <Button onClick={handleClose} disabled={saving}>{t('date-controls.actions.cancel')}</Button>
          <LoadingButton variant="contained" onClick={handleSave} loading={saving} disabled={loading || !isFormValid}>
            {dataId ? t('date-controls.actions.update') : t('date-controls.actions.save')}
          </LoadingButton>
        </Box>
      </Box>
    </Drawer>
