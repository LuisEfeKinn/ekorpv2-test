...
export function DomainsView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('domains.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('domains.columns.name') },
    { id: 'code', label: t('domains.columns.code') },
    { id: 'color', label: t('domains.columns.color') },
    { id: 'corporateScope', label: t('domains.columns.corporateScope') },
    { id: 'owner', label: t('domains.columns.owner') },
    { id: 'admin', label: t('domains.columns.admin') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetDomainPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteDomainService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('domains.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('domains.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteDomainService(id));
      await Promise.all(deletePromises);

      toast.success(t('domains.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('domains.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      content={
        <>
          {t('domains.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('domains.title')}
          links={[
            { name: t('domains.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('domains.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
