...
import {
  DeleteMeasureActionTypeService,
  GetMeasureActionTypesPaginationService
...
export function MeasureActionTypesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('measure-action-types.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('measure-action-types.columns.name') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetMeasureActionTypesPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteMeasureActionTypeService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('measure-action-types.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('measure-action-types.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteMeasureActionTypeService(id));
      await Promise.all(deletePromises);

      toast.success(t('measure-action-types.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('measure-action-types.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('measure-action-types.dialogs.delete.title')}
      content={
        <>
          {t('measure-action-types.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('measure-action-types.title')}
          links={[
            { name: t('measure-action-types.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('measure-action-types.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('measure-action-types.title') },
          ]}
          action={
            <Button
            ...
            >
              {t('measure-action-types.actions.add')}
            </Button>
          }
          sx={{ mb: { xs: 3, md: 5 } }}
        />
        ...
        <Card>
          ...
          <MeasureActionTypesTableToolbar
            filters={currentFilters}
