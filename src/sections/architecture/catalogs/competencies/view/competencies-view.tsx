...
import {
  DeleteCompetenciesService,
  GetCompetenciesPaginationService
...
export function CompetenciesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('competencies.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('competencies.columns.name') },
    { id: 'description', label: t('competencies.columns.description'), align: 'left' },
    { id: 'code', label: t('competencies.columns.code'), align: 'right' },
    { id: 'type', label: t('competencies.columns.type'), align: 'right' },
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteCompetenciesService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('competencies.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('competencies.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteCompetenciesService(id));
      await Promise.all(deletePromises);

      toast.success(t('competencies.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('competencies.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('competencies.dialogs.delete.title')}
      content={
        <>
          {t('competencies.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('competencies.title')}
          links={[
            { name: t('competencies.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('competencies.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('competencies.title') },
          ]}
          action={
            <Button
        ...
        <Card>
          ...
          {canReset && (
            <CompetenciesTableFiltersResult
              filters={currentFilters}
