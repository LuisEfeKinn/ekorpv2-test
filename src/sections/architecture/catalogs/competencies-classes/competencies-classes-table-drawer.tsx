...
export function CompetenciesClassesTableDrawer({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    if (!isFormValid) { toast.error(t('competencies-classes.messages.error.validation')); return; }
    setSaving(true);
    try {
      await SaveOrUpdateCompetenciesClassesService(formData, dataId);
      toast.success(dataId ? t('competencies-classes.messages.success.updated') : t('competencies-classes.messages.success.created'));
      onSave();
      onClose();
    } catch (error: any) {
      toast.error(dataId ? (error?.message?.[0] || t('competencies-classes.messages.error.updating')) : (error?.message?.[0] || t('competencies-classes.messages.error.creating')));
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
          <Typography variant="h6">
            {dataId ? t('competencies-classes.dialogs.edit.title') : t('competencies-classes.dialogs.create.title')}
          </Typography>
        </Box>
              ...
              <Box>
                <Box sx={{ mb: 1 }}>
                  <span style={{ fontSize: '0.875rem', color: '#666' }}>
                    {t('competencies-classes.columns.color')}
                  </span>
                </Box>
        ...
        <Box sx={{ pt: 3, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
          <Button onClick={handleClose} disabled={saving}>{t('competencies-classes.actions.cancel')}</Button>
          <LoadingButton variant="contained" onClick={handleSave} loading={saving} disabled={loading || !isFormValid}>
            {dataId ? t('competencies-classes.actions.update') : t('competencies-classes.actions.save')}
          </LoadingButton>
        </Box>
      </Box>
    </Drawer>
