...
export function OrganizationalUnitTypesTableModal({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(
        dataId
          ? t('organizational-unit-types.messages.error.updating')
          : t('organizational-unit-types.messages.error.creating')
      );
    } finally {
      setSaving(false);
    }
          ...
          <Stack spacing={3} sx={{ pt: 1 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
              ...
              <Box>
                <TextField
                  fullWidth
                  label={t('organizational-unit-types.columns.background')}
