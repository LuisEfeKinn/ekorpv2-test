...
import {
  GetSystemTypeByIdService,
  SaveOrUpdateSystemTypeService,
...
export function SystemTypesTableDrawer({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    if (!isFormValid) { toast.error(t('system-types.messages.error.validation')); return; }
    setSaving(true);
    try {
      await SaveOrUpdateSystemTypeService(formData, dataId);
      toast.success(dataId ? t('system-types.messages.success.updated') : t('system-types.messages.success.created'));
      onSave();
      onClose();
    } catch (error: any) {
      toast.error(dataId ? (error?.message?.[0] || t('system-types.messages.error.updating')) : (error?.message?.[0] || t('system-types.messages.error.creating')));
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
          <Typography variant="h6">
            {dataId ? t('system-types.dialogs.edit.title') : t('system-types.dialogs.create.title')}
          </Typography>
        </Box>
        ...
        <Box sx={{ pt: 3, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
          <Button onClick={handleClose} disabled={saving}>{t('system-types.actions.cancel')}</Button>
          <LoadingButton variant="contained" onClick={handleSave} loading={saving} disabled={loading || !isFormValid}>
            {dataId ? t('system-types.actions.update') : t('system-types.actions.save')}
          </LoadingButton>
        </Box>
      </Box>
    </Drawer>
