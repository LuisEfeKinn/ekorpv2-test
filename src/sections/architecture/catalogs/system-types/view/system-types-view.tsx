...
import {
  DeleteSystemTypeService,
  GetSystemTypesPaginationService
} from 'src/services/architecture/catalogs/systemTypes.service';
...
export function SystemTypesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('system-types.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('system-types.columns.name') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetSystemTypesPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteSystemTypeService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('system-types.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('system-types.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteSystemTypeService(id));
      await Promise.all(deletePromises);

      toast.success(t('system-types.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('system-types.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('system-types.dialogs.delete.title')}
      content={
        <>
          {t('system-types.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('system-types.title')}
          links={[
            { name: t('system-types.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('system-types.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('system-types.title') },
          ]}
          action={
            <Button
