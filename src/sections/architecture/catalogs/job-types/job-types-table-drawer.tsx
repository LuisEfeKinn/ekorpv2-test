...
type FormData = { name: string; background: string; internalExternal: boolean; user: boolean; letter: string };
...
export function JobTypesTableDrawer({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const [formData, setFormData] = useState<FormData>({ name: '', background: '', internalExternal: false, user: false, letter: '' });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (response?.data?.data) {
        ...
        setFormData({ name: data.name || '', background: data.background || '', internalExternal: data.internalExternal || false, user: data.user || false, letter: data.letter || '' });
      }
    } catch {
  ...
  useEffect(() => {
    ...
    setFormData({ name: '', background: '', internalExternal: false, user: false, letter: '' });
