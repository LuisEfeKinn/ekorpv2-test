...
export function JobTypesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('job-types.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('job-types.columns.name') },
    { id: 'background', label: t('job-types.columns.background'), align: 'left' },
    { id: 'internalExternal', label: t('job-types.columns.internalExternal'), align: 'center' },
    { id: 'user', label: t('job-types.columns.user'), align: 'center' },
    { id: 'letter', label: t('job-types.columns.letter'), align: 'center' },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetJobTypesPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteJobTypeService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('job-types.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('job-types.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteJobTypeService(id));
      await Promise.all(deletePromises);

      toast.success(t('job-types.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('job-types.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('job-types.dialogs.delete.title')}
      content={
        <>
          {t('job-types.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('job-types.title')}
          links={[
            { name: t('job-types.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('job-types.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('job-types.title') },
          ]}
          action={
            <Button
