...
import {
  DeleteProcessTypeService,
  GetProcessTypesPaginationService
...
export function ProcessTypesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('process-types.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('process-types.columns.name') },
    { id: 'level', label: t('process-types.columns.level') }
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetProcessTypesPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteProcessTypeService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('process-types.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('process-types.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteProcessTypeService(id));
      await Promise.all(deletePromises);

      toast.success(t('process-types.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('process-types.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('process-types.dialogs.delete.title')}
      content={
        <>
          {t('process-types.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('process-types.title')}
          links={[
            { name: t('process-types.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('process-types.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('process-types.title') },
          ]}
          action={
            <Button
        ...
        <Card>
          ...
          {canReset && (
            <ProcessTypesTableFiltersResult
              filters={currentFilters}
