...
export function DataTypesTableModal({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    if (!isFormValid()) {
      toast.error(t('data-types.messages.error.validation'));
      return;
    }
    ...
    try {
      ...
      toast.success(
        dataId
          ? t('data-types.messages.success.updated')
          : t('data-types.messages.success.created')
      );
      ...
      toast.error(
        dataId
          ? (error?.message[0] || t('data-types.messages.error.updating'))
          : (error?.message[0] || t('data-types.messages.error.creating'))
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    >
      <DialogTitle>
        {dataId
          ? t('data-types.dialogs.edit.title')
          : t('data-types.dialogs.create.title')}
      </DialogTitle>
      ...
      <DialogActions>
        ...
        >
          {dataId
            ? t('data-types.actions.update')
            : t('data-types.actions.save')}
        </LoadingButton>
      </DialogActions>
    </Dialog>
  );
