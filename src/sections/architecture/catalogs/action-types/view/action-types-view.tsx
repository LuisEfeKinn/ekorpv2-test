...
import {
  DeleteActionTypeService,
  GetActionTypesPaginationService
...
export function ActionTypesView() {
  ...
  const STATUS_OPTIONS = useMemo(() => [
    { value: 'all', label: t('action-types.filters.all') },
  ], [t]);

  const TABLE_HEAD: TableHeadCellProps[] = useMemo(() => [
    { id: '', width: 88 },
    { id: 'name', label: t('action-types.columns.name') },
    { id: 'nomenclature', label: t('action-types.columns.nomenclature') },
    { id: 'color', label: t('action-types.columns.color') },
  ], [t]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetActionTypesPaginationService(params);
  ...
  const handleDeleteRow = useCallback(
    async (id: string) => {
      try {
        const response = await DeleteActionTypeService(id);

        if (response.data.statusCode === 200) {
          toast.success(t('action-types.messages.success.deleted'));
          loadData(); // Recargar datos
        }
      } catch (error) {
        console.error('Error deleting data:', error);
        toast.error(t('action-types.messages.error.deleting'));
      }
    },
    [loadData, t]
  );
  ...
  const handleDeleteRows = useCallback(async () => {
    try {
      ...
      const deletePromises = table.selected.map(id => DeleteActionTypeService(id));
      await Promise.all(deletePromises);

      toast.success(t('action-types.messages.success.deletedMultiple'));
      table.setSelected([]);
      loadData(); // Recargar datos
    } catch (error) {
      console.error('Error deleting data:', error);
      toast.error(t('action-types.messages.error.deletingMultiple'));
    }
  }, [table, loadData, t]);
  ...
  const renderConfirmDialog = () => (
    <ConfirmDialog
      ...
      title={t('action-types.dialogs.delete.title')}
      content={
        <>
          {t('action-types.dialogs.delete.contentMultiple', { count: table.selected.length })}
        </>
      }
      action={
        <Button
  ...
  return (
    <>
      <DashboardContent>
        <CustomBreadcrumbs
          heading={t('action-types.title')}
          links={[
            { name: t('action-types.breadcrumbs.dashboard'), href: paths.dashboard.root },
            { name: t('action-types.breadcrumbs.catalogs'), href: paths.dashboard.architecture.catalogs.root },
            { name: t('action-types.title') },
          ]}
          action={
            <Button
