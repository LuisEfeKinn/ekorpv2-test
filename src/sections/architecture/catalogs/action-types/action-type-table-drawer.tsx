...
type FormData = { name: string; nomenclature: string; color: string };

// ----------------------------------------------------------------------

export function ActionTypeTableDrawer({ open, onClose, dataId, onSave, ...other }: Props) {
  const { t } = useTranslate('catalogs');

  const colorInputRef = useRef<HTMLInputElement>(null);
  const [loading, setLoading] = useState(false);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState<FormData>({ name: '', nomenclature: '', color: '' });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (response?.data?.data) {
        ...
        setFormData({ name: data.name || '', nomenclature: data.nomenclature || '', color: data.color || '' });
      }
    } catch {
  ...
  useEffect(() => {
    ...
    setFormData({ name: '', nomenclature: '', color: '' });
  }, [open, dataId, loadData]);

  const isFormValid = formData.name.trim() !== '' && formData.nomenclature.trim() !== '' && formData.color.trim() !== '';

  const handleSave = async () => {
    if (!isFormValid) { toast.error(t('action-types.messages.error.validation')); return; }
    setSaving(true);
    try {
      await SaveOrUpdateActionTypeService(formData, dataId);
      toast.success(dataId ? t('action-types.messages.success.updated') : t('action-types.messages.success.created'));
      onSave();
      onClose();
    } catch (error: any) {
      toast.error(dataId ? (error?.message?.[0] || t('action-types.messages.error.updating')) : (error?.message?.[0] || t('action-types.messages.error.creating')));
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box sx={{ p: 3, height: '100%', display: 'flex', flexDirection: 'column' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
          <Typography variant="h6">
            {dataId ? t('action-types.dialogs.edit.title') : t('action-types.dialogs.create.title')}
          </Typography>
        </Box>
        ...
        <Box sx={{ pt: 3, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
          <Button onClick={handleClose} disabled={saving}>{t('action-types.actions.cancel')}</Button>
          <LoadingButton variant="contained" onClick={handleSave} loading={saving} disabled={loading || !isFormValid}>
            {dataId ? t('action-types.actions.update') : t('action-types.actions.save')}
          </LoadingButton>
        </Box>
      </Box>
    </Drawer>
