...
export function RiskTypesTableModal({ open, onClose, dataId, onSave, ...other }: Props) {
  ...
  const handleSave = async () => {
    if (!isFormValid()) {
      toast.error(t('risk-types.messages.error.validation'));
      return;
    }
    ...
    try {
      ...
      toast.success(
        dataId
          ? t('risk-types.messages.success.updated')
          : t('risk-types.messages.success.created')
      );
      ...
      toast.error(
        dataId
          ? (error?.message[0] || t('risk-types.messages.error.updating'))
          : (error?.message[0] || t('risk-types.messages.error.creating'))
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    ...
    >
      <DialogTitle>
        {dataId
          ? t('risk-types.dialogs.edit.title')
          : t('risk-types.dialogs.create.title')}
      </DialogTitle>
      ...
      <DialogActions>
        ...
        >
          {dataId
            ? t('risk-types.actions.update')
            : t('risk-types.actions.save')}
        </LoadingButton>
      </DialogActions>
    </Dialog>
  );
