...
import {
  ...
  DeleteRiskToleranceLevelService,
...
export function RiskToleranceLevelsView() {
  ...
  const [list, setList] = useState<RiskToleranceLevelRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [confirmOpen, setConfirmOpen] = useState(false);
  const [rowToDelete, setRowToDelete] = useState<RiskToleranceLevelRow | null>(null);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [riskTypeName, setRiskTypeName] = useState<string>('');
  const [form, setForm] = useState<FormState>({ levelName: '', toleranceColor: '#43a047', initialRange: '', finalRange: '' });
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      const normalized: RiskToleranceLevelRow[] = (listArr || []).map((item: any, idx: number) => ({
  ...
  const openDelete = (row: RiskToleranceLevelRow) => {
    setRowToDelete(row);
    setConfirmOpen(true);
  };
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await DeleteRiskToleranceLevelService(rowToDelete.id);
      toast.success(tf('risk-types.tolerance.messages.deleted', 'Level deleted', 'Nivel eliminado'));
      forceCloseDelete();
      loadList();
    } catch {
      toast.error(tf('risk-types.tolerance.messages.errorDeleting', 'Error deleting level', 'Error al eliminar nivel'));
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleSave = async () => {
    if (!riskTypeId) return;
    if (!form.levelName) {
      toast.error(tf('risk-types.tolerance.validation', 'Name is required', 'El nombre es requerido'));
      return;
    }
    if (String(form.initialRange).trim() === '' || String(form.finalRange).trim() === '') {
      toast.error(tf('risk-types.tolerance.validationRange', 'Initial and Final are required', 'Inicial y Final son requeridos'));
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <DashboardContent>
                  ...
                  <TableCell align="center">
                    <Stack direction="row" spacing={1} justifyContent="center">
                      <IconButton
                        size="small"
                        color="primary"
                        aria-label={tf('risk-types.actions.edit', 'Edit', 'Editar')}
                      ...
                      <IconButton
                        size="small"
                        color="error"
                        aria-label={tf('risk-types.actions.delete', 'Delete', 'Eliminar')}
      ...
      <Dialog open={confirmOpen} onClose={closeDelete} fullWidth maxWidth="xs">
        <DialogTitle>
          {tf('risk-types.tolerance.dialogs.delete.title', 'Delete level', 'Eliminar nivel')}
        </DialogTitle>
        <DialogContent dividers>
          <Stack spacing={1}>
            <Typography variant="body2">
              {tf(
                'risk-types.tolerance.dialogs.delete.message',
                'Are you sure you want to delete this level?',
                '¿Estás seguro de que deseas eliminar este nivel?'
              )}
            </Typography>
            <Typography variant="body2" sx={{ color: 'text.secondary' }}>
              {rowToDelete?.levelName ?? ''}
            </Typography>
            <Typography variant="caption" sx={{ color: 'text.secondary' }}>
              {tf(
                'risk-types.tolerance.dialogs.delete.content',
        ...
        <DialogActions>
          ...
          <LoadingButton onClick={handleDelete} loading={deleting} color="error" variant="contained">
            {tf('risk-types.actions.delete', 'Delete', 'Eliminar')}
          </LoadingButton>
        </DialogActions>
      </Dialog>
    </DashboardContent>
