...
import {
  ...
  DeleteRiskProbabilityLevelService,
  SaveOrUpdateRiskProbabilityLevelService
...
export function RiskProbabilityLevelsView() {
  ...
  const loadList = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await GetRiskProbabilityLevelsService({ risktype: riskTypeId });
      const raw = (res as any)?.data;
      const listArr: any[] = Array.isArray(raw) ? (Array.isArray(raw[0]) ? raw[0] : raw) : [];
      const normalized = (listArr || []).map((item: any) => ({
        id: Number(item?.id),
        name: String(item?.probabilityName ?? ''),
        description: String(item?.description ?? ''),
        value: Number(item?.probabilityValue ?? 0),
      }));
      setList(normalized);
    } catch {
      setList([]);
  ...
  const handleDelete = async (id: number) => {
    try {
      await DeleteRiskProbabilityLevelService(id);
      toast.success(tf('risk-types.levels.probability.deleted', 'Probability level deleted'));
      loadList();
    } catch {
      toast.error(tf('risk-types.levels.probability.deleteError', 'Error deleting probability level'));
    }
  };
  const handleSave = async () => {
    if (!riskTypeId) return;
    if (!form.name) {
      toast.error(tf('risk-types.levels.probability.validation', 'Name is required'));
      return;
    }
    if (String(form.value).trim() === '') {
      toast.error(tf('risk-types.levels.probability.valueValidation', 'Value is required'));
      return;
    }
    setSaving(true);
    try {
      ...
      const payload = {
        probabilityName: form.name,
        description: form.description,
        probabilityValue: isNaN(parsedValue) ? 0 : parsedValue,
        riskType: { id: Number(riskTypeId) },
      };
      await SaveOrUpdateRiskProbabilityLevelService(payload, editingId ?? undefined);
      toast.success(editingId
        ? tf('risk-types.levels.probability.updated', 'Probability level updated')
        : tf('risk-types.levels.probability.created', 'Probability level created'));
      setDialogOpen(false);
      setEditingId(null);
      setForm({ name: '', description: '', value: '' });
      loadList();
      ...
      toast.error(editingId
        ? tf('risk-types.levels.probability.updateError', 'Error updating probability level')
        : tf('risk-types.levels.probability.createError', 'Error creating probability level'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <DashboardContent>
      <CustomBreadcrumbs
        heading={`${tf('risk-types.levels.probability.panelTitle', 'Probability Levels', 'Niveles de Probabilidad')} (${count})`}
        links={[
        ...
        action={
          <>
            ...
            <Button variant="contained" startIcon={<Iconify icon="mingcute:add-line" />} onClick={openCreate}>
              {t('risk-types.levels.probability.add')}
            </Button>
          </>
        }
        sx={{ mb: { xs: 3, md: 5 } }}
      ...
      <Card sx={{ p: 2 }}>
        {loading ? (
          ...
          <Table sx={{ borderCollapse: 'collapse' }}>
            <TableHead>
              <TableRow>
                <TableCell>{t('risk-types.levels.probability.columns.name')}</TableCell>
                <TableCell>{t('risk-types.levels.probability.columns.description')}</TableCell>
                <TableCell align="center" sx={{ width: 160 }}>{t('risk-types.levels.probability.columns.value')}</TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editingId ? tf('risk-types.levels.probability.editTitle', 'Edit Probability Level', 'Editar nivel de probabilidad') : tf('risk-types.levels.probability.addTitle', 'Add Probability Level', 'Añadir nivel de probabilidad')}</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2} sx={{ mt: 1 }}>
            <TextField
              label={t('risk-types.levels.probability.fields.name')}
              value={form.name}
            ...
            />
            <TextField
              label={t('risk-types.levels.probability.fields.description')}
            ...
            />
            <TextField
              label={t('risk-types.levels.probability.fields.value')}
              type="text"
              value={form.value}
...
export default RiskProbabilityLevelsView;
