...
import { DocumentManagementMapDiagram } from '../document-management-map-diagram';

type Props = { id: string };

export function DocumentManagementMapView({ id, sx, ...other }: Props & { sx?: SxProps<Theme> }) {
    ...
    >
      <CustomBreadcrumbs
        ...
        links={[
          ...
          { name: t('documentManagement.map.title') },
        ]}
        action={
          <Button
            variant="outlined"
          ...
          >
            {t('documentManagement.map.actions.back')}
          </Button>
        }
        sx={{ mb: 3 }}
      />
      ...
      <Box sx={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', pb: { xs: 8, sm: 10, md: 12 } }}>
        <DocumentManagementMapDiagram documentId={id} />
      </Box>
    </Container>
  );
}
