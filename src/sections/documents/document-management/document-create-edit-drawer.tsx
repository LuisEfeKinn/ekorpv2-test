...
export function DocumentCreateEditDrawer({
  ...
  const initialFormRef = useRef<DocumentUpsertFormState | null>(null);
  const initialExistingFileNameRef = useRef<string>('');
  ...
  useEffect(() => {
    ...
    if (editRow) {
      const initialValues: DocumentUpsertFormState = {
        code: editRow.code || '',
        name: editRow.name || '',
      ...
      const initialFileName = editRow.originalFile || editRow.file || '';
      setForm(initialValues);
      initialFormRef.current = initialValues;
      initialExistingFileNameRef.current = initialFileName;
      setExistingFileName(initialFileName);
      setFile(null);
      setSubmitted(false);
  ...
  const hasChanges = useMemo(() => {
    ...
    const formChanged = (Object.keys(form) as Array<keyof DocumentUpsertFormState>).some(
