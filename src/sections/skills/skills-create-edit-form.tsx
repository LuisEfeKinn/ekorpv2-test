...
export function SkillsCreateEditForm({ currentSkills }: Props) {
  ...
  const SkillsCreateSchema = z.object({
    ...
    code: z
      .string()
      .min(1, { message: t('skills.form.code.required') })
      .max(10, { message: t('skills.form.code.maxLength') }),
    color: z.string().optional(),
    expectedLevel: z.coerce.number().optional(),
    typeId: z.string().min(1, { message: t('skills.form.typeId.required') }),
  });

  const defaultValues = useMemo(
    () => ({
      name: currentSkills?.name ?? '',
      description: currentSkills?.description ?? '',
      code: currentSkills?.code ?? '',
      color: currentSkills?.color ?? '#6177d6ff',
      expectedLevel: currentSkills?.expectedLevel ?? undefined,
      typeId: currentSkills?.typeId ? String(currentSkills.typeId) : '',
    }),
    [currentSkills]
  );
