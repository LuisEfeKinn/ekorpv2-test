...
export function LearningPathCard({ row, assignment, onViewDetails, sx, ...other }: Props) {
  ...
  // Validar datos de manera segura con nueva estructura anidada
  const safeData = {
    id: row?.id || '',
    ...
    // Nueva estructura: contar cursos dentro de learningObjects.courses
    coursesCount: row?.modules?.reduce((acc, module) => {
      const moduleCourses = module.learningObjects?.reduce((sum, obj) => 
        sum + (obj.courses?.length || 0), 0) || 0;
      return acc + moduleCourses;
    }, 0) || 0,
    // Calcular duración desde courses.duration (formato "X hrs")
    totalDuration: row?.modules?.reduce((acc, module) => {
      const moduleDuration = module.learningObjects?.reduce((objSum, obj) => {
        const objectDuration = obj.courses?.reduce((courseSum, course) => {
          if (course.duration) {
            const match = course.duration.match(/(\d+)/);
            return courseSum + (match ? parseInt(match[1], 10) : 0);
          }
          return courseSum;
        }, 0) || 0;
        return objSum + objectDuration;
      }, 0) || 0;
