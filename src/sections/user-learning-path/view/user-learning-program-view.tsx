...
// import { GetCoursesByIdService } from "src/services/learning/courses.service"; Cambio de api, ahora se obtiene el learning object por id
...
import { UserLearningProgramDetailsCard } from "../user-learning-program-details-card";
...
export function UserLearningProgramView({ id, idP }: Props) {
  ...
  const loadLearningObject = useCallback(async () => {
    ...
    try {
      ...
      const response = await GetLearningObjectsByIdService(idP);
  ...
  if (error || !learningObject) {
    return (
      <Container maxWidth="xl">
        <CustomBreadcrumbs
          heading={t("learning-objects.breadcrumbs.myLearningPathsDetails")}
          links={[
            {
            ...
            {
              name: t("learning-objects.breadcrumbs.myLearningPathsDetails"),
              href: paths.dashboard.userLearning.myLearningPathsDetails(id || "")
            },
