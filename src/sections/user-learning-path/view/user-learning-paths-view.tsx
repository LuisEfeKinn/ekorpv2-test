...
export function UserLearningPathsView() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const response = await GetLearningPathsByUserManagementIdService(params);

      // Validación segura de la respuesta con nueva estructura
      const data = response?.data?.data || [];
      const meta: any = response?.data?.meta || {};
      console.log('Loaded learning paths data:', data);

      setLearningPathAssignments(data);
      // Nueva API usa meta.totalItems en lugar de meta.itemCount
      setTotalItems(meta?.totalItems || meta?.itemCount || data.length || 0);
