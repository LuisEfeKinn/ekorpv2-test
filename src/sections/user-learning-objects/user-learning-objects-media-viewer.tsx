...
export function UserLearningObjectsMediaViewer({ 
  media, 
  showCarousel = true,
  blockId,
  isCompleted,
  ...
  const handleCloseLightbox = useCallback(async () => {
    // For images: mark as completed when closing the lightbox
    // Only if not already completed to avoid sending duplicate WhatsApp messages
    if (
      ...
      try {
        await onCompleteActivity(blockId);
      } catch (error) {
        console.error('Error completing image activity:', error);
      }
    }
    setSelectedMedia(null);
  }, [selectedMedia, blockId, isCompleted, onCompleteActivity]);
            ...
            >
              ...
              <Stack spacing={1} sx={{ flex: 1, p: { xs: 2, sm: 2 }, minWidth: 0 }}>
                ...
                {onClick && (
                  ...
                  >
                    {t('learning-objects.details.preview.media.clickToExpand')}
                  </Typography>
                )}
              </Stack>
            </Stack>
            ...
            >
              ...
              <Stack spacing={1} sx={{ flex: 1, p: { xs: 2, sm: 2 }, minWidth: 0 }}>
                ...
                >
                  ...
                  >
                    {t('learning-objects.details.preview.media.clickToPlay')}
                  </Typography>
                </Stack>
              </Stack>
            </Stack>
          ...
          <Card variant="outlined" sx={{ p: 2 }}>
            ...
            <Box sx={{ mt: 2 }}>
              <audio controls style={{ width: '100%' }}>
                ...
                {t('learning-objects.details.preview.media.audioNotSupported')}
              </audio>
            </Box>
          </Card>
        );
          ...
          <Card variant="outlined" sx={{ p: 2 }}>
            <Stack direction="row" spacing={2} alignItems="center">
              ...
              <Button
                ...
                download
              >
                {t('learning-objects.details.preview.media.download')}
              </Button>
            </Stack>
          </Card>
        );
...
function MediaLightbox({ media, onClose, blockId, isCompleted, onCompleteActivity }: MediaLightboxProps) {
  const [zoom, setZoom] = useState(1);
  const videoRef = useCallback((node: HTMLVideoElement | null) => {
    // Only set up listener if not already completed (avoid duplicate WhatsApp messages)
    if (!node || !blockId || isCompleted || !onCompleteActivity || media?.type !== 'video') return undefined;
    ...
    const handleTimeUpdate = async () => {
      ...
      if (currentTime / duration >= 0.95) {
        ...
        try {
          ...
          console.error('Error completing video activity:', error);
        }
      }
    };
  ...
  }, [blockId, isCompleted, onCompleteActivity, media?.type]);
