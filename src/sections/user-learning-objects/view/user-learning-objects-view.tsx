...
import {
  SingleSignOnService,
  GetEmployeesEnrollmentPaginationService,
  GetLearningPathEmployeesPaginationService
} from 'src/services/employees/employment-enroll.service';
...
export function UserLearningObjectsView() {
  ...
  }, [table.page, table.rowsPerPage, currentFilters.name, currentFilters.status, currentFilters.learningPathId, t]);
  ...
  const handleGoToCourse = useCallback(
    async (enrollment: IEnrollment) => {
      if (enrollment.isEkorp) {
        router.push(paths.dashboard.userLearning.myLearningDetails(enrollment.enrollmentId as any));
        return;
      }

      try {
        const response = await SingleSignOnService(Number(enrollment.enrollmentId));
        const urlRedirect = response?.data?.urlRedirect;

        if (!urlRedirect) {
          throw new Error('urlRedirect not found in SSO response');
        }

        // window.location.assign(urlRedirect);
        window.open(urlRedirect, '_blank');
      } catch (error) {
        console.error('Error executing single sign on:', error);
        toast.error(t('learning-objects.messages.error.loading'));
      }
    },
    [router, t]
          ...
          {notFound ? (
            ...
            <Grid container spacing={3}>
              {enrollments.map((enrollment) => {
                // El progreso ya viene como número desde el servicio
                ...
                const learningObjectData = {
                  ...
                  bannerUrl: enrollment.course.bannerUrl,
                  categoryName: enrollment.course.categoryName,
                  difficultyLevelName: enrollment.course.difficultyLevelName,
                  isActive: true,
