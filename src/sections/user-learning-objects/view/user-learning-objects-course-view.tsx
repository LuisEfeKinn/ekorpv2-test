...
import { FinishedActivitiesService } from 'src/services/learning/courses.service';
import { GetAiCourseByIdService } from 'src/services/ai/SaveOrUpdateAiCourseGeneration.service';
...
export function UserLearningObjectsCourseView({ id, idC }: Props) {
  ...
  const handleCompleteActivity = useCallback(async (activityId: string) => {
    try {
      const response = await FinishedActivitiesService(activityId);
