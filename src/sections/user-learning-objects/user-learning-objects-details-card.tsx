...
export function LearningObjectDetailsCard({ learningObject, progress = 0 }: Props) {
  ...
  const safeData = useMemo(
    () => ({
      ...
      tags: Array.isArray(learningObject?.tags)
        ? learningObject.tags.map((tag) => String(tag).trim()).filter(Boolean)
        : typeof learningObject?.tags === "string"
        ? learningObject.tags.split(",").map((tag) => tag.trim()).filter(Boolean)
        : [],
  ...
  const TABS = [
    ...
    ...(safeData.videoUrl
      ...
      : []),
    ...((learningObject?.courses && learningObject.courses.length > 0)
      ? [
        {
          value: "courses",
          label: t("learning-paths.details.programCourses"),
      ...
      <Grid size={{ xs: 12, lg: 8 }}>
        <Card sx={{ mb: 3 }}>
          ...
          <CardContent sx={{ p: { xs: 2, md: 4 } }}>
            ...
            {/* Tab: Cursos del Programa */}
            {currentTab === "courses" && learningObject?.courses && learningObject.courses.length > 0 && (
              <Stack spacing={3}>
                <Box>
                  <Typography variant="h5" fontWeight={700} sx={{ mb: 1 }}>
                    {t("learning-paths.details.programCourses")}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {t("learning-paths.details.programCoursesDescription")}
                  </Typography>
                </Box>
                ...
                <Grid container spacing={2}>
                  {learningObject.courses
                    ...
                    .map((course) => {
                      ...
                      const courseImageUrl = isValidImageUrl(course.bannerUrl) ? course.bannerUrl : null;
                      const isEkorp = course.isEkorp === "true";

                      return (
                        <Grid key={course.id || course.lmsCourseId} size={{ xs: 12, sm: 6, md: 4 }}>
                          <Card
                            sx={{
                              height: '100%',
                              display: 'flex',
                            ...
                            <CardContent sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', p: 2 }}>
                              ...
                              {isEkorp && (
                                <Button
                                  ...
                                  onClick={() => {
                                    const learningObjectId = params.id || learningObject.id;
                                    const courseId = course.id || course.lmsCourseId;
                                    if (learningObjectId && courseId) {
                                      router.push(paths.dashboard.userLearning.myCourseLearningDetails(learningObjectId as string, courseId));
                                    }
                                  }}
                                  sx={{
                                    mt: 'auto',
