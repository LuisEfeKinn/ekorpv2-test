...
export const GetCoursesPaginationService = async (params: any) => {
...
type FinishedActivitiesPayload = {
  enrollmentId: number;
  blockId: string;
};

export const FinishedActivitiesService = async (data: FinishedActivitiesPayload) => {
  const endpoint = `${endpoints.learning.courses.finishedActivities}`;
...
export const GetLearningUnitsByEnrollmentIdService = async (enrollmentId: any) => {
  const endpoint = `${endpoints.learning.courses.learningUnitsByEnrollment}/${enrollmentId}`;
