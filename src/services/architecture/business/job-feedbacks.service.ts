...
export const SaveJobFeedbackService = async (jobId: string | number, data: FeedbackCreatePayload) => {
  const response = await axios.post(
    `${endpoints.architecture.business.jobRelations.jobLessonsLearnedAndProposalsForImprovement}/${jobId}`,
    data
  );
  return response;
};
...
export const UpdateJobFeedbackService = async (feedbackId: string | number, data: FeedbackUpdatePayload) => {
  const response = await axios.patch(`/api/feedbacks/${feedbackId}`, data);
  return response;
};

export const DeleteJobFeedbackObjectService = async (relationId: string | number) => {
