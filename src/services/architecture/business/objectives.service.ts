// utils
import type { IStrategicObjectiveFlowNode } from 'src/types/architecture/strategic-objectives';

import axios, { endpoints } from 'src/utils/axios';

// ----------------------------------------------------------------------

export const GetObjectivesFlowService = async (params?: Record<string, string | number | boolean | undefined>) => {
  const response = await axios.get<IStrategicObjectiveFlowNode[]>('/api/objective/flow', {
    params,
  });
  return response;
};
...
export const GetObjectivesPaginationService = async (params: any) => {
  const response = await axios.get<any>('/api/objectives', { 
    params,
  });
  return response;
};
...
export const DownloadObjectivesExcelService = async (params?: any) => {
  const response = await axios.get('/api/objectives/download/excel', {
    responseType: 'blob',
    params,
  });
  return response;
...
export const DownloadObjectivesTemplateService = async () => {
  const response = await axios.get('/api/objectives/download/template', {
    responseType: 'blob',
  });
  return response;
};

export const UploadObjectivesExcelService = async (formData: FormData) => {
  const response = await axios.post('/api/objectives/upload', formData, {
    headers: {
