...
export const GetJobTypesService = async () => {
  const response = await axios.get<unknown>('/api/job-types');
  return response;
};
...
export const DownloadJobsTemplateService = async () => {
  const response = await axios.get('/api/jobs/download/template', {
    responseType: 'blob',
  });
  return response;
};

export const DownloadJobsExcelService = async (params?: any) => {
