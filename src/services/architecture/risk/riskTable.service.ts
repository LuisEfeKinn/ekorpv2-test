...
export const DownloadRisksTemplateService = async () => {
  const response = await axios.get('/api/risk/download/template', {
    responseType: 'blob',
  });
  return response;
};
...
export const DownloadRisksExcelService = async (params?: any) => {
  const response = await axios.get('/api/risk/download/excel', {
    responseType: 'blob',
    params,
  });
  return response;
