...
export const DownloadToolsTemplateService = async () => {
  const response = await axios.get('/api/tools/download/template', {
    responseType: 'blob',
  });
  return response;
};
...
export const UploadToolsService = async (file: File) => {
  const formData = new FormData();
  formData.append('file', file);
  const response = await axios.post('/api/tools/upload', formData, {
    headers: {
