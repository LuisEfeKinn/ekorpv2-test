...
export const GetProcessFlowService = async (params?: ProcessFlowParams) => {
  const response = await axios.get(endpoints.architecture.process.flow.all, { params });
  return response;
};
...
export const DownloadProcessTemplateService = async () => {
  const response = await axios.get('/api/processes/download/template', {
    responseType: 'blob',
  });
  return response;
};

export const DownloadProcessExcelService = async (params?: any) => {
  const response = await axios.get('/api/processes/download/excel', {
    responseType: 'blob',
    params,
  });
  return response;
...
export const UploadProcessService = async (file: File) => {
  const formData = new FormData();
  formData.append('file', file);
  const response = await axios.post('/api/processes/upload', formData, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  });
