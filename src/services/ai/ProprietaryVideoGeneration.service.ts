...
export async function PollVideoStatusService(
  ...
  try {
    ...
    if (data?.movie?.status === 'error' || data?.movie?.success === false || data?.success === false) {
      throw new Error(data?.movie?.message || 'Error en la generación del video');
    }
      ...
      try {
        ...
        if (status === 'done' && statusResponse.movie?.url) {
          ...
          return {
            ...
            consumedCredits: statusResponse.movie.consumed_credits?.[0]?.credits,
            remainingQuota: statusResponse.remaining_quota?.time,
          };
        }
        ...
        // PollVideoStatusService throws an error when JSON2Video reports an error status
        // These are permanent errors (quota exceeded, invalid project, etc.) and should stop polling
        // Only network/timeout errors should be retried
        console.error(`[Proprietary Video] Poll error on attempt ${attempts}:`, pollError.message);
        
        // If the error message contains quota, exceeded, invalid, or other permanent error indicators, stop immediately
        const permanentErrorKeywords = ['quota', 'exceeded', 'invalid', 'not found', 'forbidden'];
        const isPermanentError = permanentErrorKeywords.some(keyword => 
          pollError.message.toLowerCase().includes(keyword)
        );
        
        if (isPermanentError) {
          console.error('[Proprietary Video] Permanent error detected, stopping polling');
          throw pollError;
        }
        
        // For other errors (network, timeout), continue polling
        console.warn('[Proprietary Video] Temporary error, will retry...');
      }
    }
  ...
  return GenerateProprietaryVideoService(
    {
      ...
      image_model: proprietaryOptions.image_model,
    },
    { onProgress }
  );
}
