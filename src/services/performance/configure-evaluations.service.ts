...
export const LaunchEvaluationCampaignService = async (id: any) => {
  const launchEndpoint = `${endpoints.performance.configureEvaluations.launch}/${id}/launch`;
  const response = await axios.post(launchEndpoint);
  return response;
};

export const CancelEvaluationCampaignService = async (id: any) => {
  const cancelEndpoint = `${endpoints.performance.configureEvaluations.cancel}/${id}/cancel`;
