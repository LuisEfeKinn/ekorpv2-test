...
export const DuplicateConfigureTestsService = async (id: any) => {
  const duplicateEndpoint = `${endpoints.performance.configureTests.duplicate}/${id}/duplicate`;
  const response = await axios.post(duplicateEndpoint);
  return response;
}