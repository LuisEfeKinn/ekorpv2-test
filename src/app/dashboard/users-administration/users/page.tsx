...
export default function Page() {
  ...
  const [userToDelete, setUserToDelete] = useState<IUserClarity | null>(null);
  ...
  const buildQueryParams = useCallback(
    (options?: { useDebounced?: boolean }) => {
      ...
      const usuario = currentFilters.usuario.trim();
  ...
  const handleOpenDeleteDialog = useCallback((user: IUserClarity) => {
  ...
  const handleConfirmDeleteUser = useCallback(async () => {
    ...
    try {
      ...
      await handleDeleteUser(userToDelete.idusuario);
      setDeleteDialogOpen(false);
      setUserToDelete(null);
    } finally {
  ...
  return (
    <DashboardContent>
      ...
      <Card>
        ...
        {canReset && (
          <FiltersResult totalResults={totalItems} onReset={handleResetFilters} sx={{ p: 2.5, pt: 0 }}>
            ...
            <FiltersBlock
              label={`${t('usersClarity.table.filters.usuario')}:`}
              isShow={currentFilters.usuario.trim().length > 0}
            >
              <Chip
                {...chipProps}
                label={currentFilters.usuario}
                onDelete={() => updateFilters({ usuario: '' })}
              />
            </FiltersBlock>
            ...
            <FiltersBlock
              label={`${t('usersClarity.table.filters.estadoUsuario')}:`}
              isShow={currentFilters.status !== 'all'}
            >
              <Chip
                {...chipProps}
                label={
                  currentFilters.status === '0'
                    ? t('usersClarity.table.filters.active')
                    : currentFilters.status === '1'
                      ? t('usersClarity.table.filters.preregistered')
                      : currentFilters.status === '2'
                        ? t('usersClarity.table.filters.inactive')
                        : currentFilters.status === '3'
                          ? t('usersClarity.table.filters.deleted')
                          : t('usersClarity.table.filters.all')
                }
                onDelete={() => updateFilters({ status: 'all' })}
              />
            </FiltersBlock>
          </FiltersResult>
        )}
      ...
      <Dialog open={deleteDialogOpen} onClose={handleCloseDeleteDialog} fullWidth maxWidth="xs">
        <DialogTitle>{t('usersClarity.dialogs.delete.title', { defaultValue: 'Eliminar usuario' })}</DialogTitle>
        <DialogContent sx={{ typography: 'body2' }}>
          {t('usersClarity.dialogs.delete.description', {
            defaultValue: '¿Estás seguro de que deseas eliminar este usuario? Esta acción no se puede deshacer.',
          })}
          {userToDelete ? <Box sx={{ mt: 1, typography: 'subtitle2' }}>{userToDelete.usuario}</Box> : null}
        </DialogContent>
        <DialogActions>
          <Button variant="outlined" color="inherit" onClick={handleCloseDeleteDialog} disabled={deletingUser}>
            {t('usersClarity.dialogs.delete.cancelButton', { defaultValue: 'Cancelar' })}
          </Button>
          <LoadingButton variant="contained" color="error" onClick={handleConfirmDeleteUser} loading={deletingUser}>
            {t('usersClarity.dialogs.delete.confirmButton', { defaultValue: 'Eliminar' })}
          </LoadingButton>
        </DialogActions>
      </Dialog>
...
function UsersClarityTableToolbar({
  ...
  const handleUsuarioChange = useCallback(
    (event: React.ChangeEvent<HTMLInputElement>) => {
      onFilters('usuario', event.target.value);
    },
    [onFilters]
  );
          ...
          >
            ...
            >
              ...
              <FormControl fullWidth>
                <InputLabel id="users-clarity-status-filter-label">
                  {t('usersClarity.table.filters.estadoUsuario')}
                </InputLabel>
                <Select
                  labelId="users-clarity-status-filter-label"
                  value={filters.status}
                  label={t('usersClarity.table.filters.estadoUsuario')}
                  onChange={handleSelectChange('status')}
                >
                  <MenuItem value="all">{t('usersClarity.table.filters.all')}</MenuItem>
                  <MenuItem value="0">{t('usersClarity.table.filters.active')}</MenuItem>
                  <MenuItem value="1">{t('usersClarity.table.filters.preregistered')}</MenuItem>
                  <MenuItem value="2">{t('usersClarity.table.filters.inactive')}</MenuItem>
                  <MenuItem value="3">{t('usersClarity.table.filters.deleted')}</MenuItem>
                </Select>
              </FormControl>
