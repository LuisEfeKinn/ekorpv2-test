 ...
 */
/**
 * Normalize a parameter label to a canonical UPPER_SNAKE_CASE key
 * e.g. "Group ID" => "GROUP_ID", "api_key" => "API_KEY"
 */
...
export function extractParameters(provider: IProvider): Record<string, string> {
  ...
  provider.parameters.forEach((p) => {
    if (p.label && p.value) {
      // Store under both the original label and the normalized key so
      // lookups work regardless of how the backend names the parameter.
      params[p.label] = p.value;
      params[normalizeParamKey(p.label)] = p.value;
    }
  });
  return params;
}
...
export async function getProviderConfig(providerName: string, token?: string): Promise<IProviderConfig> {
  ...
  // Helper: try a list of candidate keys in order
  const pick = (...keys: string[]) => keys.map((k) => parameters[k]).find(Boolean) || null;

  return {
    provider,
    models: provider.models,
    parameters,
    apiKey:               pick('API_KEY', 'APIKEY', 'API KEY', 'api_key'),
