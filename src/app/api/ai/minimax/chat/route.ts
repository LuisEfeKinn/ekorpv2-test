...
export async function POST(req: NextRequest) {
  try {
    ...
    // Ensure the endpoint has the correct chat path
    if (!apiUrl.includes('/text/chatcompletion')) {
      apiUrl = apiUrl.replace(/\/$/, '') + '/text/chatcompletion_v2';
    }

    // Remove any existing GroupId param (may be an empty placeholder from backend)
    apiUrl = apiUrl.replace(/([?&])GroupId=[^&]*/g, '$1').replace(/[?&]$/, '');

    // Always append the actual GroupId value
    ...
    //   url: apiUrl.replace(modelConfig.apiKey ?? '', '***'),
    //   model: requestBody.model,
    //   messageCount: messages.length,
    //   stream: requestBody.stream,
    //   groupId: groupIdToUse ? '(set)' : '(MISSING)',
    // });
