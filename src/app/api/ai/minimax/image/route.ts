...
export async function POST(req: NextRequest) {
  try {
    ...
    if (size) {
      ...
      else if (size.includes('1792x1024') || size.includes('1536x1024')) aspectRatio = '16:9';
      else if (size.includes('1024x1792') || size.includes('1024x1536')) aspectRatio = '9:16';
    }
    
    // Get model from request or use default from config
    // MiniMax expects lowercase model names
    const modelKey = (model || modelConfig.model?.modelKey || 'image-01').toLowerCase();
