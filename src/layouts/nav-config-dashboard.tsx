   ...
   */
  {
    ...
    items: [
      ...
      },
      {
        ...
        path: paths.dashboard.ai.programGenerator.root,
        icon: <Iconify icon="solar:book-bold" width={24} />,
        children: [
          { title: 'List', path: paths.dashboard.ai.programGenerator.root },
          { title: 'Create', path: paths.dashboard.ai.programGenerator.create },
        ],
      },
    ],
  },
...
export const useNavData = (modules?: UserModule[]): NavSectionProps['data'] => {
  ...
  try {
    ...
    if (archIdx >= 0) {
      ...
      const strategicObjectivesMapTitle = t('business.items.strategicObjectives.map');
      ...
      // Remove Map item if present (Requested by user)
      strategicObjectivesItem.children = strategicObjectivesItem.children.filter(
        (it) => it.title !== strategicObjectivesMapTitle
      );
